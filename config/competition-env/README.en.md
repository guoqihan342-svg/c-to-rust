...
- Windows/OpenCode preflight status: one focused local `opencode_multi_worker_evaluate_profile` run previously hung during the Windows/OpenCode preflight phase and was manually terminated; after adding harness process-tree timeout/cleanup, the focused OpenCode entrypoint now passes the runner and `--require-local-artifacts` deep validation under local `local-simulation`, producing an indexed `opencode-safety-transform-attempt-1.json`. This is runtime/reproduction evidence only, not a semantic gate, and it does not change `translation_coverage_numerator`; a real competition host still needs separate `competition-exact` evidence.
