...
- Known Windows/OpenCode preflight status: one focused local `opencode_multi_worker_evaluate_profile` run hung during the Windows/OpenCode preflight phase and was manually terminated; this is not a passed preflight and does not produce a reusable or publishable local OpenCode safety-transform attempt artifact. The next real smoke/OpenCode entrypoint run must rely on harness process-tree timeout/cleanup or move to a Linux/CI runner; this is only a runtime/reproduction blocker, not a semantic gate, and it does not change `translation_coverage_numerator`.
