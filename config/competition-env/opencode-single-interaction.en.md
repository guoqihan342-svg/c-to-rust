...
OpenCode safety-transform attempt artifacts are audit evidence only. A publishable `opencode-safety-transform-attempt` must bind hash-bound `handoff_contract`, hash-bound `opencode_session_evidence`, and `contract_verification`; the handoff launch policy must be `opencode` + `GLM-5.1` + `max`, and validators must recompute `contract_verification` from the session before accepting the attempt provenance. Missing session evidence, hash drift, non-GLM model, command drift, runtime-env drift, or worker-summary drift fails closed. Chat output is never semantic evidence.
