...
OpenCode safety-transform attempt artifact 只是审计证据。可发布的 `opencode-safety-transform-attempt` 必须绑定 hash-bound `handoff_contract`、hash-bound `opencode_session_evidence` 和 `contract_verification`；handoff launch policy 必须是 `opencode` + `GLM-5.1` + `c2rust-migrator` + `max`，validator 必须从 session 重新计算 `contract_verification` 后才能接受该 attempt provenance。缺少 session evidence、hash drift、非 GLM 模型、错误 OpenCode agent、命令漂移、runtime-env 漂移或 worker-summary 漂移都必须 fail-closed。chat output 永远不是 semantic evidence。
