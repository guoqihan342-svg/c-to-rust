...
- Windows/OpenCode preflight 状态：一次本机 focused `opencode_multi_worker_evaluate_profile` 运行曾卡在 Windows/OpenCode preflight 阶段并被人工终止；本轮加入 harness 进程树 timeout/cleanup 后，focused OpenCode entrypoint 已在本机 `local-simulation` 下通过 runner 与 `--require-local-artifacts` 深校验，并生成可索引的 `opencode-safety-transform-attempt-1.json`。该事项只是 runtime/reproduction evidence，不是 semantic gate，也不改变 `translation_coverage_numerator`；真实比赛机仍需单独取得 `competition-exact` 证据。
