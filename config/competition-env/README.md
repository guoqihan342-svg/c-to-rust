...
- `bundle-manifest.json`：比赛配置目录的机器可读归档合同，hash 绑定环境 profile、镜像配置、shell 入口、planned batch profiles、judge entrypoint index、review checklist 和 OpenCode runbook；同时通过 `external_refs` 绑定 `requirements.txt`、`opencode.json`、FlashDB bootstrap、repo-local C2Rust migrator 入口、Rust dependency locks、核心 CI workflow 和 `.opencode/agents/c2rust-migrator.md`。Codex skill 只作为开发者内部操作手册，不再作为比赛复现输入进入归档。`validate_judge_entrypoints` 会校验它，但它不是 semantic gate。
...
- `validate_judge_entrypoints` 也支持 `--entrypoint-id`，用于直接校验 focused 子集或 focused local artifacts；它会同步收窄 `test_contract.required_entrypoint_ids`，未知 entrypoint id fail-closed。该模式只用于 H8/H9 分段复现和排障，不能替代全量 runner 或真实 GLM/OpenCode host 验收。
...
- OpenCode safety-transform attempt 公开索引：OpenCode evaluate entrypoint 声明 `expected_artifacts.opencode_safety_transform_attempt`，`judge-evidence-index.json` 会把 worker 侧 `opencode-safety-transform-attempt-<attempt>.json` 提升到顶层 `evidence_artifact_refs`，因此 milestone bundle 和 public release packet 的 `publication_manifest.published_artifact_refs` 可以展示该证据。该 artifact 只审计 OpenCode 命令合同、单 patch/round、5 轮上限、rollback/retry 和 unsafe delta 绑定，不是 semantic gate，也不增加 `translation_coverage_numerator`。
...
该 profile 只是把 `init-run`、`plan-source-file` 或显式 `workers[]` 计划、`run-plan --execute-merge` 固化为一条命令；语义接受仍只看最终 `competition-run-summary.json`、workflow metrics 和 validator。当前 FlashDB `real-fdb-calc-crc32` exact typed-IR generated draft 已通过 `generated_draft_acceptance.status=passed`，并绑定 rustc、generated replay、C oracle、schema diff、negative diff 和 unsafe gates；raw C2Rust baseline 仍是 `candidate_context_only`，其它 generated draft 仍不得借用该验收。before/after、accepted-evidence 和 demo 展示 profile 使用 `auto_retry=true` / `max_workers=4`；显式 multi-worker evaluate profile 通过 `max_workers=2` 保持确定性 fan-out/fan-in，而 OpenCode evaluate profile 使用 `auto_retry=true` / `max_workers=2`，用于在必需的 `OpenCode + GLM-5.1 + c2rust-migrator + max` 启动策略下演练 5 轮上限。`flashdb-fdb-utils-opencode-deepseek-local-rehearsal.json` 是单 worker 本地排练 profile，显式设置 `opencode_allow_non_competition_model=true` 并使用 `deepseek/deepseek-v4-pro`；它只能验证本机 OpenCode/DeepSeek runtime 与 harness 编排，不能关闭 H9 或替代 GLM-5.1 比赛 host artifacts。retry 只允许失败 worker 消费已落盘 repair hint 并在 5 轮上限内重试，`max_workers` 是 LangGraph 风格 worker fan-out 和 planner 顺序 fan-in；`--mode opencode` 还会对第一条 shell command 前发生的 OpenCode `database is locked` 瞬时启动锁做窄口径重试，并把 `opencode_process_retries` 写入证据，但不会绕过 contract verification、summary 或 validator。两类 retry 都不能替代 validator。
