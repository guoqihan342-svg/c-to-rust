...
评委入口中的 `competition_environment_smoke` 使用同一 runner，但输出到 `target/competition-smoke-flashdb-judge-entrypoint`；`competition-smoke-summary.json` 会声明 `claim_boundary.semantic_gate=false`，只证明环境和轻量 evidence gate，不声明新的 semantic pass。`validate_judge_entrypoints --require-local-artifacts` 会校验该 summary 的 `profile_id/profile_sha256/proof_class/run_id` 与 `flashdb-harness.json` 一致，并 fail-closed 拒绝 CI/WSL/Windows 本地运行结果冒充 `competition-exact`。
