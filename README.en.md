...
Schema v7 also adds an immutable project-diagnostic intake. The host reopens the current candidate set, managed generation, and canonical RustProjectIR, then admits only structured rustc `error` diagnostics from a proven sandboxed Cargo execution. Cargo check/test now freezes the complete candidate cohort and validates gate order, execution status, unique candidate identity/content ownership, and classification of every error before recording any repair. Unit ownership comes from the cross-binding between RustProjectIR module paths and the candidate set; a cross-module unresolved symbol enters the project queue. An unknown or mixed error, duplicate owner, stale unit path, or toolchain-sensitive rustc error blocks the entire repair batch, and a project-derived unit failure rechecks its source cohort during both record and state transition. Only after whole-batch admission do uniquely owned errors become unit repairs. An `E####` compile error with a repository-relative project location and no unique unit binds the cohort, IR/interface, generation input, raw observation, and verifier receipt. Warnings, generic Cargo failures, diagnostic overflow, and environment or sandbox blockers produce zero intake and zero AI calls.
...
| `project_migration_harness.py` | Arbitrary-repository inventory, SCC/DAG, ContextPacks, role scheduling, gates, and Cargo generations | project plan, SQLite v7, worker requests, last-good project |
