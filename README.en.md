...
| Active translator task | P0-A19 g4 independent Cargo reopening and remaining h4 project-test adapter expansion; bounded single-file Make stdin is closed as h4d and discovered-inventory bidirectional equality as h4e, while P0-A18c/P0-A10 remain finite regressions |
| Current environment proof | `wsl-local-simulation`, not `competition-exact` |
| P0-A19 finite gate | Windows project-migration suite: 1,247 passed and 26 platform-conditional skips out of 1,273; WSL simulation: 1,261 passed and 12 explicit environment-conditional skips out of 1,273; the real C/Rust dual-sandbox oracle passes 4/4 under WSL |
...
The outer OpenCode process on the competition platform does not need one hand-written spec per function. Root `AGENTS.md` directs it to call only `project_migration_harness.py migrate` with the target C repository. That entrypoint joins planning and `run-to-completion` in a fixed order. Only `status=completed` with `semantic_gate=true` ends the run; it must never stop after `plan`, `preflight`, `dispatch`, or an individual gate. The harness discovers compile databases, CMake/Ninja/Meson/Make build facts, compile outputs, static archives, link edges, and translation units, then materializes SCC/DAG structure, content-addressed ContextPack catalogs/frontiers, role portfolios, and SQLite state. `migrate` defaults to `--profile competition` and `--build-closure-policy required`; `--profile development` is only an explicitly selected non-competition compatibility path. Incomplete closure defers every ready worker. `bounded-source` may produce only investigative `semantic_gate=false` candidates and cannot complete a project.
...
Project preflight fixes the exact model, `c2rust-candidate`, `max`, and an immutable agent snapshot. Each attempt inherits an explicit environment allowlist and isolated config/data/state/cache/tmp/out roots. Invocation receipts, the session-export projection, prompt/response SHAs, and provider-execution reports share the same ledger attempt/fence. Provider stdout/stderr first go to temporary files and the host reads only each limit plus one byte; timeout or ledger-start callback failure terminates the whole new process group and waits for it within a bound. Fixed host authorities own acceptance. Candidate gates, project gates, evidence hashes, latest epochs, and immutable candidate sets are stored in SQLite, and provider receipts plus content-addressed evidence are reopened both before promotion and before final project completion. Cargo output uses immutable generations behind an atomic `CURRENT` pointer. Candidate `cargo check/test` runs only in a networkless Linux bubblewrap sandbox. Under rustup, `rustup which` resolves the actual `cargo/rustc/rustdoc`; the bounded toolchain tree is fully content-hashed, rehashed before execution, and mounted read-only. Missing sandbox, oracle, or ABI evidence blocks instead of executing on the host.
...
Project logic acceptance now has `ProjectTestInventory v1` adapters for CMake/CTest and bounded explicit Make recipes. CTest reads only content-bound `ctest --show-only=json-v1` in a sandbox. Make target selection is repository-content-bound: a root `Makefile.am` plus discovered `TESTS/check_PROGRAMS` selects fixed `make -n --no-builtin-rules --no-builtin-variables check`; otherwise the fixed `test` compatibility target remains. Both run only as dry-runs in a read-only, networkless, resource-limited bubblewrap sandbox. Every `Makefile.am` path/SHA/size/declaration set enters the target binding and is rescanned at completion. Mixed CMake/Make repositories select an adapter only when BuildIR and compile-database evidence is unique; ambiguity blocks. Make output accepts only shell-control-free direct argv. Any unmapped command other than `echo` or `printf` blocks, and every inventory-declared test executable must map exactly and uniquely to one materialized BuildIR link target. Ordinary application or CLI executables are still compiled by the Cargo all-target gate, but they do not pollute the test inventory or block merely because they are not tests; actual discovered tests may not be omitted, merged, or guessed. argv and environment values use structured `literal`, `repo-path`, or bounded path-template bindings. Undeclared external paths, links, special files, implicit working-directory inputs, and fixture closures that contain the original C executable fail closed. The host creates one minimal read-only input snapshot and runs the original C executable and Rust target in separate bubblewrap sandboxes. The Rust side never mounts the C executable, C source tree, C runtime state, or C output; a trusted host compares exit, signal, stdout, and stderr. The original C baseline must succeed, so matching failures cannot impersonate equivalence.
...
The positive completion path advances gate-pending candidates through compile, oracle-replay, negative, unsafe-alias, ABI-layout, final, and promotion before integration, Cargo, the C/Rust project oracle, and project-gate aggregation. A logic mismatch is attributed through target, module, and unit identities, including transitive Rust package dependencies of the failed test executable, and atomically returns to AI repair. The completion receipt is atomically written and fsynced before one SQLite transaction binds unit/run completion transitions to its SHA. Restart reopens the receipt, final project gates, and transition set; missing or drifted evidence can only block. Current support covers CMake/CTest direct executables, content-bound Automake `check`, bounded Make `test`, and bounded Meson direct-executable inventory. For an already discovered direct-executable inventory, `project-test-completeness-v1` rejects zero-case, missing, extra, duplicate, or merged-target mappings and binds its SHA into oracle-evidence v4. Arbitrary explicit targets, recursive Make, custom runners, Kconfig, remaining stdin sources, declared or implicit fixtures, full resource contracts, discovery of extra explicit Rust tests, and real held-out whole-project acceptance remain open, so this is not a claim that arbitrary C projects or `competition-exact` qualification already pass.

The competition entrypoint follows. `migrate` is the only state machine outer OpenCode should call; it plans and runs to completion or a stable blocker in one command. `plan`, `preflight`, `dispatch`, `complete`, and `run-to-completion` remain internal diagnostic/recovery tools and are not competition acceptance points:
...
`c2rust-baseline` consumes a repository-owned `compile_commands.json`, invokes C2Rust, applies generic generated-code repairs, discovers every translated public `main`, creates Rust wrappers, runs `check --offline --all-targets` for every Cargo package, and executes every wrapper. Each wrapper retains its compilation unit's original working directory, so tests that use relative fixtures or build outputs still execute in their original context. A Bear-recorded Clang `-cc1` internal child job is ignored only when the same source has a normal driver entry; an isolated internal job still blocks. Repository path escape, a C2Rust-normalized path collision, conflicting source working directories or semantic configurations, transpilation failure, a zero-exit C2Rust run whose hash-bound stderr contains `error:`, no runnable entry point, any Cargo-check failure, or any wrapper failure blocks the run. Reports and raw streams are content-addressed; host absolute paths remain only in `private-local` artifacts.
...
An initial held-out probe of Expat `R_2_8_2` (`c61098da494eea1cbd091118118dcee417faacea`) used its default Linux non-fuzzer closure: 36 compile commands and 29 unique C TUs covering the shared library, `xmlwf`, three examples, the benchmark, and the full suite. Native CTest 1/1 and 4824/4824 internal checks passed. The probe exposed and drove a generic working-directory repair for C2Rust's common-ancestor trimming of a repository-internal source root, so the post-repair rerun is only a development regression. All 29 TUs generated Rust, Cargo all-targets and translated checks 4824/4824 passed, and valid-input differential runs passed for `xmlwf`, all examples, and the benchmark without linking the original C library. Formal report `db902e6124d9afe46d8ba05757b84115d9b89eb88f466f4e879653b6b2df9e8b` remains `blocked` because the low-level runner invokes five argument-requiring non-test wrappers with no arguments. This is not a formal pass; BuildIR still needs to bind original test targets, argv, stdin/fixtures, and equivalent Rust scenarios.

The next stress run used the complete current TinyCC `mob` source at commit `d9d02c56401e43be43760b63f7d82f771a7ed1f6`. It contains 544 tracked files, 232 C files, and 346 files under `tests/`. `./configure --cc=clang --debug --enable-cross`, full `make -j8`, and official `make test -j8` all passed, including multiple cross targets. Bear emitted 25 records for 12 unique C TUs, with 13 target-macro configurations of `tcc.c`. The hardened C2Rust baseline report `2d4a94431f18d245532be61c9103874fdcfc641439daee06280eb51e1512fe15` is explicitly `blocked`: one source has multiple semantic configurations, and C2Rust emits a fatal diagnostic for a `long double` to `uint64` conversion in `tccgen.c`. The gate stops before Cargo, so there is no wrapper-pass evidence. The AI-first plan verified BuildIR and produced 411 migration units plus 1440 assignments, but incomplete link facts and generated-header closure deferred every assignment; no model or Cargo process launched. Make collection also exposed lossless-modeling gaps for recursive sub-Make working directories and the build-time `c2str` generator. TinyCC is therefore development-regression evidence only, not held-out evidence or a translated pass.
...
flowchart LR
    ...
    W --> K
    M --> X["22. Reopen CTest / bounded Make inventory + exact source/Rust target mapping"]
...
Every edge carries schema-bound artifacts with repository-relative paths and SHA-256, not chat conclusions. This stage connects positive candidate-gate advancement, CTest direct-executable, content-bound Automake `check`, bounded Make `test`, bounded Meson direct-executable inventories, bidirectional equality for discovered cases, trusted-host C/Rust dual-sandbox comparison, logic-failure routing to AI repair, a content-addressed oracle, and crash-consistent completion evidence. Remaining work includes constrained Meson/configure generation, arbitrary-target/recursive-Make/Kconfig/custom-test adapters, complete remaining-stdin/fixture/resource projection, completeness of project-test metadata discovery, the A19e7 independent process capability, full A19e8 backend equivalence, and real held-out whole-project acceptance. These artifacts keep `competition_exact=false`; the diagram is an implementation contract, not an arbitrary-C-project success claim.
