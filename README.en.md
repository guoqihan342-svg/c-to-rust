...
- S4 progress-delta ledger: `judge-milestone-bundle.json` now exposes `progress_delta_ledger`, separating translator capability deltas, governance/evidence deltas, and workflow repair deltas. `public-release-packet.json` copies the same ledger at the top level and under `summary`, and release notes render a `Progress Delta Ledger` table so reviewers can distinguish translation-capability movement from harness-governance movement. The ledger is fixed at `semantic_gate=false`, `generated_draft_semantic_pass=false`, and `translation_coverage_numerator=0`.
