...
- Competition-facing agent work must be launched by OpenCode with `GLM-5.1`, and workers should read `.codex/skills/c2rust-migration/SKILL.md` from the repository before acting; the preflight proves model availability with `opencode models` and fails closed with `opencode_model_unavailable` when GLM-5.1 is absent.
