...
| Latest development stage | P0-A18c1: exact candidate API, one empty-completion retry, and repair-call governance |
...
The latest archived WSL auxiliary run, `ai-auxiliary-p0-a18b2c3-deepseek-wsl-20260712-192612`, produced eight candidates from the fixed 12 items: two exact passes, six exact failures, two contract failures, one execution failure, and one provider block. Reopening each bound repair report shows 12 initial plus 6 repair invocations, 18 total; the old aggregate incorrectly reported zero repairs. The six repairs ended as two timeouts, three exhausted runs, and one invalid patch. The exact passes were `real-fdb-calc-crc32` and `real-fdb-is-str`, which only restores an earlier two-pass baseline. The report SHA-256 is `7e8bd77b9afd6c5883d007a47c5058d47a12d1a5d973b86e5ff042b908e2d5c0`.

P0-A18c1 fixes that accounting and removes low-information calls. Fixture-only compile stubs exist only during checking, after which the canonical AI draft is restored byte-for-byte. Repair prompts request only a complete self-contained Rust candidate. AI repair is skipped with a recorded reason when the fresh C oracle did not pass, the target contract is absent, or no structured candidate failure exists. The auxiliary timeout defaults to 300 seconds. The implementation passes 161 focused Windows tests, but the fixed 12-item DeepSeek rerun is still pending, so P0-A18c remains open.
