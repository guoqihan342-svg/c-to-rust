...
fn real_clang_ast_tests_use_shared_visible_gate() {
    ...
    let mut pending = vec![
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests")
            .join("bounded_translation"),
    ];
    while let Some(directory) = pending.pop() {
        for entry in fs::read_dir(directory).expect("bounded translation source directory") {
            let path = entry.expect("bounded translation source entry").path();
            if path.is_dir() {
                pending.push(path);
            } else if path.extension().and_then(|extension| extension.to_str()) == Some("rs") {
                source.push_str(&fs::read_to_string(path).expect("bounded translation source"));
            }
        }
    }
