...
g4c1 进一步把 BuildIR/RustProjectIR 的对象 occurrence 显式映射到 object/source/module 身份，按 occurrence 生成根模块顺序，捕获并重开真实 rustc dep-info，并在每个最终产物中嵌入固定大小的数量/顺序 commitment；archive commitment 必须唯一定位到排除 `lib.rmeta` 的代码对象成员，并绑定成员 ordinal、名称 SHA 和 payload SHA。该证据证明对象/模块闭包、生成顺序承诺和最终代码成员位置，不证明 object、package dependency、native/system 输入在实际 linker 中的跨类别物理交错。g4c2 的统一 argv/trace 与 native/system 映射仍未完成，所以 g4 结构父项保持未勾选；shared-type layout、ownership、init/destruction、cfg/feature、完整 native link 等价性和项目语义门也继续保持 blocker。当前证明仍是 `wsl-local-simulation`，不是 `competition-exact`，更不是整项目 `semantic_pass`。
