...
RustProjectIR v3 已把 BuildIR 的 target scope 重建为确定性的多 package Cargo workspace，支持静态库、共享库和可执行目标，并按 package/target/module 身份隔离同源 variant。生成物会从绑定的 DAG、BuildIR、candidate source 和 target scope 逐字节重开；唯一入口、crate/module 命名、依赖方向或链接参数无法表达时 fail closed。生产 project Cargo 门现在固定执行 locked/offline metadata 与 workspace all-target 实际编译，把 package/target 和实际 rustc target 与 v3 逐项比较，并在沙箱清理前将 bin/cdylib/rlib/staticlib 固化到不可变 CAS 后重开 ELF/archive 类型、ABI 与成员顺序；bin/cdylib linker trace 还会绑定 package/target、同一次 compiler source SHA 和可表达的 package dependency 输入顺序。原始 object/module occurrence、archive member 与 native/system 输入尚未全部逐项映射到 linker 顺序，因此 g4 结构父项仍未关闭；shared-type layout、ownership、init/destruction、cfg/feature 和完整 native link 等价性也继续保持 blocker。
