...
| 当前翻译任务 | P0-A19 g4c2a2 已交付 RustProjectIR v3 分层链接期望；下一步由 g4c2b 捕获实际 rustc/linker argv 与 trace 并逐层对账，h4 剩余项目测试适配并行收口。g4c2/g4c/g4 与整项目语义父项仍未关闭 |
| 当前环境证明 | `wsl-local-simulation`，不是 `competition-exact` |
| P0-A19 有限门禁 | 本阶段仓库级全量均发现 3260 项：Windows 3225 通过、35 项平台条件跳过；WSL 本地比赛模拟 3240 通过、20 项平台条件跳过。两端均零失败/错误；WSL 证明类仍为 `wsl-local-simulation` |
...
RustProjectIR v3 已把 BuildIR 的 target scope 重建为确定性的多 package Cargo workspace，支持静态库、共享库和可执行目标，并按 package/target/module 身份隔离同源 variant。生成物会从绑定的 DAG、BuildIR、candidate source 和 target scope 逐字节重开；唯一入口、crate/module 命名、依赖方向或链接参数无法表达时 fail closed。生产 project Cargo 门固定执行 locked/offline metadata 与 workspace all-target 实际编译，把 package/target 和实际 rustc target 与 v3 逐项比较，并在沙箱清理前将 bin/cdylib/rlib/staticlib 固化到不可变 CAS 后重开 ELF/archive 类型、ABI 与成员顺序；bin/cdylib linker trace 还绑定 package/target、同一次 compiler source SHA 和可表达的 package dependency 输入顺序。

g4c1 进一步把 BuildIR/RustProjectIR 的对象 occurrence 显式映射到 object/source/module 身份，按 occurrence 生成根模块顺序，捕获并重开真实 rustc dep-info，并在每个最终产物中嵌入固定大小的数量/顺序 commitment；archive commitment 必须唯一定位到排除 `lib.rmeta` 的代码对象成员，并绑定成员 ordinal、名称 SHA 和 payload SHA。g4c2a1 又在 C 链接/归档命令分栏前生成统一 `ordered_link_occurrences`，保留 input、search-root、system argument、external native library 的重复 occurrence、原参数 span 和跨类别相对顺序；新鲜 Make 投影固定生成 BuildIR v3，BuildIR 只保存内容绑定身份，不复制宿主敏感路径，并在重开时校验每个分类数组与统一序列一一闭包。link closure v2 与 BuildIR extractor v3 共同承载当前权威；运行时 BuildIR/Make reopener 只接受 schema v3。历史 parser v2 Make 报告只有在宿主按当前语法重新解析且命令逐字段一致时可作为输入兼容，旧 BuildIR v2 不能直接通过运行时验证；未绑定的 forwarded response/linker-script 与未建模 archive 参数全部 fail closed。该子项已在提交 `7febc284` 推送后核对两个远端分支 SHA，只关闭源端不可逆丢序缺口。

g4c2a2 已把上述统一序列投影成每个 RustProjectIR v3 target 的内容绑定 `link_expectation`，按 `module-dep-info-commitment`、`cargo-package-product`、`native-link-input` 和 `system-link-argument` 四层保存 consumer、重复 occurrence、原参数位置、映射身份及完整顺序哈希。重复 C object occurrence 可分别保留并映射到同一 Rust module，由 dep-info 与最终 commitment 证明逻辑替代，不冒充一一对应的物理 Rust linker object；当前 Cargo 表达无法证明重复 package product 的物理发射，因此直接阻断。native/system occurrence 不去重，system 参数只接受严格闭集。任意 search root、response file、仓库位置型 native 输入、group/as-needed/whole-archive 控制、`-Bstatic/-Bdynamic` 切换、路径型或未知 system 参数均 fail closed；BuildIR 还新增不含宿主路径的 response-file presence 摘要，避免展开后丢失来源。Cargo package linker witness 同时核对依赖出现基数，native requirement 身份按实际 name/format 生成。本阶段仓库级全量 Windows 3260 项中 3225 通过、35 项平台条件跳过；WSL 本地比赛模拟 3260 项中 3240 通过、20 项平台条件跳过，两端均零失败/错误。该层只定义可重算的结构期望，尚未捕获并核对实际 rustc/linker argv 与完整 native/system trace，所以 g4c2/g4c/g4 结构父项继续未勾选；shared-type layout、ownership、init/destruction、cfg/feature、完整 native link 等价性和项目语义门也继续保持 blocker。当前证明仍是 `wsl-local-simulation`，不是 `competition-exact`，更不是整项目 `semantic_pass`，不能据此提高真实项目翻译成功率分子。
