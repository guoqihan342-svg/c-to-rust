...
| 当前翻译任务 | P0-A19 g4c2a1 已交付源端跨类别链接顺序权威；下一步把它投影进 RustProjectIR v3，并捕获实际 rustc/linker argv 与 trace 完成分层对账。h4 剩余项目测试适配并行收口；g4c2/g4c/g4 与整项目语义父项仍未关闭 |
| 当前环境证明 | `wsl-local-simulation`，不是 `competition-exact` |
| P0-A19 有限门禁 | 本阶段仓库级全量均发现 3250 项：Windows 3215 通过、35 项平台条件跳过；WSL 本地比赛模拟 3230 通过、20 项平台条件跳过。两端均零失败/错误；WSL 证明类仍为 `wsl-local-simulation` |
