...
| 最近开发阶段 | P0-A18b2c2：4 个 mutable-out/report renderer 已全部计划化 |
| 当前翻译任务 | P0-A18b2c3：清理剩余 3 个 record-buffer/opaque-context legacy renderer |
...
候选和 repair prompt 会在 ContextPack 投影前单独展示同一份 generated replay source contract，并重申函数签名、Rust public API、raw-pointer 与 unsafe 策略。完整 replay source 和 ReplayCallPlan payload 均只出现一次。声明式 plan 通过受限 DSL 绑定 C source function、Rust `api_name`、参数顺序/类型、C 参数映射、保留的 length、返回/字段类型、fixture codec、ABI、unsafe 和 plan SHA；renderer、provider readiness 与 fresh binding 使用同一对象。zlib/libuv、`fdb_is_str` 与 `fdb_calc_crc32` 已走该路径，后两者同时验证 legacy semantic-contract 规范化、inline fixture SHA 与 JSON byte-array codec；`fdb_blob_make/kv_to_blob/set/del` 已从函数名判断改为语义合同判断。9 个 record-state、4 个 scripted external/call-continue、4 个 scalar-return、4 个 mutable-out/report 和 2 个 record-pointer identity adapter 已计划化；剩余工作是 record buffer/length、opaque key、opaque key/value 3 个 legacy adapter。record identity 使用闭合 JSON 路径、递归 record initializer、受限 null/constant leaf、调用前后 reference identity 和显式 noalias proof，不接受任意 Rust 表达式或持久化地址。动态 slice 使用有界 `Vec<i32>` initializer、最多 4096 项的 `i32_slice/i32_vec` codec、whole-binding observation 和不接受任意 Rust 表达式的 fixture relation DSL；slice-sum 同时复用 `i32_slice` 与固定单元素输出 binding。call-metadata 通过显式 `input_fixture_fields`、contexts 和 source_calls 闭合，不再猜测 fixture 字段或解析 C 文本。
