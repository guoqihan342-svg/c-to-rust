...
本 change 采用 `C:\Users\Administrator\Downloads\c2r-migration-pipeline-design.md` 的主线，并按项目要求修正：测试轮数不写死；不区分廉价/强模型；`unsafe` 预算为小于 10%；C2Rust、`CLANG_PATH` 驱动的 clang AST dump JSON、tree-sitter 和 Agent 都是候选或事实来源，最终接受权只属于验证门禁。`F:\agent\c2rust-master` 是本机参考 C2Rust 源树，不是目标仓库；实现必须能在缺少可执行 C2Rust 或 clang AST dump 工具时产出 skipped/blocked evidence，而不是伪造成功。

## Goals / Non-Goals

**Goals:**

- 建立 C2Rust baseline migration pipeline：真实 C 源码绑定、build profile、tree-sitter 句法索引、`CLANG_PATH` 驱动的 clang AST dump JSON 语义事实、可选 C2Rust unsafe baseline、L0-L4 route decision、Agent candidate、validation profile 与 L3 evidence manifest。
...
### Decision 2: clang AST dump / compile profile owns semantic facts; tree-sitter owns syntax indexing
...
- compile profile, compile commands, `CLANG_PATH`-driven clang AST dump JSON, C2Rust baseline, or explicit unsupported records must provide typedef, macro expansion, ABI, integer width, struct layout, implicit cast, and declaration disambiguation facts.
...
- C2Rust or `CLANG_PATH` clang AST dump unavailable on the host -> record skipped/blocked evidence and keep deterministic gates runnable; use WSL/Linux/CI later for accepted semantic runs.
...
- Should future typed facts continue through clang JSON AST dumps, or should a separate explicit libclang adapter be added later?
