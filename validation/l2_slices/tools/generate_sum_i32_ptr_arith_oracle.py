#!/usr/bin/env python3
"""Generate C oracle evidence for the demo sum_i32_ptr_arith pointer-arithmetic slice."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--fixture-output",
        default="validation/l2_slices/fixtures/sum-i32-ptr-arith-c-oracle.json",
    )
    parser.add_argument(
        "--evidence-output",
        default="validation/evidence/demo/l3-sum-i32-ptr-arith-c-oracle.json",
    )
    parser.add_argument(
        "--producer-output",
        default="validation/evidence/demo/l3-sum-i32-ptr-arith-c-oracle-producer-evidence.json",
    )
    ...
    report = {
        ...
        "non_goals": [
            ...
            "No pointer arithmetic writes such as *(out + i) = value.",
            "No arbitrary pointer arithmetic beyond *(values + i) input reads.",
        ],
    }
...
src="$tmpdir/sum_i32_ptr_arith_oracle.c"
exe="$tmpdir/sum_i32_ptr_arith_oracle"
...
""".strip()
    ...
    return {
        "stdout": stdout,
        "compile_command": "wsl sh -lc gcc -std=c99 -Wall -Wextra -Werror sum_i32_ptr_arith_oracle.c",
        "run_command": "wsl sh -lc ./sum_i32_ptr_arith_oracle",
    }
...
static int sum_i32_ptr_arith(const int* values, int len, int* out) {{
  int total = 0;
  int i;
  for (i = 0; i < len; i++) {{
...
int main(void) {{
  ...
  for (i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {{
    int out = 0;
    int rc = sum_i32_ptr_arith(cases[i].values, cases[i].len, &out);
    int j;
