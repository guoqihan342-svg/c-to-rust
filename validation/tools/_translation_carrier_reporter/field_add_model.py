...
def validate_cases(cases: Any, contract: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, raw_case in enumerate(cases):
        ...
        if reference_outputs(case, contract) != expected:
            raise ReporterError(f"{case_id} expected outputs disagree with wrapping-add model")
    return cases


def reference_outputs(case: dict[str, Any], contract: dict[str, Any]) -> dict[str, Any]:
    inputs = case_inputs(case)
    target = field_value(inputs, contract, contract["state_output"])
    rhs = field_value(inputs, contract, contract["rhs"])
    fields = behavior_fields(contract)
    return {
        fields[0]: bool(contract["return"]["value"]),
        fields[1]: (target + rhs) & U32_MASK,
    }
...
def field_value(
    ...
    fixture_by_path = {
        path: fixture_field
        for fixture_field, path in initializer_fixture_paths(
            entries[str(field["parameter"])]["initializer"]
        )
    }
    path = tuple(str(item) for item in field["field_path"])
    return int(inputs[fixture_by_path[path]])
