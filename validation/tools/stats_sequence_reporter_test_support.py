...
from validation.tools.owner_interior_usize_add_reporter_test_support import plan_payload
from validation.tools.stats_sequence_test_support import (
    REPO_ROOT,
    build_spec,
...
def build_reporter_layout(workspace: Path) -> dict[str, object]:
    ...
    spec, fixture = build_spec(path_prefix=relative_prefix)
    fixture_path = workspace / "fixtures" / "stats-sequence-cases.json"
    source_root = workspace / "upstream"
    source_path = source_root / "stats.c"
    spec_path = workspace / "specs" / "stats-sequence.json"
    ...
    draft_path = auto_dir / f"l3-{spec['slice_id']}-rust-draft.rs"
    draft_path.write_text(renamed_rust_draft(), encoding="utf-8")
    write_json(auto_dir / f"l3-{spec['slice_id']}-auto-translation-plan.json", plan_payload(spec))
    module.generate_rust_replay_test_draft(spec, auto_dir, spec_path)
    ...
    write_json(
        auto_dir / f"l3-{spec['slice_id']}-translator-input.json",
        {
            **common,
            ...
            "build_profile": {"target": spec["build_profile"]["target"], "clang_ast_fixture": None},
        },
    )
    write_json(
        auto_dir / f"l3-{spec['slice_id']}-clang-lowering-report.json",
        {
            **common,
            "status": "lowered",
            "function_name": spec["function_name"],
