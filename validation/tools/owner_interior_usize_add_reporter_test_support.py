...
def build_reporter_layout(workspace: Path) -> dict[str, object]:
    ...
    fixture_path = workspace / "fixtures" / "owner-interior-usize-add-cases.json"
    source_root = workspace / "upstream"
    source_path = source_root / "ledger.c"
    spec_path = workspace / "specs" / "owner-interior-usize-add.json"
    ...
    write_json(
        auto_dir / f"l3-{spec['slice_id']}-auto-translation-plan.json",
        plan_payload(spec),
    )
