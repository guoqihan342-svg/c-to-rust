...
def build_reporter_layout(workspace: Path) -> dict[str, object]:
    ...
    spec, fixture = build_field_add_spec(path_prefix=relative_prefix)
    fixture_path = workspace / "fixtures" / "field-add-cases.json"
    source_root = workspace / "upstream"
    source_path = source_root / "capacity.c"
    spec_path = workspace / "specs" / "field-add.json"
    ...
    draft_path = auto_dir / "l3-merge-quota-field-rust-draft.rs"
    draft_path.write_text(renamed_rust_draft(), encoding="utf-8")
    plan = plan_payload(spec)
    write_json(auto_dir / "l3-merge-quota-field-auto-translation-plan.json", plan)
    ...
    write_json(auto_dir / "l3-merge-quota-field-translator-input.json", translator_input)
    write_json(auto_dir / "l3-merge-quota-field-clang-lowering-report.json", lowering)
    return {
        "emit_args": {
