...
def build_reporter_layout(workspace: Path) -> dict[str, object]:
    ...
    spec, fixture = build_postfix_increment_spec(path_prefix=relative_prefix)
    fixture_path = workspace / "fixtures" / "postfix-increment-cases.json"
    source_root = workspace / "upstream"
    source_path = source_root / "counter.c"
    spec_path = workspace / "specs" / "postfix-increment.json"
    ...
    draft_path = auto_dir / f"{prefix}-rust-draft.rs"
    draft_path.write_text(renamed_rust_draft(), encoding="utf-8")
    write_json(auto_dir / f"{prefix}-auto-translation-plan.json", plan_payload(spec))
    ...
    write_json(auto_dir / f"{prefix}-clang-lowering-report.json", lowering)
    return {
        "emit_args": {
