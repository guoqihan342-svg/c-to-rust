...
def build_reporter_layout(workspace: Path) -> dict[str, object]:
    ...
    spec, fixture = build_projection_spec(path_prefix=relative_prefix)
    fixture_path = workspace / "fixtures" / "projection-cases.json"
    source_root = workspace / "upstream"
    source_path = source_root / "owner_projection.c"
    spec_path = workspace / "specs" / "projection.json"
    ...
    draft_path = auto_dir / "l3-reset-current-phase-rust-draft.rs"
    draft_path.write_text(renamed_rust_draft(), encoding="utf-8")
    write_json(auto_dir / "l3-reset-current-phase-auto-translation-plan.json", plan_payload(spec))
    ...
    write_json(
        auto_dir / "l3-reset-current-phase-translator-input.json",
        {
            **common,
    ...
    write_json(
        auto_dir / "l3-reset-current-phase-clang-lowering-report.json",
        {
            **common,
            "status": "lowered",
            "function_name": spec["function_name"],
