...
from .context_index_store import (
    materialize_prepared_context_catalogs,
...
def build_context_portfolio(
    ...
    try:
        bindings, payloads, prepared = prepare_context_indexes(
            contexts, out_root_rel=out_rel
        )
        materialize_prepared_context_catalogs(prepared, out_root=output)
        dag = portfolio_dag(
    ...
    try:
        ...
        report = {
            ...
            "omitted_blocked_page_count": int(prepared["logical_page_count"]),
            "claim_boundary": {
                "semantic_gate": False,
