...
from .make_dry_run_binding import (
    fixed_make_argv, make_input_sha256, validated_artifact_ref,
...
MAKE_ENVIRONMENT_ALLOWLIST = tuple(key for key, _ in MAKE_ENVIRONMENT)
_PLAN_FIELDS = {
    "schema_version", "artifact_kind", "status", "command", "makefile_path",
    "working_directory", "repository_snapshot_ref", "input_binding_mode",
    "input_sha256",
    "timeout_seconds", "max_stdout_bytes", "max_stderr_bytes",
    "environment_allowlist", "environment", "resource_limits",
    "required_capabilities", "semantic_gate", "translation_coverage_numerator",
    "plan_sha256",
}
...
class MakeDryRunPreflight:
    ...
    def payload(self) -> dict[str, Any]:
        return create_make_host_preflight(
            ...
            make_sha256=self.make_sha256,
            probe_observation_sha256=self.probe_observation_sha256,
...
def create_make_dry_run_plan(
    *, makefile_ref: Mapping[str, Any], input_refs: Sequence[Mapping[str, Any]],
    ...
    makefile_path = str(makefile_ref.get("path"))
    command = fixed_make_argv(
        makefile_path, targets, working_directory=workdir,
    )
    snapshot = (
    ...
    core = {
        ...
        "input_sha256": make_input_sha256(
            makefile_ref, input_refs, toolchain_ref,
            working_directory=workdir,
            repository_snapshot_ref=snapshot,
        ),
...
def validate_make_dry_run_plan(
    ...
    try:
        ...
        raise ValueError("make_dry_run_plan_working_directory_invalid") from error
    makefile_path = value.get("makefile_path")
    snapshot_value = value.get("repository_snapshot_ref")
    try:
        snapshot = (
            validated_artifact_ref(snapshot_value)
            if snapshot_value is not None else None
        )
    except ValueError as error:
        raise ValueError("make_dry_run_plan_snapshot_ref_invalid") from error
    expected_mode = (
        "repository-snapshot" if snapshot is not None else "declared-input-refs"
    )
    if value.get("input_binding_mode") != expected_mode:
        raise ValueError("make_dry_run_plan_input_binding_mode_invalid")
    command = value.get("command")
    if not isinstance(command, list) or len(command) < 9:
        raise ValueError("make_dry_run_plan_command_invalid")
    try:
        expected_fixed = fixed_make_argv(
            makefile_path, command[8:], working_directory=workdir,
        )
...
def run_make_dry_run(
    ...
    preflight: MakeDryRunPreflight | None = None,
) -> MakeDryRunOutcome:
    try:
    ...
    try:
        selected_preflight = preflight or backend.preflight(
            checked, project_root=project_root, runtime_root=runtime_root,
        )
        _validate_preflight(
            selected_preflight, checked, toolchain_ref, sandbox_ref,
        )
    ...
    try:
        result = backend.execute(
            ...
            runtime_root=runtime_root, plan=checked, preflight=selected_preflight,
        )
    except (OSError, TypeError, ValueError):
...
def _execution_outcome(
    ...
    if (
        ...
        return _blocked("make_execution_binding_invalid", started=True, plan_sha=plan_sha)
    flooded = result.output_limit_exceeded or (
