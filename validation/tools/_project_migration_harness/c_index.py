...
def index_translation_units(repo_root: str | Path,
    ...
    for item in translation_units:
        unit = _validated_unit(root, item)
    ...
    for unit in sorted(units, key=lambda item: item["unit_id"]):
        ...
        dependencies = _dependency_context(root, unit)
    ...
    return {
        ...
        "parser": {"kind": "deterministic_lexical_c_subset", "limitations": list(PARSER_LIMITATIONS),
                   "blockers": sorted(blockers, key=_blocker_key)},
        "claim_boundary": {"semantic_gate": False, "translation_coverage_numerator": 0},
    }
def _validated_unit(root: Path, item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        resolved = resolve_repository_path(root, Path(*PurePosixPath(path).parts))
    ...
    return {
        ...
        "sha256": actual,
        "raw": raw,
        "compile_context": _normalize_compile_context(root, item),
    }
...
def _scan_unit(unit: dict[str, Any]) -> dict[str, Any]:
    ...
    while index < len(masked):
        ...
        if char == "{":
            ...
            if close is None:
                blockers.append({"unit_id": unit["unit_id"], "kind": "unbalanced_brace", "byte_offset": index})
                break
        ...
        if char == ";":
            ...
            blockers.append({"unit_id": unit["unit_id"], "kind": "unexpected_closing_brace", "byte_offset": index})
        index += 1
    if not functions and not blockers:
        blockers.append({"unit_id": unit["unit_id"],
                         "kind": "translation_unit_without_function_definition",
                         "byte_offset": 0})
