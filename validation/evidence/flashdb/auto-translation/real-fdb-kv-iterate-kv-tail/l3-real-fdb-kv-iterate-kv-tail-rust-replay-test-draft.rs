...
#[test]
fn replay_real_fdb_kv_iterate_kv_tail_fixture_contract() {
    let _fixture = "validation/l2_slices/fixtures/real-fdb-kv-iterate-kv-tail.json";
    let _api = "fdb_kv_iterate_kv_tail_probe";
    ...
    // Finite fixture sequence; real external callee semantics are not verified.
    {
        ...
        let actual_return = fdb_kv_iterate_kv_tail_probe(&mut actual_failed_address_first_db, actual_failed_address_first_sector_seed, &mut actual_failed_address_first_itr);
    ...
    }
    {
        ...
        let actual_return = fdb_kv_iterate_kv_tail_probe(&mut actual_one_kv_then_failed_db, actual_one_kv_then_failed_sector_seed, &mut actual_one_kv_then_failed_itr);
        assert_eq!(actual_return, false, "{} return drifted", case_id);
    ...
    }
    {
        ...
        let actual_return = fdb_kv_iterate_kv_tail_probe(&mut actual_two_kvs_then_failed_db, actual_two_kvs_then_failed_sector_seed, &mut actual_two_kvs_then_failed_itr);
        assert_eq!(actual_return, false, "{} return drifted", case_id);
