...
- `opencode-preflight` must prove model availability before launch with the same repo-local runtime env. A passed report must include `opencode_model_availability.status=available`, `required_model=GLM-5.1`, and `model_listed=true`. If `opencode models` does not expose `GLM-5.1`, the report must fail closed with `root_cause_key=opencode_model_unavailable`, `opencode_model_availability.status=unavailable` or `probe_failed`, `opencode_run_launched=false`, and no marker. A local-simulation OpenCode pass does not close P0-H9 unless it is rerun on the competition GLM/OpenCode host or an equivalent provider configuration.
