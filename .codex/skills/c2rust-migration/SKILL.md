...
- Public packets and release notes must make bundle blockers review-visible. `public-release-packet.json` must copy `judge_milestone_bundle.blockers` into `summary.blockers`, require the same string-list contract, and keep `status=passed` only when blockers are empty. The packet summary must also expose `published_artifact_ref_status` with status counts, abnormal refs, `semantic_gate=false`, and `translation_coverage_numerator=0`; a passed bundle must publish at least one hash-bound artifact ref, `published_artifact_count` must match `published_artifact_refs.length`, and a passed bundle cannot publish bad artifact ref status such as `sha256_mismatch`, `status_mismatch`, or `missing_expected_sha256`.
