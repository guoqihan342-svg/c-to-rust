...
- Public packets and release notes must make bundle blockers review-visible. `public-release-packet.json` must copy `judge_milestone_bundle.blockers` into `summary.blockers`, require the same string-list contract, and keep `status=passed` only when blockers are empty. The packet summary must also expose `published_artifact_ref_status` with status counts, abnormal refs, `semantic_gate=false`, and `translation_coverage_numerator=0`; a passed bundle must publish at least one hash-bound artifact ref, `published_artifact_count` must match `published_artifact_refs.length`, non-`present/passed` entrypoint artifact refs must become `published_artifact_not_present:<entrypoint>:<artifact>:<status>` bundle blockers, and a passed bundle cannot publish bad artifact ref status such as `sha256_mismatch`, `status_mismatch`, or `missing_expected_sha256`.
...
- `opencode-safety-transform-attempt` must be cross-bound to the real worker summary. Reopen the bound `competition-run-summary.json`, validate it with `validate_competition_run_summary`, require `summary.final_gate_status` to match the worker summary `final_gate.status`, and require the attempt-level `workflow_metrics` path/sha to exactly match the worker summary `workflow_metrics` binding. Public packet boundaries must not claim the attempt is absent when `publication_manifest.published_artifact_refs` contains an `opencode_safety_transform_attempt` ref, even if that ref is unhealthy; abnormal refs must stay visible and consistent with the publication manifest.
...
- Each failed or retried worker attempt must leave machine-readable audit evidence: attempt id, worker id, hint id, previous summary/final gate, selected last-good summary or candidate reference when available, rollback action/status, new summary/final gate, and artifact hashes. For accepted OpenCode safety-transform retries, `accepted_retry_hint.rollback_ids` must be a non-empty string list that exactly matches the hash-bound `rollback_evidence[].path` values; rollback files without matching ids are not a replayable self-heal index. Producers must not mark a retry as `revalidated_passed` unless every rollback id resolves to a hash-bound rollback artifact.
- Do not retry an OpenCode worker after evidence shows the assigned worker command already reached the shell. If an infrastructure error such as SQLite `database locked` appears after `opencode_contract_verification.status=executed`, record `opencode_database_locked_after_worker_command_seen` and `auto_retry_suppressed`; do not let process-level retry or `run-plan --auto-retry` relaunch the same worker command.
...
- Resume/replay producers must not mark an OpenCode replay command as ready from copied worker preflight fields alone. They must reopen the hash-bound `opencode_preflight_report.path`, verify its sha, and require the file body to carry `status=passed` and `contract_verification.status=executed`; otherwise record missing constraints such as `opencode_preflight_report.file_status` or `opencode_preflight_report.file_contract_status`.
