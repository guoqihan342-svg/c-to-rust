...
- Judge-facing `publishability` must be machine-readable, not inferred from prose. A passed local or local-simulation bundle can only be `publishability.status=internal_preview`; external release readiness requires real OpenCode + GLM-5.1, all entrypoints, no blockers, and competition-exact proof. Always keep `required_agent_tool=opencode`, `required_model=GLM-5.1`, `semantic_gate=false`, and `translation_coverage_numerator=0` in that object.
