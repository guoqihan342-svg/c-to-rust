...
- Judge-facing bundles and public release packets must expose the same `opencode_preflight_proof_summary`: it must bind the preflight report, `opencode models` stdout/stderr logs, `opencode_command=opencode`, `opencode_model=GLM-5.1`, `required_model=GLM-5.1`, `model_listed=true`, `contract_status=executed`, `marker_exists=true`, `semantic_gate=false`, and `translation_coverage_numerator=0`. The hash-bound `opencode_session_evidence` used by the summary must self-report `format=jsonl`, `parsed=true`, and `process_returncode=0` before contract recomputation. If OpenCode runtime is enabled and this summary is absent, failed, non-GLM, hash-drifted, not copied from the bound milestone bundle, or backed by a failed/unparsed/non-JSONL session, the public packet must fail closed.
