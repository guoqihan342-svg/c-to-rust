...
12. Condition-only logical not `!expr` `GenericTypedIr` candidates must keep `semantic_pass=false`; `if (!x)` / `while (!x)` may emit an integer zero comparison, and `!(x > 0)` may emit a negated comparison, while value-position `!` with C `int` result semantics, call/inc/dec/deref side effects, pointer/float/unsupported types continue to fail closed.
