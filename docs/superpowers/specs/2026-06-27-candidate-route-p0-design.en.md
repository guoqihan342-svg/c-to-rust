...
12. `GenericTypedIr` logical-not `!expr` candidates in conditions plus narrow value positions must keep `semantic_pass=false`; `if (!x)` / `while (!x)` may emit integer zero comparisons, `!(x > 0)` may emit a negated comparison, and value positions such as `return !x` / assignment RHS / declaration initializer may materialize a C `int` 0/1 result; readonly direct deref read operands such as `!*p` and bounded readonly offset-deref operands such as `!*(p+i)` may emit as ordinary scalar reads; call/inc/dec, unmodeled deref side effects, pointer/float/unsupported types, short-circuit logic, and full C unary `!` continue to fail closed.
13. `GenericTypedIr` comparison-expression candidates in conditions plus narrow value-position C `int` 0/1 materialization must keep `semantic_pass=false`; scalar integer comparisons in `if` / `while` may emit Rust bool conditions, and value positions such as `return x > 0` / assignment RHS / declaration initializer may materialize a C `int` 0/1 result; comparison operands with integral casts may emit when the cast source and target are supported integer types and the post-cast operand types match exactly. Readonly integer pointer parameters may emit as `Option<&[T]>` plus `.is_none()` / `.is_some()` only for direct `== NULL` / `!= NULL` presence checks. Readonly direct deref read operands such as `*p == 0` and bounded readonly offset-deref read operands such as `*(p+i) == 0` may emit as ordinary scalar reads. Arbitrary pointer arithmetic beyond the bounded readonly `*(p+i)` read, arbitrary pointer comparison, floating-point comparison, mixed-width/unsigned conversions that are not aligned by an explicit integer cast, call/inc/dec side-effect operands, pointer dereference/index use after a null check, short-circuit `&&` / `||`, and semantic acceptance continue to fail closed.
14. Scalar integer bitwise OR `|` and left shift `<<` `GenericTypedIr` candidates must keep `semantic_pass=false`; `|` accepts only same-typed lhs/rhs/result values, and `<<` accepts only lhs/result type agreement. Full C bitwise/shift semantics, usual arithmetic conversions, invalid shift counts, signed shift/overflow UB parity, pointer arithmetic, and semantic acceptance continue to fail closed.
15. `GenericTypedIr` compound-assignment integer-promotion candidates must keep `semantic_pass=false`; clang target/result types must match, compute lhs/result types must match, all involved types must be supported integers, and any target/compute mismatch must be emitted through explicit casts before assignment back to the target type. Non-simple targets, value-position compound assignment, pointer arithmetic, floating-point, volatile, complex RHS side effects, and full usual scalar conversions continue to fail closed.
