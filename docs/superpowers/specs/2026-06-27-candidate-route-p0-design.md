...
12. `GenericTypedIr` 的 logical not `!expr` candidate（条件位置 + 窄 value-position）必须保持 `semantic_pass=false`；`if (!x)` / `while (!x)` 可发射为整数零比较，`!(x > 0)` 可发射为反转 comparison，`return !x` / assignment RHS / declaration initializer 等 value-position 可 materialize 为 C `int` 0/1；readonly direct deref read operand（如 `!*p`）和 bounded readonly offset-deref operand（如 `!*(p+i)`）可作为普通标量读发射；call/inc/dec、未建模 deref side effect、pointer/float/unsupported type、短路逻辑和完整 C unary `!` 继续 fail closed。
13. `GenericTypedIr` 的 comparison expression candidate（条件位置 + 窄 value-position C `int` 0/1 materialization）必须保持 `semantic_pass=false`；`if` / `while` 中的标量整数比较可发射为 Rust bool condition，`return x > 0` / assignment RHS / declaration initializer 等 value-position 可 materialize 为 C `int` 0/1；comparison operand 上 source/target 都是可发射整数类型且 cast 后两侧类型完全一致的 integral cast 可发射；readonly integer pointer 参数仅在直接 `== NULL` / `!= NULL` presence check 中可发射为 `Option<&[T]>` 加 `.is_none()` / `.is_some()`；readonly direct deref read operand（如 `*p == 0`）和 bounded readonly offset-deref read operand（如 `*(p+i) == 0`）可作为普通标量读发射；除 bounded readonly `*(p+i)` read 外的任意 pointer arithmetic、任意 pointer comparison、float comparison、未由显式整数 cast 对齐的 mixed-width/unsigned conversions、call/inc/dec 等 side-effect operands、null check 后继续 deref/index 使用、short-circuit `&&` / `||` 和 semantic acceptance 继续 fail closed。
14. `GenericTypedIr` 的标量整数 bitwise OR `|` 和 left shift `<<` candidate 必须保持 `semantic_pass=false`；`|` 只接受左右 operand 与 result 同类型，`<<` 只接受 lhs/result 同类型。完整 C 位运算/位移语义、usual arithmetic conversions、无效 shift count、signed shift/overflow UB parity、指针算术和 semantic acceptance 继续 fail closed。
15. `GenericTypedIr` 的 compound-assignment integer-promotion candidate 必须保持 `semantic_pass=false`；clang target/result 类型必须一致，compute lhs/result 类型必须一致，所有相关类型必须是受支持整数，且 target/compute 不一致时必须通过显式 cast 计算并 cast 回 target type 后赋值。非简单 target、value-position compound assignment、pointer arithmetic、floating-point、volatile、复杂 RHS side effect 和完整 usual scalar conversions 继续 fail closed。
