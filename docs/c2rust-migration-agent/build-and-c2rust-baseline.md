...
Route-governance metrics now read the baseline manifests and publish status/output/compile-only counts into the milestone scorecard. This makes raw C2Rust availability visible to judges without changing the acceptance boundary: a generated and compile-passed baseline is still only candidate context until the shared C oracle/Rust replay/diff/unsafe/final-verification gates accept it.
