...
- `crates/c2r-translator/src/clang_frontend.rs`
  ...
  - It lowers narrow clang `ForStmt` nodes into `ClangStmtSkeleton::For`, accepting only simple scalar declaration/assignment init, including multiple simple `VarDecl` declarators, condition, step, and the existing body subset, while rejecting condition variable slots, empty condition/step, `continue` / `break`, and other unmodeled control flow.
...
- Scoped `ForStmt` is candidate generation only. It covers only simple scalar declaration/assignment init, including multiple simple `VarDecl` declarators, condition, assignment/compound-assignment/postfix inc-dec step, and the existing statement body subset. `continue` / `break` / `goto` / `switch`, condition variable slots, empty condition/step, calls/inc/dec/side effects in the condition, complex init/step, prefix inc-dec step, non-scalar step, full C for-loop control-flow semantics, and semantic acceptance still fail closed.
