...
- `crates/c2r-translator/src/clang_frontend.rs`
  ...
  - It lowers narrow clang `ForStmt` nodes into `ClangStmtSkeleton::For`, accepting only simple scalar declaration/assignment init, including multiple simple `VarDecl` declarators, condition, assignment/compound-assignment/postfix-or-prefix inc-dec step, and the existing body subset. `DoStmt` lowers to `ClangStmtSkeleton::DoWhile`. `BreakStmt` lowers to loop-body `IrStmt::Break`, and `ContinueStmt` lowers to narrow loop-body `IrStmt::Continue`; condition variable slots, empty condition/step, and other unmodeled control flow still fail closed.
...
- narrow scoped `ForStmt`: `for (int i = 0; i < limit; i++) { total = total + i; }`, `for (int i = 0; i < limit; ++i) { total = total + i; }`, and `for (int i = 0, j = 0; i < limit; i++) { total = total + i + j; }` lower to first-class `IrStmt::For` and emit a Rust block plus `while` shape; multiple init declarations emit in source order inside the loop block, do not leak outside that block, and body-local declarations do not leak into the step. Step-position simple scalar `++i` / `--i` lowers only as a statement side effect to `i = i +/- 1`; this does not mean value-position inc/dec is supported;
...
- Structured loop candidate generation is still narrow. `ForStmt` covers only simple scalar declaration/assignment init, including multiple simple `VarDecl` declarators, condition, assignment/compound-assignment/postfix-or-prefix inc-dec step, and the existing statement body subset. `DoWhile` covers only the existing body subset and conditions accepted by the current condition emitter. `break` emits only as direct loop exit. `continue` is supported only in loop bodies: a `while` body emits Rust `continue;`, a `do-while` body checks the condition before `continue;`, and the same scoped `ForStmt` body emits the current step before Rust `continue;`. `goto` / `switch`, condition variable slots, empty `ForStmt` condition/step, calls/inc/dec/side effects in conditions, complex init/step, value-position inc/dec, non-scalar step, full C loop control-flow semantics, and semantic acceptance still fail closed.
...
1. Keep extending generic typed IR coverage with red tests first. Good next cuts are nested pure direct calls, standalone typed clang inc/dec statements, struct/alias memory model design, or wider control-flow semantics for `switch` / `goto`. Multi-`VarDecl` body expansion, multi-declaration `ForStmt` init, assigned-before-read uninitialized scalar locals, fixed-width integer aliases, exact `signed char`, binary-operand integral cast preservation, condition/value-position `&&` / `||`, the simple scalar compound assignment family including narrow clang-proven promotion/truncation, value-position integer implicit cast preservation, pure integer value-position `ConditionalOperator` / `?:`, narrow scoped `ForStmt`, step-position prefix inc/dec, narrow `do-while`, loop-body `break`, narrow loop-body `continue`, readonly `*(p+i)` reads, and mutable pointer output writes now have direct/clang-lowered/real-clang smoke coverage. The bounded readonly `*(p+i)` read, mutable pointer write fail-closed const/offset boundaries, lazy `?:` branch semantics, short-circuit lazy evaluation, for-loop scope boundary, `ForStmt` continue-step ordering, step-position inc/dec boundaries, and `DoWhile` continue-condition check should stay covered by real-clang and fail-closed boundary tests, while arbitrary pointer arithmetic, arbitrary pointer comparison, floating-point comparison, unmodeled mixed-width conversions, side-effect operands, and semantic acceptance must continue to fail closed.
