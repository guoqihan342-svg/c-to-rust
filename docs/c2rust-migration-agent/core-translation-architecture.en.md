...
1. Keep extending generic typed IR coverage with red tests first. Good next cuts are `ForStmt` with an explicit scope model, value-position short-circuit materialization, or a systematic usual-conversion classification. Condition-position `&&` / `||`, the simple scalar compound assignment family, value-position integer implicit cast preservation, and pure integer value-position `ConditionalOperator` / `?:` now have direct/skeleton/real-clang smoke coverage. The bounded readonly `*(p+i)` read and lazy `?:` branch semantics should stay covered by real-clang and fail-closed boundary tests, while arbitrary pointer arithmetic, arbitrary pointer comparison, floating-point comparison, unmodeled mixed-width conversions, side-effect operands, and semantic acceptance must continue to fail closed.
