...
- `crates/c2r-translator/src/clang_frontend.rs`
  ...
  - It lowers narrow clang `ForStmt` nodes into `ClangStmtSkeleton::For`, accepting only simple scalar declaration/assignment init, including multiple simple `VarDecl` declarators, condition, step, and the existing body subset. `DoStmt` lowers to `ClangStmtSkeleton::DoWhile`. `BreakStmt` lowers to loop-body `IrStmt::Break`, and `ContinueStmt` lowers to narrow loop-body `IrStmt::Continue`; condition variable slots, empty condition/step, and other unmodeled control flow still fail closed.
...
- Structured loop candidate generation is still narrow. `ForStmt` covers only simple scalar declaration/assignment init, including multiple simple `VarDecl` declarators, condition, assignment/compound-assignment/postfix inc-dec step, and the existing statement body subset. `DoWhile` covers only the existing body subset and conditions accepted by the current condition emitter. `break` emits only as direct loop exit. `continue` is supported only in loop bodies: a `while` body emits Rust `continue;`, a `do-while` body checks the condition before `continue;`, and the same scoped `ForStmt` body emits the current step before Rust `continue;`. `goto` / `switch`, condition variable slots, empty `ForStmt` condition/step, calls/inc/dec/side effects in conditions, complex init/step, prefix inc-dec step, non-scalar step, full C loop control-flow semantics, and semantic acceptance still fail closed.
...
1. Keep extending generic typed IR coverage with red tests first. Good next cuts are limited mutable pointer writes to `&mut [T]`, nested pure direct calls, step-position prefix inc/dec, or wider control-flow semantics for `switch` / `goto`. Multi-`VarDecl` body expansion, multi-declaration `ForStmt` init, assigned-before-read uninitialized scalar locals, fixed-width integer aliases, exact `signed char`, binary-operand integral cast preservation, condition/value-position `&&` / `||`, the simple scalar compound assignment family including narrow clang-proven promotion/truncation, value-position integer implicit cast preservation, pure integer value-position `ConditionalOperator` / `?:`, narrow scoped `ForStmt`, narrow `do-while`, loop-body `break`, and narrow loop-body `continue` now have direct/real-clang smoke coverage. The bounded readonly `*(p+i)` read, lazy `?:` branch semantics, short-circuit lazy evaluation, for-loop scope boundary, `ForStmt` continue-step ordering, and `DoWhile` continue-condition check should stay covered by real-clang and fail-closed boundary tests, while arbitrary pointer arithmetic, arbitrary pointer comparison, floating-point comparison, unmodeled mixed-width conversions, side-effect operands, and semantic acceptance must continue to fail closed.
