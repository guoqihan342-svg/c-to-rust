...
- `crates/c2r-translator/src/clang_frontend.rs`
  ...
  - It lowers narrow clang `ForStmt` nodes into `ClangStmtSkeleton::For`, accepting only simple scalar init, condition, step, and the existing body subset, while rejecting condition variable slots, empty condition/step, `continue` / `break`, and other unmodeled control flow.
...
- narrow scoped `ForStmt`: `for (int i = 0; i < limit; i++) { total = total + i; }` lowers to first-class `IrStmt::For` and emits a Rust `{ let mut i: i32 = 0i32; while i < limit { total = total + i; i = i + 1i32; } }` shape; init declarations do not leak outside the loop block, and body-local declarations do not leak into the step;
