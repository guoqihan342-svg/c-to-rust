...
- `crates/c2r-translator/src/typed_ir.rs`
  ...
  - The generic emitter supports a narrow readonly pointer direct dereference read: `const uint8_t *p; return *p;` becomes `p: &[u8]` and `return p[0usize];`. This covers only direct `*p` reads from readonly integer pointers; `*(p+i)`, mutable pointers, pointer writes, and nullable pointer index/deref after a null check still fail closed.
...
- readonly integer pointer parameter direct dereference reads, for example `const uint8_t *p; return *p; -> p: &[u8]` and `return p[0usize];`; the same direct deref read can also act as an ordinary scalar read in comparison/logical-not candidate generation, for example `*p == 0` and `!*p`;
...
1. Keep extending generic typed IR coverage with red tests first. Good next cuts are `*(p+i)` pointer arithmetic reads, simple scalar compound assignment lowering, condition-position `&&` / `||`, or explicit usual-conversion classification as separate slices. Arbitrary pointer comparison, floating-point comparison, unmodeled mixed-width conversions, side-effect operands, and semantic acceptance must continue to fail closed.
