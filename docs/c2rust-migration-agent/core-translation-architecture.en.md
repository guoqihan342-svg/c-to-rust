...
1. Keep extending generic typed IR coverage with red tests first. Good next cuts are limited mutable pointer writes to `&mut [T]`, nested pure direct calls, generic inc/dec value semantics, or wider control-flow semantics for `switch` / `goto`. Multi-`VarDecl` body expansion, multi-declaration `ForStmt` init, assigned-before-read uninitialized scalar locals, fixed-width integer aliases, exact `signed char`, binary-operand integral cast preservation, condition/value-position `&&` / `||`, the simple scalar compound assignment family including narrow clang-proven promotion/truncation, value-position integer implicit cast preservation, pure integer value-position `ConditionalOperator` / `?:`, narrow scoped `ForStmt`, step-position prefix inc/dec, narrow `do-while`, loop-body `break`, and narrow loop-body `continue` now have direct/real-clang smoke coverage. The bounded readonly `*(p+i)` read, lazy `?:` branch semantics, short-circuit lazy evaluation, for-loop scope boundary, `ForStmt` continue-step ordering, step-position inc/dec boundaries, and `DoWhile` continue-condition check should stay covered by real-clang and fail-closed boundary tests, while arbitrary pointer arithmetic, arbitrary pointer comparison, floating-point comparison, unmodeled mixed-width conversions, side-effect operands, and semantic acceptance must continue to fail closed.
