...
1. Keep extending generic typed IR coverage with red tests first. Good next cuts are simple scalar compound assignment lowering, assignment RHS implicit integral cast preservation, or explicit usual-conversion classification as separate slices. Condition-position `&&` / `||` now has direct typed IR, clang skeleton, and real clang AST smoke coverage. The bounded readonly `*(p+i)` read should stay covered by real-clang and fail-closed boundary tests, while arbitrary pointer arithmetic, arbitrary pointer comparison, floating-point comparison, unmodeled mixed-width conversions, side-effect operands, and semantic acceptance must continue to fail closed.
