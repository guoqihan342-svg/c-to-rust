...
- [x] **A19b4c2e: bind ContextPages one SCC at a time during planning.** Page/catalog payloads are released after a host-owned group closure is issued. Catalogs remain in same-volume staging until every group validates, so failed retries cannot observe partial publication. JSON writes, content hashes, and ledger portfolio bindings share streaming canonical metadata. Whole-project selection and full CIndex/ContextPages/portfolio artifacts remain monolithic, so the parent stays unchecked.
