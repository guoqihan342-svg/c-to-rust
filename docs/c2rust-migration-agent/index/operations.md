英文镜像见 `operations.en.md`。

﻿# 运行与验证

- [build-and-c2rust-baseline.md](../build-and-c2rust-baseline.md)
- [bounded-auto-translation-pipeline.md](../bounded-auto-translation-pipeline.md)
- [bounded-auto-translation-pipeline.en.md](../bounded-auto-translation-pipeline.en.md)
- [evidence-governance.md](../evidence-governance.md)
- [evidence-governance.en.md](../evidence-governance.en.md)
- [testing-unsafe-cache-and-milestone.md](../testing-unsafe-cache-and-milestone.md)
- [full-regression-runner.md](../full-regression-runner.md)
- [competition-env](../../../config/competition-env/)
