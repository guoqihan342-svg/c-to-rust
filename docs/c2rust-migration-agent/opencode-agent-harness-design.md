...
`run-plan --auto-retry` 把 worker 失败路径接到已落盘的 `repair_hints` 账本。失败 worker 会用同一份 assignment 重试，直到重新验证通过或达到 `REPAIR_ROUND_CAP=5`；中间失败尝试保留审计记录，语义接受仍只来自 worker summary、最终聚合和 validator。
