...
| `struct T *` (mutable record pointer) | 窄支持 | 仅在 direct single-pointer scalar field 写/update/read-after-write、direct if-return fallthrough write、statement/simple `ForStmt` step inc-dec，以及 direct integer field 的窄 value-position inc-dec prelude 时映射为 `&mut T`；statement-position inc-dec 只按返回值丢弃的字段赋值处理；不是通用 ownership 或 alias 模型 |
...
| `++` / `--` (value-position) | 窄支持 | 简单标量 direct-call/nested direct-call 参数已有窄 prelude；direct single-pointer mutable record pointer scalar field 现在支持 declaration initializer、assignment/return value 和 direct-call argument prelude；通用 value-position、condition、复杂 target、同 base sibling read、by-value field value-position 和语义验收仍 fail-closed |
