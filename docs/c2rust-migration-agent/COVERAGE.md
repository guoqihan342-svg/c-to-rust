...
| `union` | 不支持 | 未建模 |
| `struct` (按值传递) | 窄支持 | dot-field read、简单 dot-field assignment、按值 dot-field compound assignment、statement 位置 dot-field inc/dec、simple `ForStmt` step-position direct dot-field inc/dec、本地 by-value copy、唯一具名 tag 的完整直接标量字段清单下的 whole-record return；record-field 子集已有 no-clang AST fixture replay 覆盖按值 dot 读/写、readonly arrow 读、single-pointer mutable arrow 写，以及 `fdb_blob_t` 这类 typedef spelling 通过 clang `desugaredQualType`/`canonicalQualType` fallback 到 `struct fdb_blob *` 的 reduced 形状和 opt-in real-clang smoke；`real-fdb-blob-make` 已有 committed typed-IR candidate provenance、exact generated-draft acceptance 和 L4 accepted-evidence semantic pass，`generated_draft_semantic_pass=true` 只对应绑定 SHA 的 exact draft 与 3-case fixture，accepted binding 覆盖 C oracle、Rust report、schema diff、negative diff、unsafe scan/ledger 以及 `return_same_blob` / `blob.buf` / `blob.size` observable outputs；route 仍保持 L4 refused/accepted-evidence-authoritative 边界，这不是 layout/ABI proof，也不解决 `fdb_kv_set` callee 语义；dot-field 路径仍是 minimal field candidate；whole-record inventory 拒绝同名 tag、bitfield、volatile/packed field、self-pointer/non-scalar field；pointer member access 只限 readonly、single-pointer mutable 和 explicit noalias 下的 nested record pointer 标量字段路径 copy 窄子集，不是通用 `->`；按值 value-position field update、无 noalias 的多 pointer alias-sensitive field write、通用嵌套、匿名仍不支持 |
...
| `==` `!=` `<` `<=` `>` `>=` | 窄支持 | 条件和 value-position C int 0/1；value-position、`if`、`while` 及 `do-while` condition 允许恰好一个 direct scalar inc/dec operand 与不读取同一标量的纯 scalar sibling 通过 ordered prelude 发射；`while` 在每轮判断前、`do-while` 在每次尾部判断前重新执行 prelude，对应 `continue` 都进入下一次 condition prelude，`break` 不额外判断；`for`、call/deref/member、双 side-effect operand、复杂 target 和同变量 sibling read 仍 fail-closed |
...
| `p->field` (arrow member) | 窄支持 | readonly `const struct T *p` scalar field read、null-presence/guarded readonly read，以及 direct non-nullable single-pointer mutable `struct T *p` scalar field assignment/compound update/read-after-write/direct if-return fallthrough write/statement inc-dec/simple `ForStmt` step inc-dec/direct integer field value-position inc-dec 已支持；statement-position `p->field++`/`++p->field` 只在返回值被丢弃时发射为字段赋值；multi-pointer alias、nullable mutable pointer、read-before-write、普通 maybe-write 后读取、只在 returning 分支写入、loop/复杂路径 return、复杂 base/target/RHS、同一表达式 sibling 读取同 base、非标量字段、layout/ABI 声明和 semantic acceptance 仍不支持 |
| `p.field` (dot member access) | 窄支持 | 仅按值 record dot-field read、简单 `p.field = value`、statement 位置 `p.field += value`（RHS 仅简单整数变量/字面量/整数 cast）、standalone statement 位置和 simple `ForStmt` step 位置 `p.field++` / `++p.field` / `p.field--` / `--p.field`（base 必须是直接按值 record 变量，field 必须是受支持整数）、本地 copy 后字段访问；value-position `p.field++`、nested/复杂 RHS/复杂 base、pointer-arrow step inc/dec 和 pointer/alias-sensitive field write 仍不支持 |
