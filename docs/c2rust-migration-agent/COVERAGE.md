...
| 函数调用 (direct call) | 窄支持 | 仅直接标识符 callee；用户函数 `helper`/`observe` 这类 bounded direct call 已有 no-clang AST fixture replay，参数位置的 clang-proven integer `ImplicitCastExpr` 会保留为显式 cast；`assert(int)`、`abs(int)` 和 target-ABI-bound `strlen(const char *)` 有最小模型，其它 reserved C macro/stdlib/extern surface 仍需显式模型或 extern binding，否则 fail-closed |
...
| `switch` | 不支持 | 已有 CFG/relooper/route 拒绝证据和最小 structured-recovery precondition/refusal evidence；仍需完整 CFG + relooper + Rust candidate lowering 才能支持 |
| `goto` | 不支持 | 已有 CFG/relooper/route 拒绝证据和最小 structured-recovery precondition/refusal evidence；仍需完整 CFG + relooper + Rust candidate lowering 才能支持 |
| label | 不支持 | 仅作为 `goto` refusal 的 CFG/relooper evidence 记录 |
| `case` / `default` | 不支持 | 仅作为 `switch` refusal 的 CFG/relooper evidence 记录 |
...
| 函数/头文件 | 状态 | 说明 |
|-------------|------|------|
| 其它标准库函数 | 不支持 | 无 stub / extern callee 证明；`assert(int)`、`abs(int)` 和 `strlen(const char *)` 是上方单独列出的最小模型例外 |
