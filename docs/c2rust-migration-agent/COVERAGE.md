...
| 函数/头文件 | 状态 | 说明 |
|-------------|------|------|
| 其它标准库函数 | 不支持 | 无 stub / extern callee 证明；`assert(int)`、`abs(int)`、`strlen(const char *)` 和 `memcmp(const void *, const void *, size_t)` 是上方单独列出的最小模型例外 |
