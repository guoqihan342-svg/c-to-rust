...
| 函数调用 (direct call) | 窄支持 | 仅直接标识符 callee；完整 record pointer 若只作为 direct-call passthrough，可发射为安全 `&mut Record`/`&Record`，已初始化且取地址的 local record 会发射为 `mut` 并作为 `&mut local` 参数。与 nested-field 写目标的 mutable owner 并存时必须有 `NoAliasParamPair`/restrict；缺失合同、同一 record sibling read、第二 mutable owner、nullable/复杂 pointer expression、variadic/indirect/signature-conflict 和额外 call 均 fail-closed |
