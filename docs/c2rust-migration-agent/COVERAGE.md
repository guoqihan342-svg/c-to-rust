...
| `union` | 不支持 | 未建模 |
| `struct` (按值传递) | 窄支持 | dot-field read/assignment/compound assignment/inc-dec、本地 by-value copy、完整 direct scalar field inventory 下的 whole-record return，以及 fixed integer array field inventory/read；complete local record 的纯 nested dot path 可作为 `do-while` 尾 assignment-call target，direct integer scalar member 可作为 direct-call argument。record-field subset 还覆盖 readonly/single-pointer mutable/noalias-proven pointer scalar member read/write/call argument 和 record member compound RHS；nullable/volatile/atomic/incomplete/non-scalar、未证明 mutable alias、复杂/nested pointer base 与通用 layout/ABI 仍 fail-closed。`real-fdb-blob-make` 的 accepted semantic pass 仍只对应 SHA-bound exact draft 与 3-case fixture，不外推为通用 record 语义 |
...
| 函数调用 (direct call) | 窄支持 | 仅直接标识符 callee；除既有整数 cast、modeled libc 和函数指针窄子集外，direct integer scalar record member 现在可作为参数：by-value/local complete record、readonly record pointer，或具有 readonly/noalias 证明的 mutable record pointer。nullable、volatile/atomic、incomplete/non-integer field、variadic/indirect/signature-conflict、复杂 member base、额外 call 和未证明 alias 均 fail-closed；`_Bool` direct-call 结果可用于直接或取反条件，普通整数返回 call truthiness 仍 fail-closed |
...
| `p->field` (arrow member) | 窄支持 | readonly 与受证明的 mutable scalar field read/write/inc-dec/compound/call argument 已有窄路径；strict direct non-nullable mutable record-pointer integer field 还可作为 `if ((p->field = direct_call(...)) compare sentinel)` target。nullable、readonly target、volatile/atomic、复杂/effectful base、nested member hop、第二 call、未证明 alias、layout/ABI 和 semantic acceptance 仍不支持 |
...
| `if` / `if-else` | 已支持 | 含 comparison condition 和单个 direct scalar inc/dec ordered prelude。窄形状 `if ((target = exactly one direct call) compare pure_sentinel)` 支持 direct fixed-width integer scalar，或 strict direct non-nullable mutable record-pointer integer field target，并提升为前置 `Assign` + 纯目标读取；六种整数比较与 else-if 归属均有测试。local/by-value member、nested hop、readonly/volatile/atomic/effectful target、逻辑组合、第二 call 及 `while` 同形继续 fail-closed；该项仍只有 candidate evidence |
