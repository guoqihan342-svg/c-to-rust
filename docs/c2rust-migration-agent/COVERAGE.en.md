...
| `if` / `if-else` | Supported | Including comparison condition; clang-proven integral `ImplicitCastExpr` nodes in conditions are preserved only as explicit IR casts; one direct scalar inc/dec comparison operand may emit through an ordered prelude after the narrow conflict checks above |
