...
|-----------------|--------|-------|
| Other standard library functions | Unsupported | No stub / extern callee proof; `assert(int)`, `abs(int)`, `strlen(const char *)`, and `memcmp(const void *, const void *, size_t)` are the separately listed minimal-model exceptions above |
