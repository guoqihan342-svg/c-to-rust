...
| `==` `!=` `<` `<=` `>` `>=` | Narrow | Condition and value-position C int 0/1; value position, `if` conditions, and `while` conditions allow exactly one direct scalar inc/dec operand with an ordered prelude when the other operand is a pure scalar sibling that does not read the modified scalar. `while` reruns the prelude before every condition check and `continue` returns to the next prelude. `do-while`/`for`, call/deref/member operands, two side-effect operands, complex targets, and same-variable sibling reads remain fail-closed |
