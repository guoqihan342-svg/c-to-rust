...
| `do-while` | Supported | clang-proven integral `ImplicitCastExpr` nodes in conditions are preserved only as explicit IR casts; a bounded direct scalar inc/dec comparison condition executes its ordered prelude on both the normal tail and `continue` paths, while `break` skips the condition; a strict `(scalar = direct_call(...)) != sentinel` tail condition may normalize to a body-tail assignment plus pure comparison, while current-level `continue`, complex/volatile/atomic targets, a second side effect, and optional `&&` expansion remain fail-closed; both paths have no-clang AST/runtime positives and adjacent refusal replay |
