...
| `do-while` | Supported | clang-proven integral `ImplicitCastExpr` nodes in conditions are preserved only as explicit IR casts; a bounded direct scalar inc/dec comparison condition executes its ordered prelude on both the normal tail and `continue` paths, while `break` skips the condition |
