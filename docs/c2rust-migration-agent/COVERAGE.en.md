...
|-----------------|--------|-------|
| Other standard library functions | Unsupported | No stub / extern callee proof; `assert(int)`, `abs(int)`, `strlen(const char *)`, `memcmp(const void *, const void *, size_t)`, statement-only `memset(dst, byte_literal, size)`, and statement-only `memcpy(out, src, size)` are the separately listed minimal-model exceptions above |
