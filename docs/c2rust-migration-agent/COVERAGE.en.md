...
| `==` `!=` `<` `<=` `>` `>=` | Narrow | Condition and value-position C int 0/1; value position plus `if`, `while`, and `do-while` conditions allow exactly one direct scalar inc/dec operand with an ordered prelude when the other operand is a pure scalar sibling that does not read the modified scalar. `while` reruns the prelude before every condition check and `do-while` before every tail check; their `continue` paths reach the next condition prelude, while `break` performs no extra check. `for`, call/deref/member operands, two side-effect operands, complex targets, and same-variable sibling reads remain fail-closed |
...
| `if` / `if-else` | Supported | Including comparison condition; clang-proven integral `ImplicitCastExpr` nodes in conditions are preserved only as explicit IR casts; one direct scalar inc/dec comparison operand may emit through an ordered prelude after the narrow conflict checks above. The narrow `if ((direct fixed-width integer scalar = exactly one direct call) compare pure_sentinel)` shape lifts to a preceding `Assign` plus a pure target-read `If` comparison, supports all six integer comparisons, and preserves single-statement else-if ownership. `fdb_kvdb.c:1018/:1076` are real positives; the nested second call at `:738` and pointer/member target at `:1870` are real negatives. Right-side assignment, logical composition, complex/volatile/atomic/dereference targets, a second side effect, and the corresponding `while` shape remain fail-closed. This is candidate evidence only and does not increase the semantic-pass count |
...
| `do-while` | Supported | clang-proven integral `ImplicitCastExpr` nodes in conditions are preserved only as explicit IR casts; a bounded direct scalar inc/dec comparison condition executes its ordered prelude on both the normal tail and `continue` paths, while `break` skips the condition; a strict `(scalar = direct_call(...)) != sentinel` tail condition may normalize to a body-tail assignment plus pure comparison. The same tail assignment is cloned before current-level `continue`, after which the existing do-while emitter performs the pure sentinel check; then/else branches are rewritten recursively, but nested `while`/`do-while`/`for` bodies are not entered. Complex/volatile/atomic targets, a second side effect, and optional `&&` expansion remain fail-closed; no-clang AST/runtime, assignment-order, and nested-boundary regressions cover the admitted shape |
| `for` (scoped) | Narrow | Simple init/condition/step forms; the top-level init may flatten a direct integer scalar assignment comma chain such as `i = start, j = i` in left-to-right order; memory targets, call/deref RHS expressions, inc/dec leaves, volatile reads or writes, and condition/step commas fail closed; clang-proven integral `ImplicitCastExpr` nodes in conditions are preserved only as explicit IR casts |
...
Real named-slice acceptance supplement (2026-07-10): the exact generated draft for `real-fdb-tsl-to-blob` passes a three-case C oracle, generated Rust replay, schema diff, negative diff, unsafe scan/ledger, route/profile, and final verification, including the clang-proven `uint32_t -> size_t` widening and mutable-output identity return. `real-fdb-is-str` also passes its eight-case C oracle/Rust replay/diff/negative/unsafe/route/final-verification chain. Each pass is limited to its bound fixture, SHA, and observable contract; neither proves general record layout/ABI/alias/provenance, locale behavior, or arbitrary strings. `translator_generated_semantic_pass_count` is now 23.
