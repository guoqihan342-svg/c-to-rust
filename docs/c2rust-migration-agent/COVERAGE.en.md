...
|-----------------|--------|-------|
| Other standard library functions | Unsupported | No stub / extern callee proof; `assert(int)`, `abs(int)`, `strlen(const char *)`, `memcmp(const void *, const void *, size_t)`, and statement-only `memset(dst, 0, size)` are the separately listed minimal-model exceptions above |
