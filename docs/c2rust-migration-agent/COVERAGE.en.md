...
| `struct` (by-value) | Narrow | Supports dot-field read/assignment/compound assignment/inc-dec, local by-value copy, whole-record return under a complete direct-scalar inventory, and fixed-integer-array field inventory/read. A pure nested dot path on a complete local record may be a `do-while` tail assignment-call target, and a direct integer scalar member may be a direct-call argument. The record-field subset also covers readonly/single-pointer-mutable/noalias-proven pointer scalar member read/write/call arguments and record-member compound RHS. Nullable/volatile/atomic/incomplete/non-scalar fields, unproven mutable alias, complex/nested pointer bases, and general layout/ABI remain fail-closed. The accepted `real-fdb-blob-make` semantic pass still applies only to its SHA-bound exact draft and three-case fixture |
...
| Function call (direct call) | Narrow | Direct identifier callees only. In addition to existing integer casts, modeled libc, and narrow function-pointer cases, direct integer scalar record members may now be arguments from by-value/local complete records, readonly record pointers, or mutable record pointers with readonly/noalias proof. Nullable, volatile/atomic, incomplete/non-integer fields, variadic/indirect/signature-conflict calls, complex member bases, extra calls, and unproven aliasing fail closed. `_Bool` direct-call results may be direct or negated conditions, while ordinary integer-return call truthiness remains fail-closed |
...
| `p->field` (arrow member) | Narrow | Narrow paths cover proven readonly/mutable scalar field read/write/inc-dec/compound/call arguments. A strict direct non-nullable mutable record-pointer integer field may also be the target in `if ((p->field = direct_call(...)) compare sentinel)`. Nullable or readonly targets, volatile/atomic fields, complex/effectful bases, nested member hops, a second call, unproven aliasing, layout/ABI, and semantic acceptance remain unsupported |
...
| `if` / `if-else` | Supported | Supports comparison conditions and one direct scalar inc/dec ordered prelude. The narrow `if ((target = exactly one direct call) compare pure_sentinel)` shape admits a direct fixed-width integer scalar or strict direct non-nullable mutable record-pointer integer field target, then lifts to a preceding `Assign` plus a pure target read. All six comparisons and else-if ownership are tested. Local/by-value members, nested hops, readonly/volatile/atomic/effectful targets, logical composition, a second call, and the corresponding `while` shape remain fail-closed; this is still candidate evidence only |
