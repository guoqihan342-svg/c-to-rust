...
| Function call (direct call) | Narrow | Direct identifier callees only. In addition to the existing integer/record-scalar/member and modeled-callee subsets, exactly one direct complete-record pointer variable proven by the function body to be the mutable-record owner may be an argument. The call may not contain a second mutable record borrow or a sibling read of the same record. Nullable or complex pointer expressions, multiple pointer borrows without noalias proof, variadic/indirect/signature-conflict calls, and extra calls remain fail-closed |
