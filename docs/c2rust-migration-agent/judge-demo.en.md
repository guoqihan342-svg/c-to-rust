...
Boundary: the before/after exhibit baseline is still a reviewed unsafe Rust baseline derived from the real C slice signature. The C2Rust baseline now has generated + compile-only + direct replay observable-passed evidence, but it remains `candidate_context_only` / `semantic_pass=false`; do not claim that the before/after baseline came from a verified C2Rust output. This exhibit does not create a new semantic gate; separately, the exact `real-fdb-calc-crc32` typed-IR generated draft now has `generated_draft_acceptance.status=passed`, so route/milestone metrics currently report translator-generated numerator 1.
...
- The before/after exhibit fixes `generated_draft_semantic_pass=false`; its safety-transformed candidate cannot impersonate a translator-generated semantic pass.
