...
- By-value record field reads/simple assignments: real clang AST input such as `struct point p; return p.x;` and `p.x = value; return p.x;` now lowers to typed IR `IrExpr::Member`, and the generic emitter can produce a minimal Rust `Point` struct plus `return p.x;` and `p.x = value;`. This is candidate generation only; it does not claim C record layout/ABI equivalence, record parameters with no modeled field use, `p->x`, compound/update field writes, pointer/alias-sensitive field writes, unions/bitfields/volatile fields, record locals/returns, or semantic acceptance.
