...
- By-value record field reads/simple assignments/local copy/whole-record return: real clang AST input such as `struct point p; return p.x;`, `p.x = value; return p.x;`, `struct point q = p; q = r; return q.x;`, and `struct point identity(struct point p) { return p; }` now lowers into typed IR record candidates. The generic emitter can produce minimal or unique-named complete-direct-scalar-field-inventory-driven Rust `Point` structs, field reads/writes, local copies, and `return p;`. This is candidate generation only; it does not claim C record layout/ABI equivalence; `p->x`, compound/update field writes, pointer/alias-sensitive field writes, duplicate tags, self-pointer fields, unions/bitfields/volatile/packed/non-scalar fields, and semantic acceptance still fail closed.
