...
- By-value record field reads: real clang AST input such as `struct point p; return p.x;` now lowers to typed IR `IrExpr::Member`, and the generic emitter can produce a minimal Rust `Point` struct plus `return p.x;`. This is candidate generation only; it does not claim C record layout/ABI equivalence, record parameters with no modeled field use, `p->x`, field assignment, unions/bitfields/volatile fields, record locals/returns, or semantic acceptance.
