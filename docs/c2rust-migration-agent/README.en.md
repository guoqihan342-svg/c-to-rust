...
- By-value record field reads/simple assignments/local copy: real clang AST input such as `struct point p; return p.x;`, `p.x = value; return p.x;`, and `struct point q = p; q = r; return q.x;` now lowers to typed IR `IrExpr::Member` plus local record copy, and the generic emitter can produce a minimal Rust `Point` struct, field reads/writes, and `let mut q: Point = p; q = r;`. This is candidate generation only; it does not claim C record layout/ABI equivalence, record parameters with no modeled field use, `p->x`, whole-record return, compound/update field writes, pointer/alias-sensitive field writes, unions/bitfields/volatile fields, or semantic acceptance.
