...
- Core translation boundary: the signed unary minus, comparison expression, and logical-not support above are candidate generation only; comparison currently covers conditions plus narrow value-position C `int` 0/1 materialization such as `return x > 0`, assignment RHS, and declaration initializer, while logical not covers integer zero checks and narrow value-position C `int` 0/1 materialization. Pointer comparison, floating-point comparison, mixed-width/unsigned conversions, comparison cast operands, call/inc/dec/deref/side-effect operands, pointer null tests, floating-point truthiness, unsupported types, short-circuit logic, full usual scalar conversions, unsigned/wrapping negation, floating-point negation, special literal/min-value edges, and semantic acceptance still fail closed.
