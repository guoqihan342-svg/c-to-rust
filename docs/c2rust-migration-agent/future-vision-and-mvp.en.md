...
| Active translator track | P0-A18c / P0-A10 | A18c8h4b compiles the raw AI/typed-IR candidate first and injects compile-only bindings only for declared external functions that rustc reports as exactly unresolved; self-contained candidates are no longer blocked by unused dependencies from the original C call graph. Next repair the API shape and add executable callee semantics/no-escape |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h4b | The harness selects candidate-required declared external callees from structured rustc `E0425` unresolved-function diagnostics. Strings, code drift, missing values, and undeclared names cannot trigger injection. Self-contained candidates may enter replay, while compile-only callees remain non-executable and `semantic_pass=false` |
