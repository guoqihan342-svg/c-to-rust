...
- [ ] Keep expanding the generic typed IR emitter instead of restoring crc32/FlashDB special cases: initialized record local copy, by-value record dot-field compound assignment, statement-position by-value record dot-field inc/dec, whole-record return with unique named complete direct scalar field inventory, simple readonly `const struct T *p` `p->scalar_field` reads, readonly record pointer `p == NULL` / `p != NULL` presence checks, flow-sensitive null-guarded `p->scalar_field` reads, direct mutable `struct T *p` `p->scalar_field = scalar`, simple standalone `p->scalar_field += scalar`, and same-field `p->scalar_field` reads after definite writes under the single-pointer-param gate are now in the candidate subset; next prioritize multi-pointer alias/noalias proof, value-position/complex-target field updates/inc-dec, path-sensitive mutable field definite assignment, and stronger layout/ABI evidence.
