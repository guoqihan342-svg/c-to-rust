...
| Active translator track | P0-A18c / P0-A10 | A18c8h5a closes real exact replay compiler/linker facts, and a real WSL candidate now safely feeds an identity-neutral undefined symbol into repair. Next implement h5b oracle-free case/field feedback, then converge h5c ABI gating |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h5a | Exact replay result projection is split into a small module, compile phase forwards binding-protected JSONL, and strict linker-child parsing extracts only legal C identifiers without paths, rendered source, or arbitrary child text. The 62-test stage and a real WSL replay pass; `semantic_pass=false` and the success count remains 38 |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | h5a is complete. Implement h5b case/field-only semantic feedback next, then fix h5c safe-candidate ABI false rejection; both require renamed positives, drift negatives, and rerunning the same exact gates |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8h: generic mutable-scalar address to `void *` call argument**. Model a temporary mutable borrow only for a provably complete integer-scalar direct/record-field lvalue address, a direct external call, and an explicit mutable `void *` parameter. Alias, lifetime, alignment, width, signature, and post-call read paths must be verifiable. Const, nullable, bitfield, packed/volatile/atomic, pointer arithmetic, cross-call escape, and overlapping mutable borrows remain refused; no `in_addr_t`, libuv, or fixed-callee identity mapping is allowed.
        ...
        - [ ] **P0-A18c8h5b: oracle-free replay case/field failure localization**. On runtime assertion failure, the generated replay runner may expose only `{case_id, observable_field}` recomputed from the bound ReplayCallPlan. Expected/actual values, fixture content, replay source, panic text, and line numbers remain forbidden. The mapping binds plan, replay, candidate, and assertion-inventory SHAs; duplicate, unknown, or spoofed assertions fail closed. Completion requires fully renamed multi-case/multi-field positives that let AI target the field while all oracle-value leakage tests remain empty.
