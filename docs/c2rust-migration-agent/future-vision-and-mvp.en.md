...
| Active translator track | P0-A18c / P0-A10 | A18c8a/A18c8b/A18c8b1/A18c8c/A18c8d are closed; next add hash-bound record-layout evidence, then close the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8d | When clang omits `sizeof(expression).argType`, the frontend recovers the type from the unique typed operand; the real libuv blocker advanced from missing operand metadata to explicit missing record-layout provenance |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8a/A18c8b/A18c8b1/A18c8c are complete; next close function-table indirect-call and record-layout `sizeof` typed-IR/AI-context gaps while exact validation continues handling the remaining semantic failures |
...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8e: hash-bound record-layout provenance**. Produce recomputable record size/alignment evidence verified by the compiler and target ABI, and allow `sizeof` to consume only an exactly matching record type. Missing, duplicate, target-conflicting, or hash-drifted evidence must refuse; record size must never be hardcoded from `sockaddr_in` or libuv identity.
