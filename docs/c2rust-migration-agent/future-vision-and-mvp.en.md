...
| Latest development stage | P0-A19 project-interface coordination and AI-repair authority | Arbitrary-repository inventory, SCC/DAGs, build closure, paged ContextPacks, isolated OpenCode, SQLite state, RustProjectIR/Cargo generations, immutable coordinator-receipt epochs, and a project-repair state machine with minimal-context candidate re-coordination are implemented. The global planner, sole CompletionCoordinator queue draining, positive semantic verifiers, competition-equivalent aggregate isolation, and real held-out semantic acceptance remain open. This batch runs 341 tests on Windows with 339 passing and two conditional skips, plus 341/341 on WSL; the success count stays 38 |
...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **Current boundary (2026-07-13):** A19 has completed one real WSL DeepSeek auxiliary worker plus bounded Ninja/CMake link and static-archive closure, actual rustup-toolchain content binding, non-degrading SandboxBackend requirements/VerificationPlans, a bubblewrap host capability probe, candidate/project Cargo evidence reopening, immutable coordinator-receipt epochs, a project-repair CAS/event/artifact ledger, minimal relevant IR contexts, an isolated provider-contract simulation, host IR rebuilding, and re-coordination. The global planner, sole CompletionCoordinator queue draining, A19e7 verifier capabilities/nonces, positive oracle/negative/unsafe-alias/ABI/final execution, project oracle, aggregate cgroup/equivalent backends, a real sandboxed Cargo build, and real held-out semantic acceptance remain incomplete. The parent stays unchecked and the translator numerator does not increase. This batch runs 341 tests on Windows with 339 passing and two conditional skips, plus 341/341 on WSL. The provider-contract simulation is not a real model run; a real WSL smoke without `bwrap` still fails closed as `bubblewrap_unavailable` with zero candidate execution, and none of this is `competition-exact`.
  ...
  **A19c OpenCode multi-worker candidate portfolio**
  ...
  - [x] Close plan/preflight/request/attempt drift, lease re-entry, heartbeat, non-atomic ingest, tool events, secret input, parent-environment leakage, agent permission override, pre-spawn command-start accounting, and missing provider-evidence persistence. The current finite regression runs 341 tests on Windows with 339 passing and two conditional skips, plus 341/341 on WSL.
  ...
  - [ ] **A19d3: canonical RustProjectIR and cross-unit coordination.**
    ...
    - [ ] A19d3c: route type, link, initialization, and feature diagnostics that cannot be attributed to one unit into a separate project-level repair queue that proposes bounded repairs from the complete RustProjectIR. Never assign them randomly, duplicate shared types, or bypass conflicts with fixture-specific shims.
      ...
      - [ ] A19d3c2: persist queue/events/attempts in the TransitionAuthority ledger, let the project repairer consume only the smallest relevant IR/diagnostic context and emit a new IR candidate, and make recovery, budget exhaustion, rollback, and re-coordination replayable events.
        ...
        - [ ] A19d3c2b: make the sole CompletionCoordinator automatically schedule or resume the latest unfinished project queue, route real verifier diagnostics that cannot be localized to a unit into it, and permit promotion only after whole-project final revalidation. Neither low-level APIs nor model output may complete the project directly.
