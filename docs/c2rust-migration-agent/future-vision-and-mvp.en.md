...
| Active translator track | P0-A18c / P0-A10 | A18c8h3 lets the real libuv candidate pass rustc under compile-only contracts for both external direct callees; next add executable callee semantics/no-escape, then address direct locals and the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h3 | `__bswap_16` and `uv_inet_pton` are declared by the hash-bound slice spec. The generic compile-only pointer stub erases only pointees, preserves const/mut direction, and reports the non-semantic adapter explicitly. The unsafe-free WSL clang 18.1.3 candidate passes rustc; replay still skips by contract and `semantic_pass=false` |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8h3 makes the real candidate pass rustc under external-callee compile context; A18h still needs executable callee semantics/no-escape, direct-local scalar, raw extern-boundary, and post-write initialization proofs before the function-table indirect-call gap |
