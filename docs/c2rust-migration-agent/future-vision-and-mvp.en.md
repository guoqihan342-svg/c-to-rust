...
| Active translator track | P0-A18c / P0-A10 | A18c8h4g completes a real libuv DeepSeek AI-first auxiliary run and proves repair can advance replay from compile failure to runtime failure. Next close real diagnostic propagation, oracle-free case/field feedback, and ABI gating in A18c8h5a/h5b/h5c order |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h4g | The WSL competition-like lane runs DeepSeek/max with a fresh C oracle and three repairs; an independent validator reopens 29 artifacts with no structural errors. Rounds two and three pass rustc, oracle, unsafe, and alias but fail replay/ABI/final gates, so `semantic_pass=false` and the success count remains 38 |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | Close h5a real replay compiler/linker fact propagation first, h5b case/field-only semantic feedback second, and h5c safe-candidate ABI false rejection third; each requires renamed positives, drift negatives, and rerunning the same exact gates |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8h: generic mutable-scalar address to `void *` call argument**. Model a temporary mutable borrow only for a provably complete integer-scalar direct/record-field lvalue address, a direct external call, and an explicit mutable `void *` parameter. Alias, lifetime, alignment, width, signature, and post-call read paths must be verifiable. Const, nullable, bitfield, packed/volatile/atomic, pointer arithmetic, cross-call escape, and overlapping mutable borrows remain refused; no `in_addr_t`, libuv, or fixed-callee identity mapping is allowed.
        ...
        - [ ] **P0-A18c8h5a: real exact replay compiler/linker fact closure**. The replay wrapper in `validate_auto_migrate_candidate` must forward hash-bound `compile_stderr` only for phase=`compile`, and strict rustc linker-child parsing may extract only identifier-shaped undefined symbols. Rendered source, commands, paths, and arbitrary child text remain excluded. Compiler/linker facts bind candidate, replay, fixture, and target SHAs; drift in any binding yields zero forwarded facts. Completion requires a neutral extern-name regression through the real wrapper, host-path/secret/spoof redaction negatives, and no semantic acceptance for compile-only external calls.
