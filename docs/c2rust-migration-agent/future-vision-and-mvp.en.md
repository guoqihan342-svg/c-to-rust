...
| Active translator track | P0-A18c / P0-A10 | A18c8h2 lets the real libuv function generate a Rust candidate under an explicit noalias precondition; next declare its two external direct callees, then address direct locals and the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h2 | A complete `ip -> addr` slice-spec noalias precondition closes the multi-pointer gate, and A18h matches equivalent integer typedefs by const/signedness/width. The real clang 18.1.3 TU now emits an unsafe-free Rust candidate and blocks on undeclared `__bswap_16` and `uv_inet_pton` callees |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8h2 closes the real multi-pointer gate under an explicit noalias precondition and emits a candidate; A18h still needs external-callee, direct-local scalar, raw extern-boundary, and post-write initialization proofs before the function-table indirect-call gap |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8h: generic mutable-scalar address to `void *` call argument**. Model a temporary mutable borrow only for a provably complete integer-scalar direct/record-field lvalue address, a direct external call, and an explicit mutable `void *` parameter. Alias, lifetime, alignment, width, signature, and post-call read paths must be verifiable. Const, nullable, bitfield, packed/volatile/atomic, pointer arithmetic, cross-call escape, and overlapping mutable borrows remain refused; no `in_addr_t`, libuv, or fixed-callee identity mapping is allowed.
        ...
        - A WSL clang 18.1.3 rerun with the same hash-bound libuv compile database and `x86_64-unknown-linux-gnu` ABI now emits a `GenericTypedIr` Rust candidate: `ip` remains a raw read pointer, `addr` is `&mut SockaddrIn`, the interior target is `&mut addr.sin_addr.s_addr`, and the source contains no `unsafe`. This probe exists only under `target/a18c8h-probe-04` with `semantic_pass=false`; `__bswap_16` and `uv_inet_pton` are undeclared, and rustc/replay/diff/negative/final-verification gates have not passed, so the numerator is unchanged.
        - The A18h parent remains open: external-callee/no-escape proof, direct local/parameter scalar addresses, the raw extern boundary, and post-call field initialization remain separate work. Const, nullable, variadic, indirect-call, signedness/width drift, incomplete-record, and same-root sibling-read cases have fail-closed regressions.
