...
- [ ] Pointer/alias-sensitive struct field writes (with alias/noalias proof; simple readonly `const struct T *p` `p->scalar_field` reads, `p == NULL` / `p != NULL` presence checks to `Option<&T>`, flow-sensitive null-guarded `p->scalar_field` reads, direct mutable `struct T *p` `p->scalar_field = scalar` and simple standalone `p->scalar_field += scalar` under the single-pointer-param gate, non-pointer simple by-value `p.x = value`, and standalone statement-position `p.x++` / `++p.x` / `p.x--` / `--p.x` are already in the typed IR candidate subset; multi-pointer aliasing, nullable mutable pointers, complex RHS/targets, mutable `return p->field` reads, `p->field++`, and other pointer field update/read forms still fail closed)
...
- [ ] Keep expanding the generic typed IR emitter instead of restoring crc32/FlashDB special cases: initialized record local copy, by-value record dot-field compound assignment, statement-position by-value record dot-field inc/dec, whole-record return with unique named complete direct scalar field inventory, simple readonly `const struct T *p` `p->scalar_field` reads, readonly record pointer `p == NULL` / `p != NULL` presence checks, flow-sensitive null-guarded `p->scalar_field` reads, and direct mutable `struct T *p` `p->scalar_field = scalar` plus simple standalone `p->scalar_field += scalar` under the single-pointer-param gate are now in the candidate subset; next prioritize mutable field read-after-write, multi-pointer alias/noalias proof, value-position/complex-target field updates/inc-dec, and stronger layout/ABI evidence.
