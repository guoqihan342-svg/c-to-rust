...
| 6 | P0-A10 fixed-suite real acceptance | Run the complete suite once after resource recovery and publish reproducible metrics |
| 7 | P0-A6 / P0-H9 competition-host replay | A valid resource package and competition-exact host are available; simulation cannot substitute |
...
- [x] **P0-A12: file-based prompt transport**
  ...
  Completion evidence: candidate generation and bounded repair share `opencode-file-attachment-v1`; argv retains only `--file=<prompt-path>` plus a fixed short message, while the complete prompt remains a SHA-bound file. The schema-v3 manifest and schema-v2 repair report record the file option/style, fixed-message SHA, and `inline_prompt_in_argv=false`; both summary and exact-evidence validators independently reject transport drift. All 167 AI candidate, repair, router, exact-evidence, and competition-summary regression tests pass on both Windows and WSL; WSL uses the explicit `/root/.cargo/bin` competition-toolchain PATH.
...
- [x] **P0-A15: recomputable AI caller/callee context and prompt scope**
  ...
  Completion evidence: ContextPack v3's 16 KiB C boundary now includes `external_direct_callees` and `call_expression_contract` under the common sensitive-field and host-path sanitizer, including quoted secret assignments inside ordinary strings. The independent `context_scope.py` derives a stable scope only from nonempty loaded excerpts, real failure states, and named direct-callee facts; normal generation and cache hits share this rule. Provider admission returns zero-call `required_callee_context_incomplete` when a required callee boundary is compacted or truncated. The schema-v5 manifest records scope, while the fresh-run summary validator requires v5, reopens the hash-bound ContextPack, byte-recomputes the prompt, and rejects scope drift or version downgrade. v1-v4 retain schema and accepted-evidence archive readability but cannot self-select downgrade for a fresh run. This adds no project, function, or fixture special case and does not increase the semantic numerator. The same 155-test group passes on Windows and WSL, with one platform-specific skip on each.
