...
| Latest development stage | P0-A18c8h5d | A separate 121-line compiler-header classifier projects strictly bound system-header inlines as declaration-only without repository source blocks or bindings and never permits execution. A separate readiness module recomputes and rejects namespace/header/symbol/signature/hash drift. Windows passes 95/95 and the WSL functional stage passes 91/91; the full suite retains its known baseline, and the success count remains 38 |
...
| 2 | P0-A19 unfamiliar-C-project whole-project AI harness | Pending | Starting from any repository root, discover the build closure and translation units, decompose by dependency graph, generate candidates with isolated OpenCode workers, reconstruct a compilable Cargo project, and repair through generic gates. Project/function/path/fixture identity dispatch is forbidden |
...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  1. **A19a arbitrary-repository intake and build-closure discovery (pending)**: starting from a repository root, discover supported compile databases and Make/CMake/Meson build facts, expand response files, generated includes, defines, targets, and link closures, and emit a translation-unit inventory. Missing facts fail closed by category; users should not have to pre-author one spec per function, and compile arguments must never be guessed.
  2. **A19b project dependency graph and bounded decomposition (pending)**: derive a stable migration DAG from ASTs, symbol tables, call graphs, SCCs, global state, type layouts, macro dependencies, and FFI boundaries. Separate independently safe units, context-coupled units, and boundaries that must remain. Page and deduplicate ContextPacks by dependency rather than a fixed function list.
  3. **A19c OpenCode multi-worker candidate portfolio (pending)**: use the existing harness at runtime to schedule isolated planner, translator, reviewer, and repairer workers with separate state/cache/tmp/out roots and bound context budgets. AI is the primary candidate source; typed IR and C2Rust provide candidates or facts. Parallelize only independent units, merge by DAG and gates, and never replace semantic gates with model voting.
  4. **A19d Rust-project reconstruction and incremental integration (pending)**: generate a stable module tree, Cargo targets/features, public API/FFI boundaries, shared types, and initialization order, then integrate passing units into a buildable project. Symbol conflicts, cycles, layout requirements, and linker gaps return structured diagnostics rather than being hidden by handwritten project templates.
  5. **A19e project-level verification and precise repair (pending)**: normalize rustc/linker, tests, C-oracle/Rust-replay, schema-diff, negative, unsafe/alias/ABI, and integration failures into candidate-bound facts, and send only allowlisted minimal diagnostics to the owning worker. Support last-good state, failure deduplication, bounded budgets, resume, and failed-unit reprioritization without feeding expected/actual values or test answers to the model.
  6. **A19f anti-special-case and finite held-out acceptance (pending)**: add identifier/path/project full-renaming equivalence tests, unknown-project import tests, and an identity-dispatch static check. A finite set of at most 20 cases must cover at least five real projects, whole-project builds, and 12 distinct construct families; at least two projects must not have participated in the corresponding rule development. Publish initial/repair/compile/integration/semantic/refusal metrics once, with no 1,000/10,000-round runs and no success inflation from repeated near-duplicate slices.
