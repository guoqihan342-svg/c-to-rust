...
| Active translator track | P0-A18c / P0-A19 / P0-A10 | h5a-h5c close compiler, runtime, and ABI repair facts. Next finish the nonblocking h5d compiler-header source classification, then move to unfamiliar-repository whole-project discovery, decomposition, parallel candidates, integration, and repair instead of extending syntax around one known slice |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h5c | A separate 121-line candidate-source ABI classifier lets safe ABI-free candidates pass independently, requires exact execution bindings for target-only dependencies, and refuses layout/FFI dependencies without dedicated proof. Repair receives only bounded dependency kinds. Windows passes 76 tests with one skip, WSL passes 76/76, the full suite adds no failure, and the success count remains 38 |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | h5a/h5b/h5c are complete. Next, h5d changes only compiler-header declaration-only source classification and drift refusal without expanding semantic coverage. P0-A19 whole-project harness work follows |
| 3 | P0-A19 unfamiliar-C-project whole-project AI harness | Pending | Starting from any repository root, discover the build closure and translation units, decompose by dependency graph, generate candidates with isolated OpenCode workers, reconstruct a compilable Cargo project, and repair through generic gates. Project/function/path/fixture identity dispatch is forbidden |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8h: generic mutable-scalar address to `void *` call argument**. Model a temporary mutable borrow only for a provably complete integer-scalar direct/record-field lvalue address, a direct external call, and an explicit mutable `void *` parameter. Alias, lifetime, alignment, width, signature, and post-call read paths must be verifiable. Const, nullable, bitfield, packed/volatile/atomic, pointer arithmetic, cross-call escape, and overlapping mutable borrows remain refused; no `in_addr_t`, libuv, or fixed-callee identity mapping is allowed.
        ...
        - [ ] **P0-A18c8h5d: compiler-header declaration-only source classification**. Classify a system-header inline as declaration-only only when `definition_status`, strict `compiler-header:<relative-header>#<symbol>`, header inventory, signature_ref, and `stub_boundary=compile_only` all agree. It creates no repository source block and no misleading repository-path error. Malformed namespaces, symbol/header/signature drift, or compile-only execution remain fail-closed. This does not currently block provider admission and follows h5a-h5c.
