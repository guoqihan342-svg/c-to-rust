...
| Active translator track | P0-A18c / P0-A10 | A18c8a/A18c8b/A18c8b1/A18c8c/A18c8d/A18c8e are closed; next model generic mutable record-pointer conversion for `memset`, then close the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8e | The same hash-bound compile-DB command now derives clang record layouts, and `sizeof(record)` consumes only layouts with exact type, argument SHA, compile-DB SHA, and target-ABI bindings; the real libuv blocker advanced to the mutable record-pointer BitCast used by `memset` |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8a through A18c8e are complete; next close generic `memset` record-pointer conversion and function-table indirect-call gaps while exact validation continues handling the remaining semantic failures |
...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8f: generic mutable record-pointer byte writes**. Model C `memset` `void*`/byte BitCast only for a proven mutable, non-null, complete single-level record pointer. The write length must be bounded by `sizeof(record)` from the same layout binding; readonly, nullable, out-of-bounds, type/layout drift, and arbitrary non-standard memory calls remain refused. No libuv, `sockaddr_in`, or fixed-function identity may select this path.
