...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **A19a arbitrary-repository intake and build-closure discovery**
  ...
  - [ ] Add Meson introspection/target metadata and a constrained configure/generate stage. Missing facts remain fail-closed and compile/link arguments are never guessed.
  ...
  **A19d Rust-project reconstruction and incremental integration**
  ...
  - [ ] Complete cross-unit public APIs, shared types, initialization order, FFI/link boundaries, and target/features reconstruction. Do not claim whole-project conversion before a real multi-file project build passes.
  ...
  **A19e project-level verification and precise repair**
  ...
  - [ ] Connect the positive candidate runner and CLI completion path, then execute sandboxed compile/link/test, C-oracle/Rust-replay, negative, unsafe/alias/ABI, and final verifiers. `record-candidate-gate` cannot obtain a pass from caller-provided records alone. A project completes only when the latest host-owned gates for the same candidate set all pass.
  ...
  **Execution order:** the real auxiliary model, Ninja/static-archive closure, and provider-evidence chain are closed. Next add Meson/configure facts, complete the positive candidate verifier/repair/quarantine path, provide bubblewrap or an equivalent competition sandbox, and run the fixed finite A19f real held-out build/oracle matrix. Development does not launch a model after every small edit and never runs 1,000/10,000 rounds.
