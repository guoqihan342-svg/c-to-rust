...
| Active translator track | P0-A18c / P0-A10 | A18c8h1 covers a safe-adapter candidate for record-field scalar addresses; next close the real function's multi-pointer alias proof, then direct locals and the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h1 | Dedicated AST/IR provenance accepts only a direct FunctionDecl, a mutable `void *` parameter, and a complete integer record-field path, then emits a safe `&mut` adapter call; real libuv now reaches function IR and blocks on record-write alias proof with multiple pointer parameters |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8h1 completes the record-field safe-adapter candidate; A18c8h still needs multi-pointer alias, direct-local scalar, raw extern boundary, and post-write initialization proofs before the function-table indirect-call gap |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8h: generic mutable-scalar address to `void *` call argument**. Model a temporary mutable borrow only for a provably complete integer-scalar direct/record-field lvalue address, a direct external call, and an explicit mutable `void *` parameter. Alias, lifetime, alignment, width, signature, and post-call read paths must be verifiable. Const, nullable, bitfield, packed/volatile/atomic, pointer arithmetic, cross-call escape, and overlapping mutable borrows remain refused; no `in_addr_t`, libuv, or fixed-callee identity mapping is allowed.
        ...
        - A WSL clang 18.1.3 rerun with the same hash-bound libuv compile database and `x86_64-unknown-linux-gnu` ABI removes the old `in_addr_t * is not a mutable record pointer` blocker and produces complete function IR. The new blocker is `mutable record pointer field assignment requires exactly one pointer param for alias proof`, caused by missing alias proof when the real function has multiple pointer parameters. No libuv Rust draft, C/Rust replay/diff, or semantic acceptance exists, so the success numerator is unchanged.
        - The A18h parent remains open: direct local/parameter scalar addresses, raw extern/no-escape boundaries, post-call field initialization, and exact alias proof for the real multi-pointer function remain separate work. Const, nullable, variadic, indirect-call, type-drift, incomplete-record, and same-root sibling-read cases have fail-closed regressions.
