...
| Active translator track | P0-A18 / P0-A10 | Put the exact Rust replay contract before the model and finish finite cross-project acceptance instead of extending one FlashDB line sequence |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A17 | Auxiliary cross-project runner, per-unit OpenCode isolation, strict source/span/spec binding, and high-salience boundary prompts are closed |
...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  - [x] **P0-A18a: executable replay source contract**. `auto_migrate` generates the replay draft before provider launch. ContextPack v4 binds the complete replay source, SHA, size, and real call count; candidate and repair prompts present the same contract once before the projected ContextPack. Provider readiness, manifest v8, cache, scope, and the fresh summary validator fail closed. Missing, sensitive, oversized, call-free, or drifted replay evidence results in zero provider calls or an invalid fresh run. `_auto_migrate_ai_exact.py` was also split into a 489-line compatibility facade plus routing, persistence, and validation modules.
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [x] **P0-A18c5: separate safe `NonNull` types from raw-pointer operations**. The exact alias gate masks a `core/std::ptr::NonNull<T>` path only when the candidate contains no `unsafe` and the path appears in a generic type position. `NonNull::<T>` construction, other `ptr` APIs, `as_ptr/as_mut_ptr/from_raw/into_raw`, raw-pointer types, and every `unsafe` + `NonNull` combination still fail closed and require a current-candidate alias proof. The rule reads no project, function, or field names; the related 53-test gate passes on both Windows and WSL.
      ...
      - The same-profile `kv_set` router `4a6b7b99095e2c91fde2c40a9eac9ca141a9bb276d4c981dd886115350d6f6a0` still failed: rustc/unsafe/oracle passed, while replay expected external-callee fixture result `7` and the candidate implemented `-1`. **P0-A18c6** focuses on a generic executable external-callee behavior contract; project/function names and fixture-value hardcoding remain forbidden. All three WSL worktree runs reported `repo_commit=UNKNOWN0`, so they are local hash-bound AI-routing evidence only.

  WSL run `ai-auxiliary-p0-a18b2b2a-wsl-20260712-160700` used clang 18.1.3, OpenCode 1.17.18, `opencode/deepseek-v4-flash-free`, zero repairs, and four workers: 12/12 preflight ready, 12 provider calls, 8 candidates, 2 exact passes, 6 exact failures, 2 contract failures, 1 execution failure, and 1 provider block. The exact passes were `real-fdb-calc-crc32` and `real-fdb-is-str`. This improves the prior A18b run from one exact pass to two, but a fixed 12-item sample does not justify a generic-stability or competition claim. Both contract failures were invalid applied-artifact bindings; the interior-projection execution failure was the AI draft failing the unique safe-mutable-interior-projection syntax gate, not a ReplayCallPlan v2 executor regression; libuv was provider-blocked by an invalid AI response. This remains `wsl-local-simulation`, with `competition_success_numerator=0` and `translation_coverage_numerator=0`.

  The post-P0-A18b2c2 WSL auxiliary run `ai-auxiliary-p0-a18b2c2-deepseek-wsl-20260712-180000` used the same fixed 12 items, `opencode/deepseek-v4-flash-free`, at most one repair round, and three workers. Preflight was 12/12 ready; 12 provider calls produced 8 candidates, with 1 exact pass, 7 exact failures, 3 contract failures, 1 execution failure, and no provider block. No repair round actually executed. The only exact pass was `real-fdb-is-str`; `real-fdb-calc-crc32` regressed to common exact gates failed, all three contract failures remained invalid applied-artifact bindings, and the execution failure remained the interior projection. This does not improve on the previous two exact passes, so replay planning cannot yet be claimed to improve AI success rate. The report is `target/ai-auxiliary-p0-a18b2c2-deepseek-wsl-20260712-180000/summary/ai-auxiliary-cross-project-suite-report.json` with SHA-256 `bc26d70321086cd5a6e27b9e5d13912aaa29c32445160536364fe9ad00f27458`; the run remains `wsl-local-simulation` with `competition_success_numerator=0` and `translation_coverage_numerator=0`.

  The post-P0-A18b2c3 WSL auxiliary run `ai-auxiliary-p0-a18b2c3-deepseek-wsl-20260712-192612` used the same fixed 12 items, `opencode/deepseek-v4-flash-free`, at most one repair round, and three workers. Windows and WSL preflight were both 12/12 ready. Twelve initial provider calls produced eight candidates, with 2 exact passes, 6 exact failures, 2 contract failures, 1 execution failure, and 1 provider block. Reopening the unit-bound repair reports proves another six repair calls, 18 total; the old aggregate incorrectly reported zero repairs. Those six repairs ended as two timeouts, three exhausted runs, and one invalid patch. The exact passes were `real-fdb-calc-crc32` and `real-fdb-is-str`. Both contract failures came from fixture-only compile stubs replacing the canonical draft and invalidating the applied-artifact binding; the execution failure remained the interior projection, and read-kv-body was provider-blocked by an invalid AI response. This recovers from the immediately preceding one-pass run but only returns to the earlier two-pass baseline, so P0-A18c remains open. The report is `target/ai-auxiliary-p0-a18b2c3-deepseek-wsl-20260712-192612/summary/ai-auxiliary-cross-project-suite-report.json` with SHA-256 `7e8bd77b9afd6c5883d007a47c5058d47a12d1a5d973b86e5ff042b908e2d5c0`; the run remains `wsl-local-simulation` with `competition_success_numerator=0` and `translation_coverage_numerator=0`.

  The post-P0-A18c1 WSL auxiliary run `ai-auxiliary-p0-a18c1-deepseek-wsl-20260712-204206` used the same fixed 12 items, `opencode/deepseek-v4-flash-free`, at most one repair round, and three workers. Preflight was 12/12 ready. Twelve initial plus four response-proven repair calls produced 11 candidates from 16 total provider invocations. Results were 2 exact passes, 9 exact failures, no contract failure, 1 execution failure, and no provider block; no unit triggered the empty-completion retry. The exact passes remain `real-fdb-calc-crc32` and `real-fdb-is-str`, so exact success did not improve. Relative to the previous run, candidates increased 8 to 11, contract failures fell 2 to 0, provider blocks fell 1 to 0, and total calls fell 18 to 16. First-round rustc API mismatches under a valid target contract fell 3 to 1: `tsl_to_blob` and `kv_set` now compile, while `kv_to_blob` still fails. The sole execution failure was a zero-start AI draft missing the unique safe owner-interior alias, which the old path surfaced as an unstructured `ValueError`; P0-A18c2 fixes that harness boundary. The report is `target/ai-auxiliary-p0-a18c1-deepseek-wsl-20260712-204206/summary/ai-auxiliary-cross-project-suite-report.json` with SHA-256 `512641d1ece1d6dcf8b5bc20af3208da467a1c5284931d3c2a32bdb5cf8551c1`. It remains `wsl-local-simulation` with both public numerators at zero, and P0-A18c remains open.

  The P0-A18c3 targeted WSL `kv_to_blob` run used `opencode/deepseek-v4-flash-free`, one initial call, and zero repairs. The model emitted every supporting struct on the first attempt and passed both rustc and generated replay, proving that the previous run's only API compile mismatch is removed for this sample. The exact router remained semantic false because an unobserved compiler-owned proxy field still used `*mut c_void` and triggered `alias_proof_missing`. Router SHA-256 is `0c5f8f7130f4b6b8c163f1007af130cc6016f48234a54e6dde4485a8a7b31316`; candidate-manifest SHA-256 is `16a7fa57a8038cb9c66a57c8f8a8eeb04cb171dba68a5cc9b6802aa55529f18b`. This targeted run is not fixed-suite success-rate evidence. P0-A18c4 fixes the safe proxy type and awaits another targeted exact run.
