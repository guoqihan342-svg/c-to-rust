...
- [x] **P0-A15: recomputable AI caller/callee context and prompt scope**
  ...
  Completion evidence: ContextPack v3's 16 KiB C boundary now includes `external_direct_callees` and `call_expression_contract` under the common sensitive-field and host-path sanitizer. The independent `context_scope.py` derives a stable scope from actual loaded excerpts, failure summaries, and direct-callee facts; normal generation and cache hits share this rule. The schema-v5 manifest records the result, and the summary validator reopens the hash-bound ContextPack, independently recomputes it, and rejects scope drift while preserving v1-v4 compatibility. This adds no project, function, or fixture special case and does not increase the semantic numerator. The same 153-test group passes on Windows and WSL, with one platform-specific skip on each.
