...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  - [ ] **P0-A18b: structured ReplayCallPlan**. Extract one structured plan from the replay generator with parameter names/order/Rust types, C mappings, retained length fields, return type, ABI, unsafe policy, and call args. The renderer and prompts must consume the same plan. Remove legacy function-name dispatch for `fdb_kv_to_blob/set/del`; a function name is contract data only.
    ...
    - [ ] **P0-A18b2: plan every legacy adapter**. The remaining scalar, record, and external-sequence renderers still construct calls directly instead of consuming ReplayCallPlan, so the parent P0-A18b item remains open.
      ...
      - [ ] **P0-A18b2b: record/external plan v2**. Migrate the inventoried nine record-state and four scripted external/call-continue renderers.
        - [x] **P0-A18b2b1: record plan-v2 core**. Bounded `bindings`, `binding_value/borrow_mut`, recursive record initializers, `binding_path` observations, typed `u32/usize/bool` codecs, and mutable-root noalias closure are wired into ContextPack, schema, and fresh binding. Constant, field add, field-plus-scalar add, postfix increment, interior projection, stats sequence, and guarded stats have left legacy dispatch. The v2 implementation is split into a 38-line facade and 377/347/218-line parts.
        - [ ] **P0-A18b2b2: remaining state/external paths**.
          ...
          - [ ] **P0-A18b2b2b: scripted external 4/4**. Add bounded scripted-callee stimulus, call count/args, and event-sequence probes, then migrate the external/call-continue renderers.
      - [ ] **P0-A18b2c: eliminate simple legacy renderers**. The inventory also found 13 direct-call renderers for scalar returns, mutable out reports, record/pointer identity, opaque contexts, and related shapes. Convert all to plan producers before deleting the legacy fallback and readiness OR list.
  ...
  WSL run `ai-auxiliary-p0-a18b-wsl-20260712-1434` used the same clang/OpenCode/DeepSeek setup with zero repairs and four workers: 12/12 preflight ready, 12 provider calls, 9 candidates, 1 exact pass, 8 exact failures, 1 contract failure, 1 execution failure, and 1 provider block. Compared with A18a's 10 calls, 7 candidates, and 2 exact passes, the two replay pre-provider blockers are gone, but exact success did not improve and no translation-rate claim is justified. zlib's first candidate correctly emitted `adler32(adler, buf, len)` and passed rustc plus the new replay; libuv's first candidate passed rustc but the new typed replay rejected report field type/value mismatches. Both remain blocked by their fresh C-oracle compilation lanes. This is still `wsl-local-simulation`; all competition and translator numerators remain zero.
