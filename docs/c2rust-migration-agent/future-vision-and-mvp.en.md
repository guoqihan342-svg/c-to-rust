...
| 2 | P0-A9 generic raw C2Rust/C2Rust+repair exact materialization | Complete; only regressions and cross-project-data-driven tuning remain |
| 3 | P0-A10 fixed-suite harness closure | Inputs, one-shot execution, strict summary validation, and provider circuit breaking are complete; one full-suite acceptance run with real GLM candidates and publishable success metrics remains |
...
- [ ] **P0-A6: real GLM-5.1 candidate generation lane**
  ...
  Current progress: the candidate generator, schema-v2 manifest, sensitive/host-path cleanup, strict JSON parsing, candidate materialization SHA check, balance/auth/timeout classification, and `auto_migrate --ai-first-candidate` are implemented. WSL can list and launch `zai/glm-5.1`. Earlier calls reported insufficient balance/resource package; with the current credential on 2026-07-12, two consecutive calls returned `provider_timeout` and an empty response under both 30-second and 180-second windows. No real GLM candidate exists yet, so this item remains open. Model visibility, a started process, and a passing deterministic fallback are not candidate evidence.
