...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **Current boundary (2026-07-14):** A19 has completed one real WSL DeepSeek auxiliary worker plus bounded Ninja/CMake link and static-archive closure, actual rustup-toolchain content binding, non-degrading SandboxBackend requirements/VerificationPlans, a bubblewrap host capability probe, candidate/project Cargo evidence reopening, immutable coordinator-receipt epochs, a project-repair CAS/event/artifact ledger, minimal relevant IR contexts, an isolated provider-contract simulation, host IR rebuilding and re-coordination, deterministic single-step CompletionCoordinator scheduling, and a schema v7 host Cargo project-diagnostic intake. That intake admits only structured rustc project compile errors from the current cohort/generation; warnings, generic failures, overflow, environment or sandbox blockers, and stale unit paths cannot trigger AI. Receipt-v2/shared-queue wiring, post-repair reverification, dedicated link/init/feature/ABI producers, the global planner, A19e7 verifier capabilities/nonces, positive whole-project gates, and real held-out semantic acceptance remain incomplete. The parent stays unchecked and the translator numerator does not increase; existing provider-contract simulations and local WSL smoke are not `competition-exact`.
      ...
      - [ ] A19d3c2: persist queue/events/attempts in the TransitionAuthority ledger, let the project repairer consume only the smallest relevant IR/diagnostic context and emit a new IR candidate, and make recovery, budget exhaustion, rollback, and re-coordination replayable events.
        ...
        - [ ] A19d3c2b: make the sole CompletionCoordinator automatically schedule, execute, or resume the latest unfinished project queue and route real verifier diagnostics that cannot be localized to a unit into it. This layer emits only a new RustProjectIR/receipt and hands control to A19e6; it has no promotion or completion authority, and neither low-level APIs nor model output may complete the project directly.
          ...
          - [ ] A19d3c2b3: normalize compile/link/init/feature/ABI diagnostics that a dedicated host verifier cannot reliably map to a unit into project diagnostics, bind them to the current cohort, generation, raw observation, and verifier receipt, and admit them to the same queue. Environment or sandbox failures must not impersonate repair diagnostics.
            ...
            - [ ] A19d3c2b3b: include validated intakes in a v2 coordinator receipt compatible with old receipts and in the shared project-repair queue. Reopen the intake, current cohort/generation, and original verifier bindings before dispatch or resume. A verifier-origin diagnostic disappearing from a static IR receipt may become only `pending-reverification`, never resolved; only an explicit new receipt from the same gate on the new generation can close the item.
  ...
  **Execution order, driven by completion conditions rather than test identity:**
  ...
  5. A19d3c2b3 and A19d3d2: connect real project-diagnostic intake to the implemented bounded AI-repair execution/recovery path, then extend RustProjectIR/Cargo to real multi-target, feature, and native-link projects. This may proceed in parallel with A19e7 dedicated semantic runners, but Cargo generation must not bypass RustProjectIR.
