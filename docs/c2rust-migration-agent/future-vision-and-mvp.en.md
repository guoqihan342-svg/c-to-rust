...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  WSL run `ai-auxiliary-p0-a18-wsl-20260712-1320` used clang 18.1.3, OpenCode 1.17.18, `opencode/deepseek-v4-flash-free`, zero repairs, and four workers. It made 10 provider calls, generated 7 candidates, and produced 2 exact passes across 12 items. `real-fdb-is-str` generated `value: &[u8], len: usize -> bool` on the first attempt and passed every exact gate, proving A18a fixes the original dropped-`len` failure. `zlib/adler32-step` and `libuv/ip4-addr` were blocked before provider launch because their replay drafts still contain no real function call. Five exact failures, two fresh-artifact contract failures, and one WSL worktree `.git` pointer environment failure remain non-successes. This is `wsl-local-simulation`; all competition and translator numerators remain zero.
