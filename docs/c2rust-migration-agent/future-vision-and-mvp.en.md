...
| Active translator track | P0-A18c / P0-A10 | A18c8a/A18c8b are closed; next implement the A18c8c generic executable C-oracle call plan while retaining two scripted-replay model-stability gaps |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8b | The repo-owned compile-DB/generated-include/static-library/link/symbol/ABI closure is closed, separating `c_boundary.files` from the link closure |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8a/A18c8b are complete; next make a generic C-oracle call plan execute fixture calls and output comparison while AI, deterministic fallback, and exact validation continue handling two scripted model failures |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [x] **P0-A18c8b: real-project fresh-C-oracle build closure**. Prefer a bound compile database, generated includes, and build-artifact/link contracts. Missing context fails closed; zlib/libuv names cannot select semantic stubs or test-only link flags.
        ...
        - Producer and validator rebuild the same ordered argv from the closure. Invalid closures produce `native_build_closure_invalid`, zero argv, and structured diagnostics. The slice spec/build profile and ContextPack carry the closure manifest path/SHA; fresh-oracle verification reopens the closure and includes compile-DB/include/library/link/symbol/config/toolchain/ABI identity in flags and the reuse key. The translator does not pretend to replay the compile DB yet: the Rust frontend still lacks compile-command entry selection and argument sanitization.
        ...
        - The full zlib auto-translation schema validator remains blocked because the real macro-heavy source currently emits an empty CFG. The obsolete `adler32_step` AST fixture was removed and must not be restored as synthetic proof for real `adler32_z`. This is a generic preprocessing/compile-context translator gap; it does not invalidate the native-build closure proven here, but zlib translation must not be called passed until that generic parser path closes.
      - [ ] **P0-A18c8c: generic executable C-oracle call plan**. Generate data-driven harness calls, input initialization, return/record-field encoding, and stdout comparison from C signatures, fixture codecs, observable outputs, and ABI. Project/function identities cannot select adapters. Fresh oracle may enter AI repair/exact only after the harness executes fixture target calls and every output gate matches.
