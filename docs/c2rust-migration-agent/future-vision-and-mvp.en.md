...
| Active translator track | P0-A18c / P0-A10 | A18c8a/A18c8b/A18c8b1/A18c8c1 are closed; next extend A18c8c with output-parameter and record-field projections, while separately closing function-table indirect calls and record-layout `sizeof` gaps |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8c1 | The harness now generates direct-return C calls generically from ReplayCallPlan and regenerates the complete invocation source during validation; record output parameters remain fail closed and no C/Rust semantic pass is claimed |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8a/A18c8b/A18c8b1/A18c8c1 are complete; next extend the generic C-oracle call plan with output parameters, record-field projections, and a strict structured output protocol while AI, deterministic fallback, and exact validation continue handling the remaining semantic gaps |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8c: generic executable C-oracle call plan**. Generate data-driven harness calls, input initialization, return/record-field encoding, and stdout comparison from C signatures, fixture codecs, observable outputs, and ABI. Project/function identities cannot select adapters. Fresh oracle may enter AI repair/exact only after the harness executes fixture target calls and every output gate matches.
        ...
        - This stage remains `semantic_pass=false` and does not increase the translation coverage numerator. libuv still refuses because its omitted C output parameter and C record-field projections are undeclared. Output initialization, record/byte-order projections, a strict structured output protocol, and C/Rust diff remain later A18c8c work.
