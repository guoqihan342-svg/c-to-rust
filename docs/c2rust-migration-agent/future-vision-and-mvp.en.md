...
| Active translator track | P0-A18c / P0-A10 | A18c8h4a compiles candidate+replay while still forbidding compile-only callee execution, exposing the real libuv candidate versus safe replay API mismatch as machine-readable evidence; next repair the API shape and add executable callee semantics/no-escape |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h4a | Compile-only external bindings now trigger a candidate+replay compile preflight with `execution_allowed=false`. The WSL clang 18.1.3 probe confirms that the draft passes rustc while preflight fails on missing `Ip4AddrReport`/safe API shape; replay still skips and `semantic_pass=false` |
