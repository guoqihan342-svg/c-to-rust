...
- [ ] **P0-A6: real GLM-5.1 candidate generation lane**
  ...
  Current progress: the candidate generator, schema-v2 manifest, sensitive/host-path cleanup, strict JSON parsing, candidate materialization SHA check, balance/auth/timeout classification, and `auto_migrate --ai-first-candidate` are implemented. WSL can list and launch `zai/glm-5.1`. OpenCode writes provider errors to its bounded log and then continues internal retries, so the old harness reached its outer 30/180-second limit first and recorded an empty response as `provider_timeout`. On 2026-07-12, a new log-offset diagnostic that emits only fixed sentinels and never stores raw logs or secrets reclassified one real call as `provider_insufficient_balance`. No real GLM candidate exists yet, so this item remains open. Model visibility, a started process, and a passing deterministic fallback are not candidate evidence.
