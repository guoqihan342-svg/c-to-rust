...
| Active translator track | P0-A18c / P0-A10 | A18c8a/A18c8b/A18c8b1/A18c8c are closed; next close the zlib function-table indirect-call and libuv record-layout `sizeof` translator gaps separately |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8c3 | Generic C call plans now support declarative output pointees, record-field/BE16/object-byte projections, and strict actual-value records; real libuv matches 12 records while the translator still refuses unproven `sizeof(*addr)` layout |
