...
| Latest development stage | P0-A19 build closure, verification authority, and real auxiliary-model lane | Arbitrary-repository inventory, SCC/DAGs, CMake/Ninja/link/archive closure, paged ContextPacks, isolated OpenCode, SQLite state, Cargo generations, toolchain content binding, host project gates, and read-only held-out revalidation are implemented. Positive candidate verifiers, Meson/configure, a usable competition-equivalent sandbox, and real held-out semantic acceptance remain open. This batch passes 134 tests on Windows with one symlink-permission conditional skip and 134/134 on WSL; the success count stays 38 |
...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **Current boundary (2026-07-13):** A19 has completed one real WSL DeepSeek auxiliary worker plus bounded Ninja/CMake link and static-archive closure, actual rustup-toolchain content binding, CLI path/exit-code authority, and read-only-ledger held-out revalidation. Positive candidate compile/oracle/negative/unsafe-alias/ABI/final execution is not fully connected, and project-oracle, a real sandboxed Cargo build, and real held-out semantic acceptance remain incomplete. The parent stays unchecked and the translator numerator does not increase. This batch runs and passes 134 tests on Windows with one symlink-permission conditional skip, and passes 134/134 on WSL. These remain only `local-simulation` and `wsl-local-simulation`.
  ...
  **A19c OpenCode multi-worker candidate portfolio**
  ...
  - [x] Close plan/preflight/request/attempt drift, lease re-entry, heartbeat, non-atomic ingest, tool events, secret input, parent-environment leakage, agent permission override, pre-spawn command-start accounting, and missing provider-evidence persistence. This batch passes 134 tests on Windows with one conditional skip and 134/134 on WSL.
