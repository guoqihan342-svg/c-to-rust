...
| Active translator track | P0-A18c / P0-A10 | A18c8a through A18c8g are closed; next model generic mutable-scalar addresses passed to `void *`, then close the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8g | Clang type candidates retain the deepest canonical spelling while ABI is pending, translation-unit typedefs bind function bodies and record fields, and alias/desugared conflicts fail closed; the real libuv blocker advanced to mutable-scalar address passing into `void *` |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8a through A18c8g are complete; next close generic mutable-scalar address-to-`void *` and function-table indirect-call gaps while exact validation continues handling the remaining semantic failures |
