...
| Active translator track | P0-A18c / P0-A19 / P0-A10 | h5b has closed oracle-free case/field feedback, and h5c candidate-source-driven ABI gating is now active. Work then moves directly to unfamiliar-repository whole-project discovery, decomposition, parallel candidates, integration, and repair instead of extending syntax around one known slice |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h5b | ReplayCallPlan-derived opaque assertion inventory, stable Windows/POSIX panic aliases, the real runner, wrapper, exact gate, negative mutation, and repair fact are closed. The model receives only `{case_id, observable_field}`. The Windows stage passes 67 tests with one platform skip and WSL passes 67/67; both 163-test full runs add no failure. `semantic_pass=false` and the success count remains 38 |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | h5a/h5b are complete. h5c candidate-source-driven ABI classification is active: safe candidates with no ABI dependency must not inherit replay-failure noise, while raw/extern/repr/layout/target-width dependencies still require complete proof. Rerun the same exact gates after completion |
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8h: generic mutable-scalar address to `void *` call argument**. Model a temporary mutable borrow only for a provably complete integer-scalar direct/record-field lvalue address, a direct external call, and an explicit mutable `void *` parameter. Alias, lifetime, alignment, width, signature, and post-call read paths must be verifiable. Const, nullable, bitfield, packed/volatile/atomic, pointer arithmetic, cross-call escape, and overlapping mutable borrows remain refused; no `in_addr_t`, libuv, or fixed-callee identity mapping is allowed.
        ...
        - [ ] **P0-A18c8h5c: precise ABI gating for safe Rust candidates**. Audit whether `abi_target_binding_failed` falsely rejects self-contained safe candidates with no raw pointers, `extern`, or `unsafe`. The ABI gate may require proof only when the candidate actually depends on layout, FFI, or target width, instead of inheriting unconditional risk from the original C boundary. Any `repr(C)`, raw pointer, extern ABI, target-width cast, or layout dependency still requires complete hash-bound target/layout proof. Completion requires a renamed safe positive, equivalent raw/extern/layout-drift negatives, and no relaxation of other final-verifier gates.
