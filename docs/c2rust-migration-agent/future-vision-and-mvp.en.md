...
| Active translator track | P0-A18c / P0-A10 | A18c8h4f unifies LF/CRLF hash binding for primary and external-callee source. Real libuv ContextPack provider readiness is now `ready`; next launch a fresh DeepSeek AI-first auxiliary run and converge candidate/API/semantic-gate failures |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8h4f | External callees and behavior dependencies bind actual SHA, declared SHA, and `exact/newline_equivalent` mode, all recomputed by readiness. Non-newline content, mode, or binding drift remains fail-closed. Real readiness is ready, but no model is invoked yet and `semantic_pass=false` remains unchanged |
