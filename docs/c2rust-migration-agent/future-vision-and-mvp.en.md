...
- [ ] Keep expanding the generic typed IR emitter instead of restoring crc32/FlashDB special cases: initialized record local copy, by-value record dot-field compound assignment, and whole-record return with unique named complete direct scalar field inventory are now in the candidate subset; next prioritize pointer-aware record access, record update/inc-dec field write design, and stronger layout/ABI evidence.
