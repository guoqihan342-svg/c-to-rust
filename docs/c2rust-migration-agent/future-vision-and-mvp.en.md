...
| Latest development stage | P0-A19 project-interface coordination and AI-repair authority | Arbitrary-repository inventory, SCC/DAGs, build closure, paged ContextPacks, isolated OpenCode, SQLite state, RustProjectIR/Cargo generations, immutable coordinator-receipt epochs, project-repair state, authoritative-IR recovery, and one-step latest-queue scheduling/recovery are implemented. The global planner, CompletionCoordinator execution and queue draining, positive semantic verifiers, competition-equivalent aggregate isolation, and real held-out semantic acceptance remain open. This batch runs 351 tests on Windows with 349 passing and two conditional skips, plus 351/351 on WSL; the success count stays 38 |
| Latest recorded strict-evidence snapshot | `25/33` | Historical run `20260711T-finite-p0-t31`; `stress_loops=0`, and eight historical evidence drifts remain. This is not the current 351-test contract gate |
...
| 2 | P0-A19 unfamiliar-C-project whole-project AI harness | In progress: generated closure and positive verification path | Repository discovery, paged DAGs, role portfolios, SQLite state, Cargo generations, CMake/Ninja/link/archive closure, and a fail-closed sandbox contract exist. Next complete Meson/configure, positive candidate verifiers, a usable competition-equivalent sandbox, and real held-out build/oracle acceptance. Project/function/path/fixture identity dispatch is forbidden |
...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **A19a arbitrary-repository intake and build-closure discovery**
  ...
  - [ ] **A19a6: close Meson and constrained-generation facts.**
    - [ ] A19a6a: read existing repository-local `meson-info/intro-targets.json`, `intro-buildoptions.json`, and compiler/dependency metadata without execution; bind targets, sources, generated sources, compile/link arguments, and introspection-file SHAs. Fail closed on incomplete schemas, paths, or closure.
  ...
  - [ ] **A19a7: canonical BuildIR and build-adapter convergence.**
    ...
    - [x] A19a7b: require Make, CMake, Ninja, and Meson adapters to emit only BuildIR. Adapter-specific raw facts may exist only as audit attachments referenced by BuildIR; planners, DAGs, ContextPacks, Rust reconstruction, and verifiers must not consume private adapter shapes. Facts that cannot be normalized losslessly become explicit blocked/refused outcomes, never guesses or silent omissions.
    - [ ] A19a7c: use one BuildIR validator to reopen and recompute every provenance/hash, generated-input closure, target/TU uniqueness rule, and ordered link/archive closure. Source, build-metadata, toolchain, or canonical-projection drift must fail closed before worker launch and final verification.
      - [x] A19a7c1: reopen raw attachments and recompute the compile database, source/generated bindings, Meson facts, TU/target uniqueness, ordered target DAG, and canonical projection. Any repository-fact drift fails closed before planning and worker admission.
      - [ ] A19a7c2: under the competition profile, bind and reopen the actual C compiler/archiver/linker binaries, versions, target/sysroot, and environment fingerprint, then let CompletionCoordinator rerun the same BuildIR verifier before `project-final`. A compile-database driver token alone cannot close toolchain drift.
  ...
  - [ ] **A19a8: explicitly enabled, strongly isolated Make dry-run fact collection.**
    ...
    - [ ] A19a8b: fix the invocation to `make -B -n -j1 --no-print-directory -f <makefile> -- <targets>` and reject arbitrary commands, flags, environments, shells, configure steps, or generators. Because `make -n` may still execute `$(shell ...)` and remake included Makefiles, always run it inside the proven sandbox and reject recursive Make, shell control, command chains, libtool/configure, ambiguous compile-plus-link commands, path escape, timeout, and output flooding.
    - [ ] A19a8c: normalize only losslessly parsed direct compiler, archiver, ranlib, and linker invocations into BuildIR. Bind the collection report to Makefile/source/toolchain/sandbox facts and raw-output SHA; BuildIR may mark declared but absent outputs as `materialized=false`. The report is always nonsemantic evidence with numerator zero and may enter the migration DAG only after the shared BuildIR validator reopens it.
  ...
  **A19b project dependency graph and bounded decomposition**
  ...
  - [ ] **A19b3: canonical CIndex, content-addressed evidence, and exact provenance.**
    - [ ] A19b3a: distinguish raw source/header blob SHA from normalized semantic-fact SHA. Header/include records retain reopenable content references only, so storage for a header shared by many TUs grows near-linearly with unique content rather than reference count.
    - [ ] A19b3b: bind every parser/preprocessor/include blocker to source path, source SHA, directive SHA when available, offset/range, and a reopenable evidence-set reference. Summaries may compress presentation but must not collapse same-offset findings from different files or allow dangling/tampered evidence.
        ...
        - [ ] A19d3c2b: make the sole CompletionCoordinator automatically schedule, execute, or resume the latest unfinished project queue and route real verifier diagnostics that cannot be localized to a unit into it. This layer emits only a new RustProjectIR/receipt and hands control to A19e6; it has no promotion or completion authority, and neither low-level APIs nor model output may complete the project directly.
          ...
          - [ ] A19d3c2b3: normalize compile/link/init/feature/ABI diagnostics that a dedicated host verifier cannot reliably map to a unit into project diagnostics, bind them to the current cohort, generation, raw observation, and verifier receipt, and admit them to the same queue. Environment or sandbox failures must not impersonate repair diagnostics.
            ...
            - [ ] A19d3c2b3c: uniquely attribute a complete batch of whole-cohort verifier diagnostics before admitting link/init/feature/ABI repairs from independent evidence producers. Any unclassified, mixed, or environment failure blocks the entire batch. Do not check b3 until b3b and b3c close.
              ...
              - [ ] A19d3c2b3c2: normalize an unresolved project symbol as a link diagnostic only when it binds uniquely to the current RustProjectIR. Missing linker/toolchain/native library, offline dependency, permission, resource, and other environment failures remain blockers with zero intake and zero provider calls.
  ...
  **A19e project-level verification and precise repair**
  ...
  - [ ] **A19e4: close positive candidate verification, quarantine, and completion.**
    - [ ] **A19e4a: make the run contract and DAG cohort nonreplaceable.**
      - [ ] A19e4a1: generate the integration manifest before ledger creation and bind its path/SHA/size, complete dependency edges, and DAG SHA into the run metadata/schema. CLI and workers no longer accept a replaceable manifest.
      - [ ] A19e4a2: compute transitive closure only from current active roots, their latest completed Rust candidates, and valid last-good dependencies, then bind it as an immutable verification candidate set. Missing members, non-latest candidates, incomplete dependencies, unrelated members, or cohort drift fail closed.
      - [ ] A19e4a3: distinguish `wave-provisional` from `project-final` cohorts. Before completion, run a final revalidation barrier over the complete last-good set so early-wave gates cannot impersonate final whole-project gates.
    - [ ] A19e4b: generate an isolated immutable quarantine Cargo generation from that set. It must not modify `CURRENT`/last-good and may run `cargo check/test` only in a proven networkless OS sandbox.
    - [ ] **A19e4c: make the host compile decision nonforgeable.**
      - [ ] A19e4c1: derive candidate compile pass/fail from real execution in a fixed host adapter. CLI/API callers cannot supply pass, verifier ids, candidate-set ids, manifests, or arbitrary commands.
      - [ ] A19e4c2: bind compile evidence to candidate source, integration manifest, quarantine manifest/generation state, verification candidate set, fixed Cargo command, toolchain, and sandbox contract; final verification and promotion reopen every binding and reject any drift.
    - [ ] A19e4d: route module-local Rust diagnostics to the matching repairer. Sandbox, toolchain, timeout, and resource failures remain blocked/deferred and never become semantic candidate failures. Unattributed project errors enter project-level repair rather than arbitrary unit attribution.
  ...
  **Execution order, driven by completion conditions rather than test identity:**
  ...
  4. A19c4c, A19e4a-A19e4d, and A19e8: close candidate sets, quarantine, host-derived compile decisions, and precise repair on the new authority and non-degrading sandbox, and first drive information-gain scheduling from verifier-owned failure, critical-path, cost, and convergence facts. Model self-scores must not enter priority. Bubblewrap and equivalent SandboxBackend implementations may be developed in parallel, but all share one conformance suite.
  5. A19d3c2b3c2-c3, A19e8b2, and A19d3d2: use the closed whole-cohort compile admission, AI-repair, and same-gate revalidation loop as the baseline. First persist reopenable raw stdout/stderr and a recomputable classification receipt, then connect independent link/init/feature/ABI verifiers and extend RustProjectIR/Cargo to real multi-target, feature, and native-link projects. This may proceed in parallel with the A19e7 capability runner, but semantic families must not be guessed from Cargo text and Cargo generation must not bypass RustProjectIR.
  ...
  **A19h code reduction and module boundaries**
  ...
  - [ ] Split every P0-A19 production module, test, and shared test-support file touched by the phase along single-responsibility boundaries to at most 300 lines, enforced by the source-layout gate. Do not evade the limit through compressed formatting, duplicated thin wrappers, or moving large logic into unchecked directories.
