...
| Latest development stage | P0-A19 build closure, verification authority, and real auxiliary-model lane | Arbitrary-repository inventory, SCC/DAGs, CMake/Ninja/link/archive closure, paged ContextPacks, isolated OpenCode, SQLite state, Cargo generations, toolchain content binding, SandboxBackend capabilities/probes, host project gates, and read-only held-out revalidation are implemented. Positive semantic verifiers, Meson/configure, A19e7 process-issued receipts, competition-equivalent aggregate isolation, and real held-out semantic acceptance remain open. This batch passes 288 tests on Windows with two conditional skips and 288/288 on WSL; the success count stays 38 |
...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **Current boundary (2026-07-13):** A19 has completed one real WSL DeepSeek auxiliary worker plus bounded Ninja/CMake link and static-archive closure, actual rustup-toolchain content binding, non-degrading SandboxBackend requirements/VerificationPlans, a bubblewrap host capability probe, candidate/project Cargo evidence reopening, CLI path/exit-code authority, and read-only-ledger held-out revalidation. A19e7 verifier capabilities/nonces, positive oracle/negative/unsafe-alias/ABI/final execution, project oracle, aggregate cgroup/equivalent backends, a real sandboxed Cargo build, and real held-out semantic acceptance remain incomplete. The parent stays unchecked and the translator numerator does not increase. This batch passes 288 tests on Windows with two conditional skips and 288/288 on WSL. A real WSL smoke without `bwrap` fails closed as `bubblewrap_unavailable` with zero candidate execution; these remain only `local-simulation` and `wsl-local-simulation`.
  ...
  **A19c OpenCode multi-worker candidate portfolio**
  ...
  - [x] Close plan/preflight/request/attempt drift, lease re-entry, heartbeat, non-atomic ingest, tool events, secret input, parent-environment leakage, agent permission override, pre-spawn command-start accounting, and missing provider-evidence persistence. The current finite regression passes 288 tests on Windows with two conditional skips and 288/288 on WSL.
