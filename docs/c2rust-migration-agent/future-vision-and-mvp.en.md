...
- [ ] Make the competition clang lane explicit: `config/competition-env/environment.json` does not currently list clang as a baseline tool, and `auto_migrate.py --emit-clang-lowering-report` is opt-in. Either declare clang as an optional/required capability with `CLANG_PATH`, install/check instructions, and a competition wrapper that fails clearly when missing, or keep the default competition lane documented as non-clang legacy/fail-closed evidence. Do not silently rely on local Windows clang.
...
- [ ] Fix unsigned integer modulo semantics before claiming checksum/hash translation breadth: typed IR emission must not use profile-dependent bare Rust `+`, `-`, or `*` for C unsigned arithmetic. Add debug/overflow-checks runtime RED tests for `u32::MAX + 1`, `0u32 - 1`, and unsigned multiply, then emit explicit wrapping operations or an equivalent audited lowering strategy. Signed overflow and invalid shift counts remain separate fail-closed/contract questions.
...
- [ ] Turn route metadata into a real candidate-selection layer, or narrow the naming to provenance. Today `GenericTypedIr` / `Unsupported` mostly labels the result after typed IR emission; future route governance should record the candidate set, selected generator, fallback chain, refusal reason, and validation gates, and must not silently fallback from clang-lowered typed IR to the legacy string translator without visible telemetry.
