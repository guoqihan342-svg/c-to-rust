...
| Active translator track | P0-A18c / P0-A10 | A18c8a through A18c8f are closed; next recover generic typedef canonical/desugared types, then close the zlib function-table indirect-call gap |
| External parallel track | P0-H9 | Revalidate the exact OpenCode + GLM-5.1 contract on the real competition host |
| Latest development stage | P0-A18c8f | `memset(record*, 0, sizeof(record))` enters dedicated typed IR only when the destination type exactly matches the same record-layout proof, then emits a safe complete-record zero-value assignment; the real libuv blocker advanced to typedef field-type recovery |
...
| 2 | P0-A18c8 remaining exact-failure closure | In progress | A18c8a through A18c8f are complete; next close generic typedef canonical/desugared recovery and function-table indirect-call gaps while exact validation continues handling the remaining semantic failures |
...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  - [ ] **P0-A18c: finite cross-project acceptance**. All fixed 12 cases must generate a real replay call, first-round rustc API mismatches must materially improve over A17, and quality remains counted only by exact gates.
    ...
    - [ ] **P0-A18c8: generic semantic closure for the remaining exact failures**.
      ...
      - [ ] **P0-A18c8g: generic typedef canonical/desugared type recovery**. Recover fixed-width integers or already-supported record types from clang type-object `desugaredQualType`/canonical spelling and translation-unit typedef declarations. Spelled typedef, canonical type, and target-ABI width/signedness must agree. Unknown, cyclic, conflicting, or platform-uncertain typedefs remain refused; no `sa_family_t`, libuv, or fixed-header identity mapping is allowed.
