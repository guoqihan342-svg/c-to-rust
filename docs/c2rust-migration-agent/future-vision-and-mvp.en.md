...
- [ ] **P0-A19: unfamiliar-C-project whole-project AI-harness generalization**
  ...
  **Current boundary (2026-07-14):** A19 has completed one real WSL DeepSeek auxiliary worker plus bounded Ninja/CMake link and static-archive closure, actual rustup-toolchain content binding, non-degrading SandboxBackend requirements/VerificationPlans, a bubblewrap host capability probe, candidate/project Cargo evidence reopening, immutable coordinator-receipt epochs, a project-repair CAS/event/artifact ledger, minimal relevant IR contexts, an isolated provider-contract simulation, host IR rebuilding and re-coordination, and one deterministic latest-receipt scheduling step before Cargo generation with idempotent launch ownership, expired-unstarted recovery, and manual-reconciliation blocking. Low-level completion fails closed while the queue is unfinished, and duplicate runtimes cannot acquire model-launch authority twice. The global planner, automatic queue execution/draining, real verifier project-diagnostic intake, A19e7 verifier capabilities/nonces, positive oracle/negative/unsafe-alias/ABI/final execution, project oracle, aggregate cgroup/equivalent backends, a real sandboxed Cargo build, and real held-out semantic acceptance remain incomplete. The parent stays unchecked and the translator numerator does not increase. This batch runs 351 tests on Windows with 349 passing and two conditional skips, plus 351/351 on WSL. The provider-contract simulation is not a real model run; a real WSL smoke without `bwrap` still fails closed as `bubblewrap_unavailable` with zero candidate execution, and none of this is `competition-exact`.
