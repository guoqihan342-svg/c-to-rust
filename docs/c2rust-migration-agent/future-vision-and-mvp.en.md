...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  - [ ] **P0-A18b: structured ReplayCallPlan**. Extract one structured plan from the replay generator with parameter names/order/Rust types, C mappings, retained length fields, return type, ABI, unsafe policy, and call args. The renderer and prompts must consume the same plan. Remove legacy function-name dispatch for `fdb_kv_to_blob/set/del`; a function name is contract data only.
    ...
    - [ ] **P0-A18b2: plan every legacy adapter**. The remaining scalar, record, and external-sequence renderers still construct calls directly instead of consuming ReplayCallPlan, so the parent P0-A18b item remains open.
      ...
      - [ ] **P0-A18b2c: eliminate simple legacy renderers**. Ten of the original 13 direct-call renderers for scalar returns, mutable out reports, record/pointer identity, and opaque contexts are migrated, leaving three. Convert all to plan producers before deleting the legacy fallback and readiness OR list.
        ...
        - [ ] **P0-A18b2c3: record-pointer/opaque 5/5**. Record pointer identity and record projection identity now use the generic v2 producer; record buffer/length identity, opaque key, and opaque key/value remain. This is replay/harness capability only: it does not increase the translation coverage numerator or replace C oracle/Rust replay/diff semantic evidence.
