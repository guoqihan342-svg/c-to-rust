...
- [ ] **P0-A18: put the replay-compatible Rust API contract before the model**
  ...
  The post-P0-A18b2c3 WSL auxiliary run `ai-auxiliary-p0-a18b2c3-deepseek-wsl-20260712-192612` used the same fixed 12 items, `opencode/deepseek-v4-flash-free`, at most one repair round, and three workers. Windows and WSL preflight were both 12/12 ready. Twelve provider calls produced eight candidates, with 2 exact passes, 6 exact failures, 2 contract failures, 1 execution failure, and 1 provider block; no repair round actually executed. The exact passes were `real-fdb-calc-crc32` and `real-fdb-is-str`. Both contract failures were invalid applied-artifact bindings, the execution failure remained the interior projection, and read-kv-body was provider-blocked by an invalid AI response. This recovers from the immediately preceding one-pass run but only returns to the earlier two-pass baseline, so P0-A18c remains open. The report is `target/ai-auxiliary-p0-a18b2c3-deepseek-wsl-20260712-192612/summary/ai-auxiliary-cross-project-suite-report.json` with SHA-256 `7e8bd77b9afd6c5883d007a47c5058d47a12d1a5d973b86e5ff042b908e2d5c0`; the run remains `wsl-local-simulation` with `competition_success_numerator=0` and `translation_coverage_numerator=0`.
