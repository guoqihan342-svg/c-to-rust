...
flowchart TD
    ...
    by-value record dot-field reads/simple assignments,
...
- `crates/c2r-translator/src/typed_ir.rs`
  ...
  - generic emitter 支持窄化的按值 record dot-field 读取和简单字段赋值，可根据实际访问到的标量字段发射一个最小 Rust struct，例如 `struct point { int x; }; int set_point_x(struct point p, int value) { p.x = value; return p.x; }` 会生成 `pub struct Point { pub x: i32 }`、`mut p: Point`、`p.x = value;` 和 `return p.x;`。这只是 candidate generation，不声明完整 C record layout 或 ABI 等价。
...
- 按值 record dot-field 读取和简单字段赋值，并要求字段结果/赋值 RHS 是受支持标量，例如 `struct point { int x; int y; }; int set_point_x(struct point p, int value) { p.x = value; return p.x; }` 可经真实 clang AST 进入 typed IR，并生成最小 Rust `Point` struct、`mut p: Point`、`p.x = value;` 和 `return p.x;`；
- 标量整数二元表达式 `+`、`-`、`*`、`/`、`%`、`&`、`|`、`^`、`<<`、`>>`；
...
- 按值 record dot-field 读取和简单字段赋值只是 candidate generation。当前发射的 Rust struct 是从实际访问到的标量字段派生出来的最小候选形状，不是 C layout 或 ABI proof。没有任何建模字段使用的 record 参数、pointer member access `p->x`、compound/update 字段写、指针/alias-sensitive 字段写、嵌套/匿名 record、union、bitfield、非标量字段、record return/local、struct array、address-taken record、alias write、volatile field 和 semantic acceptance 继续 fail closed。
...
1. 继续用红测优先扩展 generic typed IR 覆盖；当前更适合的后续切口是 pointer-aware record access、record local/return，或继续设计 `switch` / `goto` 的控制流语义。multi `VarDecl` body expansion、`ForStmt` init 多声明、assignment-before-read 无初始化标量局部声明、按值 record dot-field 读取/简单赋值、fixed-width integer aliases、精确 `signed char`、binary operand integral cast preservation、condition/value-position `&&` / `||`、包含窄化 clang-proven promotion/truncation 的 simple scalar compound assignment family、value-position integer implicit cast preservation、纯整数 value-position `ConditionalOperator` / `?:`、窄化 scoped `ForStmt`、step-position prefix inc/dec、standalone value-discarded inc/dec statement、窄化 `do-while`、loop-body `break`、窄化 loop-body `continue`、readonly `*(p+i)` read、mutable pointer output write 和一层单个 nested direct call argument 已有 direct/clang-lowered/real-clang smoke 覆盖；bounded readonly `*(p+i)` read、mutable pointer write 的 fail-closed offset/const 边界、按值 record dot access 与 `->` 的 fail-closed 边界、nested direct call 的 deeper/multiple sibling 边界、lazy `?:` 分支语义、short-circuit lazy 求值、for-loop scope 边界、`ForStmt` continue-step 顺序、standalone/value-position inc/dec 边界和 `DoWhile` continue-condition check 要继续保留 real-clang 与 fail-closed 边界覆盖；除这些窄切片外的任意 pointer arithmetic、任意 pointer comparison、float comparison、未建模 mixed-width conversions、side-effect operands 和 semantic acceptance 仍要 fail closed。
