...
- 只允许出现在条件中的 logical not `!expr`，例如 `if (!value)` / `while (!value)` 生成 `value == 0`，`!(value > 0)` 生成反转后的 comparison condition；
...
- condition-only logical not 只是 candidate generation。value-position `!` 的 C `int` 结果语义，例如 `return !x`、assignment RHS 或 declaration initializer，仍 fail closed；operand 含 call、inc/dec、未建模 deref/side effect、pointer/float/unsupported type 时继续 fail closed。
- 复杂函数指针、未建模 alias write、volatile/硬件寄存器、宏副作用和跨线程/中断语义仍应 fail closed 或进入更高路线。

## 下一步实现切口

1. 继续用红测优先扩展 generic typed IR 的标量表达式覆盖；下一小步候选是 value-position logical not 的 C `int` 结果语义，例如 `return !x` / RHS / initializer，但 pointer/deref/call 副作用、unsigned/wrapping 算术和完整 C unary 语义在 typed rules 明确前继续 fail closed。
