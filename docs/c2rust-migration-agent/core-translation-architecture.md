...
- `crates/c2r-translator/src/typed_ir.rs`
  ...
  - generic emitter 也支持 single-pointer-param alias gate 下的 narrow mutable record pointer scalar field assignment、简单 field compound update、同字段 definite write 后 read，以及 clang desugar 后的 statement inc/dec update。例如 `void bump_point_x(struct point *p) { p->x++; --p->x; }` 会生成 `pub fn bump_point_x(mut p: &mut Point)` 以及 `p.x = (p.x + 1i32);` / `p.x = (p.x - 1i32);`。多 pointer 参数、nullable mutable pointer、read-before-write、maybe-write 后读取、复杂 base/target/RHS、非标量字段、layout/ABI 声明和 semantic acceptance 仍 fail closed。
...
- single-pointer-param gate 下的 direct mutable record pointer scalar field assignment、简单 field compound update、同字段 definite write 后 read，以及 clang desugar 后的 statement-position field inc/dec。例如 `void bump_point_x(struct point *p) { p->x++; ++p->x; p->x--; --p->x; }` 会生成 `p: &mut Point` 和四条 `+/- 1i32` 的直接 Rust 字段赋值；
- 标量整数二元表达式 `+`、`-`、`*`、`/`、`%`、`&`、`|`、`^`、`<<`、`>>`；
