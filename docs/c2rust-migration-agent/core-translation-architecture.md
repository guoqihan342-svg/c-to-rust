...
1. 继续用红测优先扩展 generic typed IR 覆盖；当前更适合的后续切口是 struct/alias memory model 设计，或继续设计 `switch` / `goto` 的控制流语义。multi `VarDecl` body expansion、`ForStmt` init 多声明、assignment-before-read 无初始化标量局部声明、fixed-width integer aliases、精确 `signed char`、binary operand integral cast preservation、condition/value-position `&&` / `||`、包含窄化 clang-proven promotion/truncation 的 simple scalar compound assignment family、value-position integer implicit cast preservation、纯整数 value-position `ConditionalOperator` / `?:`、窄化 scoped `ForStmt`、step-position prefix inc/dec、standalone value-discarded inc/dec statement、窄化 `do-while`、loop-body `break`、窄化 loop-body `continue`、readonly `*(p+i)` read、mutable pointer output write 和一层单个 nested direct call argument 已有 direct/clang-lowered/real-clang smoke 覆盖；bounded readonly `*(p+i)` read、mutable pointer write 的 fail-closed offset/const 边界、nested direct call 的 deeper/multiple sibling 边界、lazy `?:` 分支语义、short-circuit lazy 求值、for-loop scope 边界、`ForStmt` continue-step 顺序、standalone/value-position inc/dec 边界和 `DoWhile` continue-condition check 要继续保留 real-clang 与 fail-closed 边界覆盖；除这些窄切片外的任意 pointer arithmetic、任意 pointer comparison、float comparison、未建模 mixed-width conversions、side-effect operands 和 semantic acceptance 仍要 fail closed。
