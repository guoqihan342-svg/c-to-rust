...
> Date: 2026-06-27
> Historical analysis based on the `codex/flashdb-rust-skeleton` branch; current maintenance branch is `codex/flashdb-rust-skeleton`
...
- `ForStmt` with `goto` / `switch`, a condition variable slot, empty condition/step, inc/dec in conditions/return values, call-argument inc/dec outside the single scalar direct-call or single-chain nested direct-call shapes, complex init/step, non-simple-scalar init/step, non-simple integer-variable step targets, complex standalone inc/dec targets, parenthesized/comma steps, or general value-position inc/dec. `DoWhile` conditions containing calls/inc/dec/side effects still fail closed. `break` is open only as direct loop-exit candidate generation inside loop bodies, and `continue` is open only as narrow loop-body candidate generation with explicit `DoWhile` condition checks or `ForStmt` step-before-continue emission.
...
- Function-pointer callees, complex call side effects, and nested calls in conditions. Direct-call arguments only allow one-level ordinary `outer(inner(value))` shapes plus single-chain multi-layer `outer(middle(inner(value++)))` shapes with one scalar inc/dec leaf; non-chain deeper nesting, multiple sibling nested calls, ordinary arguments mixed into any side-effect layer, and nested calls hidden inside binary/index/cast operands still fail closed.
