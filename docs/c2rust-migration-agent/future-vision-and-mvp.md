...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
  ...
  **A19a 任意仓库入口与构建闭包发现**
  ...
  - [ ] **A19a7：canonical BuildIR 与构建 adapter 收敛**。
    ...
    - [x] A19a7d：用不含项目身份的等价构建 fixture 证明各 adapter 对同一构建语义产生相同 canonical projection，并用静态边界测试拒绝下游导入 adapter 私有字段；有限 held-out 项目必须先通过 BuildIR 验证才能进入迁移 DAG。
      ...
      - [x] A19a7d2：把同源 fixture 扩展到 Meson、multi-TU、archive/ranlib、多输入和 external dependency/toolchain 语义，并让有限 held-out 项目通过 BuildIR validator 后再进入迁移 DAG。
              ...
              - [ ] A19a7d2e3b2：实现不可伪造的 host interface transition 与最小状态机 `IR_PARTIAL -> CANDIDATE_PROJECT_VERIFIED -> IR_PROMOTED -> PROJECT_FINAL_VERIFIED -> COMPLETED`。逐 section closure receipt 必须深重开 DAG、全部 BuildIR、candidate/source CAS、固定 validator 版本和七类接口事实；candidate Cargo check/test/trace 只能运行在 quarantine generation，不得更新 final `CURRENT` 或 final project gate。canonical RustProjectIR v3 只允许宿主从完整 closure evidence 原子晋升；晋升后必须按新的 `verification_context_sha256` 与 `promotion_epoch` 重新生成并重跑 final integration/Cargo，禁止旧 Cargo evidence、ABA epoch 或 post-Cargo settlement 回写 IR 形成自引用。
                ...
                - [ ] A19a7d2e3b2b：实现七类真实宿主 section validator。每个 validator 必须从 deep-reopened C/BuildIR/DAG、candidate source 与编译器事实独立复算对应事实；任意 JSON、模型声明、固定 issuer 字符串或只有哈希一致均不得签发 closure。当前简单 Rust symbol regex、空 section 和 `unresolved-before-compiler` 不能用于关闭 public signature、type layout、global ownership 或 init/destroy。
                  ...
                  - [ ] A19a7d2e3b2b2：把 b2b1 parser 接到固定宿主 runner、原始输出 CAS 和完整 deep-reopen domain，补齐 Rust post-cfg public signature/layout witness、cfg/feature target matrix、global owner/access、init/destroy 顺序图与 C/Rust 对照器，再由七个独立 validator 签发可重算 receipt。parser 单测、Cargo build-finished、正确哈希或空事实均不能关闭本项。
                    ...
                    - [x] A19a7d2e3b2b2b：为 Clang AST 与 record-layout 增加固定隔离 runner、原始输出 CAS，并建立会重开 run contract、父/子 DAG、全部 BuildIR 及原 C 仓库绑定、candidate/source CAS、base v2 IR、quarantine generation、b2a receipt 和固定 toolchain/target 的公共 D0 validation domain。
                    - [ ] A19a7d2e3b2b2c：实现 Rust post-cfg typed/layout witness、cfg/feature target matrix、module/target ownership、global access 与 init/destroy 图，再由七个独立 C/Rust comparator/validator 对非空覆盖域签发可重算 section receipt；普通空数组不能关闭任何 section。
                      - [ ] A19a7d2e3b2b2c1：完成 compiler-backed Rust witness producer 子链；每个 witness 必须绑定 D0、b2a quarantine generation、raw compiler output、Cargo topology、实际 cfg/features/target/toolchain，且保持 candidate-only。
                        - [x] A19a7d2e3b2b2c1a：从 D0 固定的 b2a receipt 深重开 metadata/check raw CAS，重跑 Cargo metadata 与 compiler-artifact parser，生成可重算的 package→target ownership、实际 target/profile 和 active-feature topology witness。该 witness 只覆盖现有 `--all-targets --all-features` lane；featureful 项目、target 缺失/多余、package 身份歧义或 source/CAS 漂移均 blocked，不能签发 section closure。
                        - [x] A19a7d2e3b2b2c1b：以内容绑定的 `syn`/等价成熟 Rust parser 提取 pre-cfg module/item/signature/type/global/init 候选与全部 cfg/cfg_attr；macro 展开、解析歧义或 unsupported attribute 必须显式 blocked，禁止 regex 充当 Rust 语法权威。
                        - [ ] A19a7d2e3b2b2c1c：在固定 sandbox/toolchain 下为 bounded feature/target matrix 运行 post-cfg compiler witness，捕获 public signature、typed layout/offset、module ownership、global access 和 init/destroy 图的 raw/derived CAS；默认/all/no-default 等 lane 不完整时不得闭合。
                      - [ ] A19a7d2e3b2b2c2：实现七个相互独立、版本固定、从 D0 与 raw witness 重算的 C/Rust comparator/validator；每个 receipt 只关闭自身非空覆盖域，任一 missing/empty/stale/swapped fact 必须 fail closed。
                        - [ ] A19a7d2e3b2b2c2a：`public-signature` comparator/validator。
                        - [ ] A19a7d2e3b2b2c2b：`shared-type-layout` comparator/validator。
                        - [ ] A19a7d2e3b2b2c2c：`global-ownership` 与 access comparator/validator。
                        - [ ] A19a7d2e3b2b2c2d：`initialization-destruction` 顺序图 comparator/validator。
        ...
        A19a7d2e3b2b2b 有限阶段证据（2026-07-15）：Clang 工具链由独立 competition-profile host receipt 固定 binary/version/target/resource-dir，并为每个 BuildIR translation unit 生成独立 AST 与 record-layout 计划；Linux runner 只经无 shell 的 bubblewrap 执行，执行一旦启动就先把 stdout/stderr 写入按 plan/unit/gate/stream 隔离的不可变 raw CAS，再生成独立 derived CAS。record-layout 只能消费同 unit 的已重开 AST receipt，并只选择原 C 源绑定的具名完整记录；D0 在同一只读 SQLite snapshot 内重开 run contract、exact candidate set、父/子 DAG、每个 BuildIR、原 C 仓库、candidate/source CAS、base v2 IR、quarantine generation、b2a receipt、全部 Clang plan 与 live toolchain target。真实 WSL lane 安装并实测 bubblewrap 0.9.0 capability probe，随后在只读仓库/工具链挂载下完成真实 Clang AST→`Packet` layout，原始与派生证据均可重开；该 live lane 还发现并修复了空环境变量被 argv 校验误拒以及 `/workspace` 绝对源路径无法进入可移植 AST 位置证据两个通用缺陷。聚焦回归 Windows 48 项通过、1 项 live 条件跳过，WSL live 48/48；阶段全量 Windows 914 项通过、7 项平台条件跳过，WSL live 914/914、0 skip/failure/error，新增/修改生产 Python 文件均不超过 300 行。该阶段没有调用 provider/model，没有实现 Rust post-cfg witness、七类 comparator/validator、v3 晋升、独立 verifier 或真实 held-out Cargo/C oracle；因此只勾选 b2b2b，b2b2/b2b 及所有上层项继续未勾选，`section_closure=false`、`semantic_gate=false`、numerator 0。

        A19a7d2e3b2b2c1a 有限阶段证据（2026-07-15）：新增 candidate-only Rust Cargo topology witness。物化入口只接受 D0 引用并深重开 D0，由 D0 固定 run、candidate set、base v2 IR 与 b2a；随后从 b2a 重新读取 metadata/check raw CAS、重跑 Cargo metadata/compiler-artifact parser，并把 package→target ownership、实际 target/profile、active feature set、b2a verification context、quarantine generation 与 Cargo fact binding 写入可重算 CAS receipt。输出不含 workspace/host path；missing/extra/duplicate target、package 身份歧义、all-features observation 不一致、自洽篡改或当前 D0/b2a/CAS 漂移均 fail closed。只有 featureless package 的现有 `--all-targets --all-features` lane 可得到 topology `ready`；任何声明 feature 的项目继续以 `rust_cargo_feature_matrix_lanes_missing` blocked，不能把单 lane 解释成完整 cfg matrix。25 项聚焦回归通过；阶段全量 Windows 923 项通过、7 项平台条件跳过，WSL live 923/923、0 skip/failure/error；新增/修改生产 Python 文件均不超过 300 行。该阶段没有调用 provider/model，没有 pre-cfg `syn` producer、post-cfg typed/layout witness、七类 validator、v3 晋升或 held-out oracle，因此只勾选 c1a，c1/c2/b2b2c 及所有上层项继续未勾选，`section_closure=false`、`semantic_gate=false`、numerator 0。

        A19a7d2e3b2b2c1b 有限阶段证据（2026-07-15）：`c2r-translator` 新增锁定 `syn=2.0.118`、`quote=1.0.46` 的独立 stdin JSONL parser；源码常规解析上限为 4 MiB，流式输入硬上限为 64 MiB，单条语法事实为 64 KiB，聚合事实为 20,000 条，输出为 16 MiB。parser 不读取候选路径，仅由原始字节计算 source SHA-256，并提取 root/inline/external module、普通/trait/impl/foreign function signature、struct/enum/union/type/trait、const/static/foreign static、initializer/Drop 候选、每个语法位置的 attribute 与 macro invocation。`cfg` 原样保留；`cfg_attr`、derive/函数式 macro、未知 attribute、未解析外部 module、verbatim/parse ambiguity、非法 UTF-8、空源码或任一限额越界均显式 blocked。宿主 materializer 深重开 D0、b2a quarantine generation、base v2 IR candidate CAS 与 c1a Cargo topology；parser binary 由 repo-relative path/size/SHA-256 固定，每个候选的 stdout/stderr 先写入不可变 raw CAS，再把严格重算后的 witness 写入 derived CAS。receipt 重开会重新读取候选原始字节、父证据和 CAS，重新执行当前同一 binary，并逐字节比较 raw output 与逐字段比较 derived witness；只重写内外 JSON/hash、替换 binary、交换 candidate、删 blocker 或修改计数均不能通过。binary 当前只在限时/限输出/最小环境的独立进程中运行，receipt 明示 `sandboxed=false`；固定 Linux sandbox、macro expansion 后的 post-cfg typed/layout/target matrix 仍由 c1c 负责，c1b 不具备该权限。Rust 聚焦测试 5 项、宿主 parser/process/evidence 与源码布局聚焦测试 12 项均通过；Rust `--all-features` 已通过 267 项库测试、688 项 bounded test（133 项按既有规则 ignored）、4 项 integer conversion 与新增 5 项 witness test。Windows 项目级全量首轮 934 项中出现 1 项与本改动无关且单独复跑通过的既有并发路径瞬态；最终阶段全量 Windows 934 项通过、7 项平台条件跳过，WSL live 934/934、0 skip/failure/error。新增/修改生产 Rust/Python 文件均不超过 300 行。该阶段没有调用 provider/model，未实现 c1c、七类 comparator/validator、v3 晋升或 held-out oracle；因此只勾选 c1b，c1/c1c/c2/b2b2c 及所有上层项继续未勾选，`post_cfg=false`、`section_closure=false`、`semantic_gate=false`、numerator 0。
  ...
  - [ ] **A19a8：显式启用、强隔离的 Make dry-run 构建事实采集**。
    ...
    - [ ] A19a8e：把真实递归 Make 和构建期生成器扩展为可重开、可校验的 typed build events；在工作目录、输入、输出、执行顺序或沙箱身份不能无损绑定时继续 fail closed，禁止从文本位置猜测子目录或直接放行任意程序。
      - [ ] A19a8e1：解析并 hash 绑定递归 Make 的 enter/leave directory 事件，为每条子 Make 命令保留自己的 repository-relative working directory、父子调用边和目标作用域；目录栈不平衡、链接路径、跨仓库路径或并发输出无法确定归属时阻断。
      - [ ] A19a8e2：把“编译并链接 host tool -> 执行 tool 生成源码/头文件 -> 消费生成物”建模为 generated-node closure；只有 A19e8 沙箱允许执行内容绑定的 host tool，并绑定 argv、环境、输入、输出、退出状态和 raw streams。未建模前，compile+link 生成步骤和后续任意命令保持 `make_collection_output_unrepresentable`。
  ...
  - [ ] **A19c4：最大化 AI 项目推理、候选多样性与信息增益**。
    ...
    - [ ] A19c4b：同一高风险 unit 可在共享总预算内生成有明确策略标签的多样候选/接口提案，经 source SHA 去重后由相同 host gates 比较；reviewer/critic 只能提出结构化风险和修复假设，模型投票、自评分或多数意见不得产生 pass。
      ...
      - [ ] A19c4b2：让同一高风险 unit 在一个共享 token/时间预算内保留多个并行或顺序候选，按 source SHA 去重并在同一 cohort 上运行完全相同的 host gates；选择结果必须由 gate evidence 决定并可恢复重放。
        - [x] A19c4b2a：planner-bound 高风险 unit 在 translator 共享 attempt 上限内顺序生成最多两个唯一候选；候选文件按 attempt 隔离、按源码 SHA 去重，重复内容不占唯一候选数。宿主优先验证最新且尚未失败的候选；门禁失败后先回退到未验证候选，全部失败时仅按最新 host gate 通过数、失败数和源码 SHA 选择确定性 repair 基线。事实层、调度器、候选集和最低层验证入口共用同一内容绑定选择器，reviewer 自评不能改变结果。
        - [ ] A19c4b2b：为同一 unit 的每个唯一候选建立不可变 candidate-evaluation cohort，在不因首个失败而提前结束比较的前提下完整运行同一组 compile/semantic/final host gates；持久化选择 receipt，并在多个全通过候选之间按 host 风险事实确定性择优。当前 b2a 的顺序失败回退提高成功率，但不能替代该全候选同 cohort 比较，因此 b2 与 b2b 保持未完成。

        A19c4b2a 有限阶段证据（2026-07-16）：新增通用、内容绑定的候选池选择器，planner assignment 仅作为结构风险事实决定唯一候选目标 2，不读取项目、函数、路径或测试身份。两个独立 translator attempt 绑定轮换策略；重复源码仍保存独立 attempt provenance，但按完整源码 SHA 只计一个唯一候选。最新未失败候选先进入现有相同 reviewer/compile/semantic/final 链，失败后 unit 转为可恢复状态并切换未验证候选；当全部失败时，修复基线只由最新 host verifier records 的通过数、失败数和 SHA 稳定排序。Windows 项目 harness 全量 1170 项通过、24 项平台条件跳过；WSL 加载 `huawei-competition-ubuntu-24.04` 仓库配置后，候选池、调度、候选集、门禁、恢复与源码布局 29 项通过。生产 harness 对已知项目身份和历史密钥片段的精确扫描零命中，新增/修改 Python 文件均不超过 300 行。本阶段没有调用 provider/model，没有运行比赛平台，也没有建立每候选不可变 evaluation cohort 或比较多个完整通过候选；因此 b2b、b2 及父项继续未完成，`semantic_gate=false`、numerator 0。
      ...
      - [ ] A19d3d2：从当前 flat library module 投影扩展到经验证的嵌套 module tree、多 crate/bin/example、完整 target/feature/cfg、初始化/析构和 native link 配置，并在真实多文件 Cargo build/test 与 project-final 全门禁通过后才关闭 A19d3。
        - [ ] **A19d3d2a：把目标 C 仓库 `tests/` 下的测试作为一等 test target 完整迁移，而不是仅因其出现在 `compile_commands.json` 就当作普通 library module 翻译。** host 必须从 BuildIR 及 CMake/CTest/Meson/Make 测试元数据中发现并内容绑定每个测试目标、C 测试源码、runner、fixture/data、工作目录、环境、参数、expected exit/signal 与 timeout；RustProjectIR 再确定性投影为 `#[test]`、`tests/*.rs` 或显式 Rust test binary，并保持目标依赖和执行语义。每个发现的 C test target 必须一一对应 Rust test target，或者产生带证据的 fail-closed refusal；验收必须同时运行原 C suite oracle 与 Rust suite，比较测试集合与逐项结果，并拒绝 `cargo test` 为 0 项、漏测、静默删除断言或只编译不执行。至少用两个 held-out C 项目证明不存在 `tests/` 路径、测试名称或项目身份硬编码。
          ...
          - [ ] A19d3d2a2：把 BuildIR 中经内容绑定的编译变体、链接命令、动态导出/native library、测试 runner 和资源限制投影进 C2Rust/Cargo 验收。未知项目遇到 computed goto 等工具不支持特性时，只能从项目自身元数据发现并先通过原生 oracle 的可移植构建变体；Rust 侧必须继承等价链接语义和显式资源配置，并把原 C/转换后测试命令、逐项结果和差异写入正式报告。必须区分“所有 translated main 均完成 Cargo 编译”和“只有原构建系统声明的测试/验证场景才是运行门禁”，从 BuildIR 绑定每个测试目标的 argv、stdin、fixture、工作目录、资源限制与期望退出/输出合同；需要参数的普通 CLI 不能被无参数假失败，也不能未经有效场景执行就冒充语义通过。Lua 压测已证明手工选择 `LUA_USE_JUMPTABLE=0`、补 `--export-dynamic`、恢复默认栈后官方全套可通过，Expat 诊断已证明手工补有效 XML/benchmark 参数可通过，但这些手工步骤都不计本项完成证据，生产代码也禁止读取 Lua、Expat 或任何项目身份。
            ...
            - [x] A19d3d2a2c：`c2rust-baseline` 新增身份中立、内容绑定的声明式执行场景合同。合同必须精确覆盖全部生成 wrapper；每个 wrapper 只能显式标记 `compile_only`，或声明一个或多个具有唯一 ID 的场景，并严格绑定 argv、UTF-8 stdin、受限环境变量、仓库内工作目录、expected exit 和 timeout。原始合同与规范化合同都进入不可变 CAS，参与 run identity；报告 schema v2 绑定场景输入、stdin artifact、执行计划、实际退出码和逐项状态，重开时会逐项交叉验证规范化合同并对合同、输入、计划或 CAS 漂移 fail closed。未提供合同时保留旧兼容路径，但显式合同模式只执行声明场景，普通 CLI 不再因无参数启动而假失败，也不能以 `compile_only` 冒充语义通过。聚焦门禁在 Windows 与 WSL 均为 39/39；完整 `project_migration` 回归在 Windows 为 1019 项通过、11 项平台条件跳过，在 WSL 为 1019 项通过、6 项平台条件跳过，均零失败。生产代码不读取项目名称、测试名称或 fixture 身份。Expat 原始 compile database（36 条编译记录、29 个唯一 C 源文件）仍因 7 组同源多配置在 Cargo 前正确阻断，报告 SHA-256 `09f21f2b6d16ce60367cc8dcb1ec65785eab2f8be2659fb70e3b0b5de1abc588`；仅为隔离验证本子能力而构造的测试专用单变体投影完成 29 TU、6 wrapper、6 个声明场景且实际退出码全为 0，报告 SHA-256 `dd7f2194702ee1a45dc4601beb2e51753adff2907635a0bbf8cfebea63bbb2f4`。后者不是生产变体选择规则、不是 held-out 或 Expat 全项目通过；两份报告都固定 `semantic_gate=false`、numerator 0，且尚未证明构建系统测试清单一一对应，因此不关闭 A19d3d2a2a、A19d3d2a2、A19d3d2a 或其父项。
            - [x] A19d3d2a2d：BuildIR 到 AI ContextPack 的同源变体/目标归属投影已闭合为身份中立、可重算的阶段能力。每个 `unit_id` 必须一一绑定 object target，variant index/count 必须覆盖同源完整集合；目标归属沿 `object -> archive/link -> downstream link` 依赖图求传递闭包，缺失依赖、重复 object owner、输出不一致、孤立目标、基数漂移或可达环均 fail closed。CIndex 严格重验该上下文，ContextPack 将共享 target fact 去重，并为每个 unit 单独绑定 target-membership fact，避免把同一源码的不同配置任意选一。链接闭包同时通用支持实际存在且内容绑定的版本化 `.so.N`/非传统后缀输入；目录证据仅允许根内相对、非悬空、无环的符号链接，仍拒绝单文件链接输入、绝对链接和路径逃逸。本轮原始 Expat `R_2_8_2` 开发回归在 WSL 上得到 discovery/closure/link `ready`、closure verification `verified`、36 个 BuildIR unit、43 个 object/link target、7 组同源双变体和 0 blocker；聚焦回归 Windows 62 项通过、9 项因链接权限条件跳过，WSL 62/62；阶段全量 Windows 1036 项通过、20 项平台条件跳过，WSL 1036 项通过、6 项平台条件跳过。所有本轮新增/修改 Python 文件不超过 300 行，生产代码不读取项目、target 或测试身份。该阶段只证明发现、闭包、BuildIR 和模型输入归属，不生成 target-scoped Rust namespace，不运行 C2Rust/Cargo/test oracle；所以 A19d3d2a2a 及所有父项继续未勾选，`semantic_gate=false`、numerator 0。
            - [x] A19d3d2a2e：迁移图的外部函数/全局可见性已改为 BuildIR target-domain 驱动，并保持无链接事实时的保守兼容。只有存在真实 archive/link consumer 证据时才按 `object -> consumer` 交集隔离符号；只有 object 编译事实时不臆造最终程序边界，仍允许跨 TU 解析。不同目标的同名定义不再互相制造伪歧义，汇入同一目标时仍 fail closed。CIndex 同时从内容绑定的源码和去重头文件快照识别任意函数式宏生成的定义，将其保留在上下文并标成显式 parser boundary，而不伪造普通函数节点。原始 Expat `R_2_8_2` competition-profile 开发回归生成完整计划：36 个 BuildIR unit、43 个 target、7 组同源双变体、489 个函数节点和 407 个 SCC，`ambiguous_external_definition` 从上一阶段的 130 个 SCC 降为 0；剩余 155 个 parser blocker 均保留分类证据。模型与 Cargo 均未启动，`semantic_gate=false`、numerator 0。聚焦门禁 Windows/WSL 均 35/35；阶段全量 Windows 1039 项通过、20 项平台条件跳过，WSL 1039 项通过、6 项平台条件跳过；触及的生产/测试 Python 文件均不超过 300 行。该项只关闭 target-scoped C 图与模型输入消歧，不关闭 A19d3d2a2a 或任何整项目翻译父项。
            - [x] A19d3d2a2f：新增 BuildIR-bound C2Rust schema v3 路径。先重开完整 BuildIR/compile-database 内容绑定，再按 `unit_id` 为每条编译记录创建独立输入数据库、输出目录和不可变生成快照，彻底禁止同源 variant 通过 `--overwrite-existing` 相互覆盖；随后从 BuildIR 原始 `ordered_inputs` 递归生成 terminal-target plan，保留 archive 层级、成员顺序、重复 occurrence、链接参数、native/system 边界和 unit fan-out。不同 target 的同名 export 可并存，同一 target closure 内的重复全局定义必须证据化拒绝，不得按路径选 winner。本切片完成后整个 baseline 仍应以 `c2rust_target_cargo_assembly_pending` 阻断，不得用逐 unit 转译成功冒充 Cargo 或语义通过。
              - [x] A19d3d2a2f1：完成 schema v3 的 BuildIR/compile-database 全量重开、entry index/SHA 与解析事实复算、安全 unit key、每 unit 单记录数据库、独立 fresh output、不可变生成快照及 terminal-target plan。计划按原始 `ordered_inputs` 递归保留 target occurrence、archive 语义、重复成员、link arguments、object owner、unit fan-out 和 target-owned external/native boundary；缺 owner、未知依赖、环、输出冲突或不可表达顺序均 fail closed。Windows 与 WSL 的 C2Rust baseline 聚焦回归各 55/55 通过，源码布局门通过，所有新增/修改 Python 文件不超过 300 行。真实 Expat `R_2_8_2` development regression 在 WSL 用原始 36 条编译记录完成 36/36 个真实 C2Rust 进程和 36/36 个独立快照，覆盖 29 个唯一 C 源文件、6 个 terminal target 和 99 个 unit occurrence；当前可重开 report SHA-256 为 `515576d6e134e0cb333a2af72b87988b2a29223b6e69b2ce539d8017c2848457`，唯一 blocker 为 `c2rust_target_cargo_assembly_pending`。本证据未运行 AI、Cargo 或项目测试，`semantic_gate=false`、numerator 为 0；它不是 Expat 全项目翻译通过。
              - [x] A19d3d2a2f2：完成 target-scoped export inventory 及重开门。运行时从每个 unit 的内容绑定生成快照解析外部函数/全局定义，按 terminal target occurrence closure 汇总并写入不可变 CAS；报告重开会重新读取 target plan、snapshot manifest 和每个 generated-file CAS，逐 unit 重算后与 inventory 字节级比较。不同 terminal target 的同名 export 可并存；同一 closure 内无论跨 unit 还是同一 unit 多文件重复定义，均生成稳定 `c2rust_target_export_conflict` 并在 Cargo 前拒绝，绝不按源码路径或模块顺序选择 winner。聚焦回归在 Windows/WSL 各 55/55 通过；真实 Expat 报告重开得到 380 个 unit-level export、764 个 target-occurrence export、0 冲突，export inventory CAS SHA-256 为 `3f04656e6dbaf6edb116a182f589b201cb0196c7fccd2aa69cbfc7891ceb4c2f`。该门只证明 export 冲突分类，不运行 Cargo 或提高语义 numerator。
            - [ ] A19d3d2a2g：把 SCC 到 BuildIR 的身份桥写入 migration DAG/integration manifest，逐组绑定 `source_unit_ids`、object/owner/reachable target scope 并在重开时从 BuildIR 重算。新增向后兼容的 RustProjectIR v3：以 `workspace -> packages -> targets -> modules` 为一等拓扑，用 target namespace 和 candidate identity 共同生成 module identity，允许相同源码内容属于不同 variant；按 archive/link 产品类型重建多 package Cargo workspace，并用 `cargo metadata --locked --offline`、实际编译目标和 linker evidence 证明 package/target 集合完全一致。产品类型、输入顺序、native consumer、入口 `main` 或命名空间冲突无法证明时必须阻断，不能回退到 flat single-crate。
              - [x] A19d3d2a2g1：完成宿主派生的 `scc-build-target-scope-v1`。migration graph 先从 SCC node membership 派生去重 `source_unit_ids`，再绑定每个 unit 的 variant、唯一 object owner、完整 reachable archive/link 集合、terminal target 集合和共享 target domain；portfolio DAG 将 scope 纳入 group content SHA，integration manifest 同时绑定最终 ledger group SHA、原 migration-graph artifact 和逐组 scope SHA，避免复制完整 scope。run contract 会交叉核对 ledger group，RustProjectIR reopener 则重开 canonical migration graph 和全部 BuildIR，逐 SCC 重算并逐字节比较；cohort DAG 只能从父 manifest 精确裁剪 scope binding。未知/重复 node 或 unit、owner 冲突、target 环、不相交 domain、group/graph/order/scope/BuildIR 漂移均 fail closed；legacy 无 scope 清单仍只兼容 v2，不能取得 v3 权限。阶段有限证据：Windows `test_project_migration*.py` 1065 项通过、20 项平台条件跳过；WSL 同组 1065 项通过、6 项平台条件跳过；文档镜像与 300 行源码边界 5/5。原始 Expat `R_2_8_2` competition-profile development regression 重新生成 36 个 BuildIR unit、407 个 SCC scope（404 个 `target-bound`、3 个 `shared-target-domain`）和 6 个 terminal target，scope-set SHA-256 为 `d0aa78cf98106490f1c1eccb35b825cea8d3919238391126d33d6b18435d78e8`，integration manifest SHA-256 为 `64e3b5c35dd4d0981fe7924570cef0c002f46e0bcd5d35ba996763bdb0e008fa`；hash-only scope binding 将该清单从 749984 字节降到 138749 字节。该运行只证明计划和身份重开，未启动模型/Cargo/test oracle，`semantic_gate=false`、numerator 0；g2-g4、g 和所有整项目父项继续未勾选。
              - [x] A19d3d2a2g2：完成向后兼容且 fail-closed 的 RustProjectIR v3。通用验证/重开入口仅按 `schema_version` 分派，v2 单 crate 契约保持不变；v3 以 `workspace -> packages -> targets -> modules` 为一等拓扑，不再携带重复的 `crate` 权威。宿主只从 BuildIR `archive`、普通 linker-driver `link`、`-shared/--shared` 或 `/DLL` 事实分类 static library、executable 和 shared library，不读取项目名、输出扩展名或测试身份；`-r/--relocatable`、object-only、未知产品、缺失/不相交 scope、same-source variant 混合及 namespace 冲突都形成结构化 blocker。module identity 同时哈希 target namespace、SCC group、candidate SHA 和完整 source-unit variant scope；相同候选内容可以在有闭包证据的不同 target namespace 中取得不同 module identity。target 保留重复且有序的 BuildIR input occurrence 与 link argument，package dependency 只从可证明的非 object target edge 派生。v3 reopener 会重开 DAG、显式 migration graph、全部 BuildIR、candidate source 和 compilation-facts binding，从 BuildIR 重算 scope 与完整 workspace/package/target/module projection，并逐字段拒绝 artifact SHA、产品集合、module scope、输入顺序或拓扑漂移；候选 descriptor SHA 也改为在 IR 创建前与实际源码字节重算比较。所有新增/拆分 Python 文件均不超过 300 行。阶段证据：Windows 全量 `test_project_migration*.py` 1094 项通过、20 项平台条件跳过；WSL 同组 1094 项通过、6 项平台条件跳过；落盘 v3 ready/blocked binding 测试 6/6。真实 Expat `R_2_8_2` 的 36-unit/407-SCC BuildIR+scope 回放，在仅构造 candidate reference、未提供翻译语义的边界下得到 7 个 package/target（1 个 shared library、6 个 executable）、407 个 target-scoped module、0 blocker，拓扑 SHA-256 为 `4452878618cf2538afbd7e3d56851d895c366c24dbdebf26a3e0f7c4a2358012`。该阶段没有启动模型、Cargo 或测试 oracle；生产 Cargo 路径在 g3 完成前仍保持 v2，`semantic_gate=false`、numerator 0，g3-g4、g 和全部整项目父项继续未勾选。
               - [x] A19d3d2a2g3：从 v3 确定性生成多 package Cargo workspace，保留 archive/link 产品类型、依赖方向、输入 occurrence 顺序、入口点、crate type 和 target-owned export namespace；无法证明唯一 `main`、crate/module 名称无碰撞或 native/system link 参数可表达时 fail closed，禁止回退到 flat single-crate。生产 generation domain 已在完整 target-scoped competition 输入上选择 v3，development 仍只保留 v2 兼容；staticlib、cdylib、bin 和双 package workspace 有确定性重开回归，双 package 产物实际通过 locked/offline Cargo metadata 与 all-target check。g4 的独立生产 verifier/linker witness 仍未完成，所以本项只关闭生成，不声明结构或语义终验。
              - [ ] A19d3d2a2g4：增加独立重开门，执行 `cargo metadata --locked --offline`、all-target 实际编译和 linker trace/ELF/archive 重开；metadata package/target 集合、实际 rustc target、链接输入顺序和 RustProjectIR v3 必须完全一致。该门通过仍只关闭结构重建，不得在 ProjectTestInventory、C/Rust oracle diff 与 project-final 未通过时提高 semantic numerator。
                - [x] A19d3d2a2g4a：生产 project Cargo verifier 已对 v3 generation 固定执行 locked/offline metadata 和 `cargo build --workspace --all-targets --all-features --offline --locked --message-format=json --jobs 1`。独立 witness 从原始 CAS 重跑 metadata/compiler-artifact parser，把 package/target/default/workspace 集合及实际 rustc target 与 v3 预期逐项比较，并拒绝 fresh、缺失、额外、重复或漂移目标。最终 bin/cdylib/rlib/staticlib 在沙箱清理前通过预先锚定的 target 目录读取，写入不可变 CAS，再按 ELF executable/PIE/shared-object 或 Unix archive/ET_REL member 类型、ABI 和成员顺序深重开；target 根替换、链接、越界路径、产物缺失、类型伪造或 CAS 漂移均 fail closed。Cargo stdout/stderr 改由 guest 不可见的宿主私有控制目录捕获，整条 raw/parser 上限统一为有界 16 MiB。ready topology reference 已绑定 completion receipt v3；该收据继续固定 `semantic_gate=false`、numerator 0。
                - [x] A19d3d2a2g4b：linker trace 已从全局路径清单升级为 package/target 归属证据，并与同一次 Cargo compiler-artifact source SHA 强绑定。持久化工件只保留 guest path/member SHA 和每 target 原始顺序，不泄露宿主路径；对每个 bin/cdylib，witness 将 RustProjectIR 中可表达的 package dependency target occurrence 顺序与实际 linker 输入逐项匹配，反序、缺失、冒名 target、额外 workspace dependency、路径歧义或跨 capture 拼接均 blocked。通用测试不读取项目/测试身份；Windows 新增 target-trace/order 聚焦 9/9 通过，真实 WSL bwrap+Cargo+rustc live 回归对 staticlib 与 bin 共 2/2 通过并产出 `ready` 可重开收据。阶段有限全量 Windows 共 1333 项，1303 项通过、30 项平台条件跳过；WSL 本地模拟同组 1321 项通过、12 项平台条件跳过，均为 0 failure/error。该 WSL 为 Ubuntu 24.04.3 与 Rust/Cargo 1.95，接近但不等于画像要求的 Ubuntu 24.04.4 与 Rust/Cargo 1.96，因此不声明 `competition-exact`。
                - [ ] A19d3d2a2g4c：闭合从 BuildIR 原始 input occurrence 到 Rust 最终产物的逐 occurrence 可验证链。g4c1 已关闭对象/模块顺序承诺与最终产物定位这一结构子边界；g4c2 仍须证明所有输入类别在实际 rustc/linker argv/trace 中的统一物理顺序。因此 g4c、g4 与结构重建父项继续未勾选。
                  - [x] A19d3d2a2g4c1：RustProjectIR v3 已为每个 BuildIR 输入 occurrence 显式绑定 `ordinal`、`occurrence_id`、`role`、`dependency_target_id`、`object_target_id`、`source_unit_id`、`module_id` 与 `binding_sha256`，并在重开时从 BuildIR 严格重算；未知归属、重复身份或顺序漂移均 fail closed。Cargo 生成根按对象 occurrence 派生模块顺序，生产 verifier 捕获并内容寻址重开真实 rustc dep-info 以核对编译源码闭包；每个最终 bin/cdylib/rlib/staticlib 还嵌入固定大小的 occurrence commitment（数量与完整顺序 SHA-256）。ELF 必须唯一携带该 commitment，archive 则必须把它唯一定位到排除 `lib.rmeta` 的代码对象成员，并绑定成员 ordinal、名称 SHA 与 payload SHA。该项证明对象/模块 occurrence 闭包、生成顺序承诺及最终代码成员定位，不证明跨 object/package-dependency/native/system 类别的实际 linker 物理交错，不提供语义通过，也不是 `competition-exact`。本阶段有限回归：Windows 项目迁移全量 1356 项中 1326 通过、30 项平台条件跳过；WSL 本地比赛模拟同组 1344 通过、12 项显式环境条件跳过，两端均零失败/错误。
                  - [ ] A19d3d2a2g4c2：捕获并重开统一的实际 rustc/linker argv 与 linker/archive trace，把 object、package dependency、允许的 native/system 输入及原始 archive member occurrence 逐项映射到同一序列，保留重复 occurrence 和跨类别完整相对物理顺序。dep-info、生成根顺序或最终 commitment 都不能替代该证据；native/system 参数无法唯一归属、trace 不完整或任一顺序不一致时必须 blocked。只有该项完成后，g4c 与 g4 结构父项才具备关闭条件；ProjectTestInventory、C/Rust oracle diff 与 project-final 未通过前仍不得提高 semantic numerator。
                    - [x] A19d3d2a2g4c2a1：已完成并交付源端跨类别顺序权威。C linker 与 GNU/MSVC archive 命令在按类别分栏前生成固定 schema 的 `ordered_link_occurrences`，逐项保存连续 occurrence ordinal、删除输出参数后的语义 argument index/span、类别和该类别 reference ordinal；input、search-root、system argument、external native library 的重复与完整相对顺序都保留。新鲜 Make 投影固定生成 BuildIR v3；BuildIR 只携带 input/binding/dependency 或 external-dependency 内容身份，不复制原始宿主路径/参数，并严格重算参数 span 不重叠、四个分类数组双射闭包、独立 search-root 摘要、input binding、native 原参数位置和 system 类别顺序。link closure v2 与 BuildIR extractor v3 共同承载当前权威，运行时 BuildIR/Make reopener 只接受 schema v3；历史 parser v2 Make 报告仅在宿主按当前语法重新解析且命令逐字段一致时兼容，旧 BuildIR v2 不能直接通过运行时验证。未内容绑定的 forwarded response/linker-script 及未建模 archive 参数全部 fail closed。代码不读取项目或测试身份。最终仓库级门禁为 Windows 3250 项中 3215 通过、35 项平台条件跳过，WSL 本地比赛模拟 3250 项中 3230 通过、20 项平台条件跳过，两端均零失败/错误；实现提交 `7febc284784da229b68d53c2b991a1b72bebe2c7` 已推送且 `codex/target-scoped-rust-project`、`codex/flashdb-rust-skeleton` 两个远端分支 SHA 均已核对。该项只证明 C/BuildIR 源端序列，不证明 Rust 端实际物理链接顺序，因此不关闭 g4c2、g4c、g4 或语义父项。
                    - [x] A19d3d2a2g4c2a2：已把 BuildIR 的统一 `ordered_link_occurrences` 投影为内容哈希绑定、可重算且可重开的 RustProjectIR v3 分层 `link_expectation`。每个 occurrence 保留 ordinal、argument span、consumer、内容身份和跨类别顺序，并显式区分 `module-dep-info-commitment`、`cargo-package-product`、`native-link-input`、`system-link-argument` 四层。重复 object occurrence 保留不同 occurrence identity 与顺序，但映射到同一 object/source/module 逻辑所有者，不冒充重复的 Rust 物理 linker 输入；重复 native/system occurrence 可保留，同一 Cargo package target 重复出现则在缺少物理展开策略时 fail closed。BuildIR 以不含宿主路径的摘要保留 response-file presence，任意非空 response-file、forwarded `@` response、search-root、repository positional native input、group/as-needed/whole-archive control、`-Bstatic/-Bdynamic` 切换或严格 system allowlist 之外的参数均拒绝；package linker witness 还核对依赖出现基数，native requirement identity 按实际 name/format 生成。仓库级全量 Windows 3260 项中 3225 通过、35 项平台条件跳过；WSL 本地比赛模拟 3260 项中 3240 通过、20 项平台条件跳过，两端均零失败/错误。该项只关闭可重算期望层，实际 rustc/cc/ld argv 与 linker/archive trace 仍由 g4c2b 证明，因此 g4c2、g4c、g4 与语义父项保持未勾选，`semantic_gate=false`、numerator 0；当前证据仍是 `wsl-local-simulation`，不是 `competition-exact`。
                    - [ ] A19d3d2a2g4c2b：用受信、资源有界且可重开的探针捕获实际 rustc/cc/ld argv 与 linker/archive trace，并按表示层逐项核对 RustProjectIR v3 期望。module 用真实 dep-info 与最终 commitment 证明，package/native/system 用实际 argv/trace 证明；隐式 toolchain 输入必须与项目输入隔离。缺项、额外项、重复丢失、顺序漂移、response-file 展开不完整、路径歧义或 capture source SHA 不一致均 blocked。
             - [ ] A19d3d2a2h：新增 `ProjectTestInventory v1` 及 `project-test-mapping`/`project-test-oracle-diff`。CTest 只消费内容绑定的 `ctest --show-only=json-v1`，Meson 消费 `intro-tests.json`；Make 的完整目标只接受 Automake `TESTS/check_PROGRAMS`、项目自有显式清单或宿主验证的显式 dry-run target，target 选择必须绑定构建元数据或经宿主验证的 AI 提案，不能只按名称猜测。Kconfig/selftests 等必须有独立适配证据；禁止按 `tests/` 路径、`test/check` 名称或项目身份猜测测试集合。每个源测试身份必须唯一映射到 Rust test target/selector，并用同一规范化 argv/stdin/env/cwd/fixture/resource 合同分别运行 C 与 Rust；遗漏、额外、合并、zero-test、compile-only、silent skip、exit/signal/stdout/stderr 差异均阻断 Q4-Q8。
               - [x] A19d3d2a2h1：完成 CMake/CTest 首个生产适配。只执行沙箱化 `ctest --show-only=json-v1`，将 command[0] 精确绑定到一个 materialized BuildIR link target；完成期重新采集 CTest、重开 BuildIR/清单并重算全部记录，新增/删除/属性/二进制内容漂移均 fail closed。每个 inventory 声明的测试 executable 必须唯一映射到 RustProjectIR 目标；普通应用/CLI executable 由 all-target Cargo 门编译，但不再被误当成测试或仅因未出现在测试清单而阻断。未支持的 verdict-changing CTest property 不猜测。
               - [x] A19d3d2a2h2：完成 direct-executable 双沙箱 oracle。argv/env 使用结构化 literal/repo-path/path-template；宿主建立一次最小只读输入快照，目录、文件、权限、排除策略与显式输入都进入哈希。C 与 Rust 使用独立 bubblewrap runtime，Rust 侧不可见 C executable/source/state/output，比较由宿主执行；原 C baseline 必须 exit 0，exit/signal/stdout/stderr、timeout 或 oversized output 差异均不能通过。
               - [x] A19d3d2a2h3：项目逻辑 mismatch 已按 test -> source target -> Rust target/module -> unit 归因并原子回到 AI repair。`run-to-completion` 会调度 repairer/reviewer，重新执行 candidate compile/oracle/negative/unsafe-alias/ABI/final/promotion，再回到项目 oracle。完整 oracle 为内容寻址 artifact，host gate 与 completion receipt 绑定其 SHA，完成前重开 inventory/mapping/oracle/raw observation/gate summary/SQLite record；漂移不得发布 completed。
               - [x] A19d3d2a2h4a：完成受限、身份中立的 Make 显式 `test` 配方 inventory 历史兼容层。宿主在只读、无网络、带 RLIMIT 的 Linux bubblewrap 中执行固定 `make -n --no-builtin-rules --no-builtin-variables test`；输出必须是无管道、重定向、替换或 shell 控制的直接 argv。仅固定 `echo/printf` 展示命令可忽略，Python、自定义 runner、编译命令及其它未映射命令全部 fail closed；测试 executable 必须精确绑定 materialized BuildIR link output。inventory adapter、模型安全 test contract、完成期重新采集和逐字段重算共用同一 `make-dry-run-v1` 身份。Windows 聚焦 28/28、全量 1184 项通过且 25 项平台条件跳过；WSL competition-profile/live 22/22、CTest 环境修复相关 16/16 和 sandbox argv 4/4 通过。固定提交 FlashDB 的临时副本原生构建后，真实 bwrap 采集得到 2 个测试、2 个 target binding、0 blocker。一次原 profile WSL 全量仍因华为 sparse crates 镜像返回 HTTP 502 而在两个 Cargo setup class 阻断；改用已有全局离线缓存的同 7 项通过，但不计 competition-profile 通过。2026-07-17 安全复核确认 GNU Make `-n` 仍可能执行 `+`/递归/Makefile-remake 路径，`info` 等展开也能伪造 stdout；因此该项只保留旧工件重开兼容，不能再作为新计划测试清单权威。h4f 已用零子进程的宿主静态 direct-leaf 通道替代新计划入口；递归 Make 和 custom runner 仍未完成，因此不关闭 h4、Q4-Q8 或整项目语义验收。
               - [x] A19d3d2a2h4b：完成内容绑定的 Automake 目标选择历史兼容层。受限扫描全部 `Makefile.am`，绑定 path/SHA/size 与实际出现的 `TESTS/check_PROGRAMS`；根 manifest 加任一测试声明才选择固定 `make -n --no-builtin-rules --no-builtin-variables check`，否则保留 h4a 的固定 `test`。完成期重新派生 target binding，manifest 漂移 fail closed。混合 CMake/Make 只从 BuildIR raw provenance、输出路径和与 BuildIR metadata 同 SHA/大小的 compile database 选择唯一 adapter；无证据或双重证据均阻断。Windows 相关组 48 项通过且 1 项 Linux 条件跳过，WSL 20/20 与真实 bubblewrap `make check` inventory 通过。代码不读取项目名或测试身份；受 h4a 同一 `make -n` 权威缺陷影响，该路径仅保留旧工件重开，新计划必须走 h4f 的 AI 候选/宿主静态通道。递归 Make 和 custom runner 仍未完成。
               - [x] A19d3d2a2h4c：完成有界、身份中立的 Meson 项目测试清单。宿主只在只读、无网络、带资源上限的 Linux bubblewrap 中执行固定 `meson introspect --tests`，并绑定 compile database、Meson 构建文件快照、`intro-tests.json`、工具与沙箱身份。仅接受可唯一映射到 materialized BuildIR direct-link target 的直接 executable 测试；zero-test、过期 `meson-info` 元数据，以及 protocol、runner、fixture 或资源合同超出当前表达能力时均 fail closed。完成期会重新采集并重开全部依赖；代码不读取项目名、测试名或固定路径。Windows Meson 聚焦组 18 项通过、1 项 POSIX 条件跳过；该子项不证明任意 Meson runner 或整项目语义通过。
               - [x] A19d3d2a2h4d：完成 Make 直接测试命令的受限单文件 stdin 合同。只接受命令末尾、空白分隔的一个 `< repo-file`；输入必须是仓库内非链接普通文件且不超过 1 MiB，并进入最小只读输入快照。模型安全合同升级为 v2，仅暴露 `repo-path` 形状与路径且不暴露内容；旧 v1 无 stdin 合同仍可重开。C baseline 与 Rust replay 在独立 bubblewrap 中接收完全相同的原始字节，执行期 invocation 绑定 stdin SHA-256/长度与 snapshot SHA；落盘 oracle evidence v3 保存可重算但不泄露参数、环境和 cwd 原值的 invocation commitment，完成期逐 case 交叉核对两侧 commitment、快照文件 SHA/长度与 stdin，旧 v2 evidence 仅在 inventory 无 stdin 时兼容。stdout 相同但 stdin 或 invocation commitment 不同均不能通过。未加引号 `~`、管道、输出重定向、命令替换、多重或粘连 `<`、here-doc 仍 fail closed，`/workspace` 仅按 guest-root 边界转换，参数上限与执行器同为 128。最终 Windows 聚焦回归 29/29、项目测试组 86 项中 80 通过且 6 项平台条件跳过、项目迁移全量 1261 项中 1235 通过且 26 项平台条件跳过；WSL 项目迁移全量 1261 项中 1249 通过且 12 项显式环境条件跳过，另有真实 C/Rust 双沙箱二进制 stdin oracle 1/1 通过，全部零失败。两轮独立只读复核确认最初 6 个风险及 invocation 重开复现均已关闭。生产代码不读取项目或测试身份；本项不证明 runner、CTest/Meson 自定义 stdin 或完整 fixture/resource 支持。
               - [x] A19d3d2a2h4e：对当前已支持的 direct-executable adapter 路径补齐 `project-test-completeness-v1`。宿主先从 inventory 与 RustProjectIR v3 重新派生 canonical mapping，再要求每个 inventory test id 恰好出现一次、每个测试源 target 至少绑定一个 case、每个源 target 唯一绑定一个 Rust executable target，且每个 target 的 test-id/executable-path 分组逐项相等；zero-case mapping、遗漏、额外、重复 case、遗漏/额外 target、一个 Rust target 合并多个源 target、一个源 target 拆成多个 mapping、跨 target 交换 case 或字段级 mapping 漂移均 fail closed。工件固定 `silent_skip_count=0`、`required_test_omission_count=0` 等可重算计数，并在完成期从落盘 inventory/mapping 重新打开；oracle evidence 升级为 v4，把 completeness SHA 同时绑定到候选 workspace、逐项证据、observation 和总 oracle hash。生成的 Cargo bin/lib 目标显式设置 `test=false`、`bench=false`，lib 另设置 `doctest=false`，避免 Cargo 隐式测试 harness 绕过显式 C/Rust 场景映射。旧 v2/v3 stdin case evidence 的独立重开入口保持原边界，只有新 v4 才要求 completeness。完整性专项 11/11、相关聚焦门禁 36/36；最终 Windows 项目迁移全量 1273 项中 1247 通过且 26 项平台条件跳过，WSL 全量 1273 项中 1261 通过且 12 项显式环境条件跳过，WSL 真实 C/Rust 双沙箱 oracle 4/4，全部零失败。生产代码不读取项目名、测试名、目录名或固定 fixture。该子项只证明“已发现清单”的双向全等，不证明任意 target、递归 Make、custom runner、Kconfig、完整 fixture/resource、额外显式 Rust selector/`#[test]` 发现或未知项目测试元数据本身完整，因此 Q4/Q7 与 h4 继续未勾选。
               - [x] A19d3d2a2h4f：完成 AI 提议、宿主静态验证的显式 Make target 最小通道。外层 OpenCode/AI 只提交 canonical JSON 候选，工件绑定 provider/model 标签及 prompt/response SHA，`prepare-make-test-target-proposal` 由宿主生成可直接复用的路径/SHA/大小参数；AI target 只是选择键，不是执行或语义权威。宿主按 GNUmakefile/makefile/Makefile 优先级选择真实入口，建立有界、仓库内、无链接的字面 include 闭包，绑定每个 manifest 的路径/SHA/大小，并拒绝缺失、重复、循环、动态、绝对或通配 include、条件/define/eval/shell/file/guile/info/warning/error/load、continuation、自定义 recipe prefix、ONESHELL、second expansion、所有未建模变量/导出、`.SUFFIXES` 与 grouped `&:`。目标必须恰有一个字面声明、无双冒号/分组/静态模式/同行或 shadow recipe，直接 recipe 不得含递归 Make、`+`/`-` 前缀、变量展开或续行；宿主生成 content-addressed candidate id。每个 prerequisite 必须是仓库内无链接真实文件，绑定 path/SHA/size，与 materialized BuildIR output 的 SHA/size 完全一致，并且不是 phony、显式规则目标、匹配的 pattern/单后缀/双后缀目标，也不存在 GNU 内建或 RCS/SCCS 中间源链；完成期重新扫描并逐项重绑。随后才复用受限 argv/env/stdin 解析与 BuildIR executable 映射。该路径完全不启动 GNU Make 或任何子进程，observation 固定 `subprocess_executed=false`；stdout 注入不能增加测试项。新 `plan/migrate` 的 Make 路径未提供 AI proposal 时以 `project_test_make_ai_target_proposal_required` 阻断，并在 compilation facts、ContextPack/portfolio、ledger 与 worker/AI 前停止；固定 `test/check` 或 `make -n` stdout 不再是新计划权威，旧 dry-run 工件只保留完成期重开兼容。完成期从落盘 proposal、manifest 闭包、声明、recipe、prerequisite 与 candidate id 逐项重建，任一漂移 fail closed。最终安全/入口专项 25/25、相关项目测试适配组 118 项中 112 通过且 6 项平台条件跳过、Make 编排关联组 52/52、Windows 仓库级全量 3161 项中 3130 通过且 31 项平台条件跳过；WSL 本地比赛模拟项目迁移全量 1295 项中 1283 通过且 12 项显式环境条件跳过，全部零失败。独立攻击复核未发现剩余 P0/P1；这仍不是 `competition-exact`。该子项只覆盖静态 direct-leaf target，不覆盖递归 Make、自定义 runner、动态/条件 Make 或完整 fixture/resource，因此不关闭 h4、Q4-Q8 或整项目语义验收。
               - [ ] A19d3d2a2h4：补齐递归 Make、Kconfig 和自定义 runner 适配，以及动态/条件 Make、其余 stdin 来源、显式及隐式 fixture、完整资源合同、参数中多路径模板、额外显式 Rust selector/`#[test]` 发现和 adversarial process policy；随后在未参与规则开发的真实多目标 C 项目上由比赛入口证明项目元数据发现完整，并取得一对一全清单通过。当前支持 CMake/CTest direct executable、h4c 的有界 Meson direct-executable inventory、h4d 的受限 Make 单文件 stdin、h4e 的已发现 direct-executable case 双向全等，以及 h4f 的 AI 候选/宿主静态 direct-leaf Make target；h4a/h4b 的 `make -n test/check` 仅保留历史工件重开兼容，不再作为新计划权威。h 与所有 Q4-Q8/整项目父项继续未勾选。

  **A19e 项目级验证与精确 repair**

  - [x] 候选 Cargo 命令只允许进入 Linux bubblewrap 无网络沙箱；实际 `cargo/rustc/rustdoc` 由 `rustup which` 解析，toolchain 树受大小/数量约束、完整内容哈希、执行前复算并只读挂载。早期 WSL smoke 在缺 bubblewrap 时返回 `bubblewrap_unavailable` 且零候选执行，不把 `--offline` 冒充沙箱；后续安装的 `bwrap` 目前只完成 Clang fact live lane，不能据此宣称 Cargo 候选已重跑。
  ...
  - [x] 接入 host-owned integration 与 Cargo check/test adapter，并把受限 Cargo JSON 模块诊断回投到对应 candidate compile gate 触发 repair；早期 WSL smoke 因缺 `bwrap` 而零候选执行且不产生 pass，后续 Clang fact live lane 不改变这条历史 Cargo 证据。
  - [ ] **A19e4：正向 candidate verifier、quarantine 与完成路径**。
    ...
    - [ ] A19e4e：为 C oracle/Rust replay/diff、negative、unsafe/alias、ABI 与 candidate final gate 接入专属 host-owned runner 和专属 raw observation schema；底层通用 ledger API 不允许写入调用方构造的 pass。promotion 前重新派生同一 verification context，任一 gate、源码、generation 或 cohort 过期均拒绝 last-good。
      ...
      - [ ] A19e4e2：接入可实际执行的 oracle/replay/diff、negative、unsafe/alias 与 ABI 后端，并由 A19e7/A19e8 receipt 支撑 promotion 和 `project-final` 复验。
        - [x] A19e4e2a：依赖闭合的标量函数 lane 已在 Linux bubblewrap 内实际执行 C oracle、Rust replay、diff、`forbid(unsafe_code)` 和 ABI；行为样本在候选固定后由宿主生成 32 个 held-out case，negative 门实际编译运行首项 `wrapping_add(1)` mutant，旧 6 样本查表候选会被拒绝。C oracle driver 还必须与 CIndex 中原 BuildIR 编译器 basename、binary SHA 和大小一致。WSL 4 项实跑在 298.420 秒内通过；该子项不覆盖依赖函数、指针/记录、项目测试或 A19e7 独立进程 receipt，因此不关闭 A19e4e2。
          - [x] A19e4e2a1：把原来主要落在 `[-4095,8191]` 的随机标量样本升级为源码边界驱动。宿主在注释/字符串/字符/浮点数之外受限提取函数内整数常量，优先加入阈值本身及 `-1/+1` 邻域，再加入类型安全幂与 nonce 补位；仅在 compile facts 明确含 `-fwrapv` 时启用有符号类型极值，避免把 C signed-overflow UB 伪装成 Rust 错误，unsigned 仍覆盖 wrapping 边界。`x == 100000` 的正负例与 lexical 过滤单测通过；一次真实 WSL 四门先发现极值 UB 假失败，修正后同一 live test 在 258.737 秒通过。该子项本身不解析宏常量，也不证明任意输入无 UB。
          - [x] A19e4e2a2：把源码、CIndex、头文件和 compile options 中有证据绑定的 object-like 整数宏加入同一标量 stimulus 边界。解析器只接受有界整数表达式与已解析宏依赖，不使用 `eval`；支持 `-DNAME`、`-D NAME=value`、`-U`，并对循环、冲突重定义、条件定义、function-like 或复杂表达式 fail closed。宏阈值及 `-1/+1` 邻域仍服从类型范围和 `-fwrapv` 规则。Windows 聚焦组 36 项通过、2 项平台条件跳过；该有限 producer 不等于完整预处理器、cfg/type/layout 求解或全输入 UB 证明。
  ...
  - [ ] **A19e6：唯一 CompletionCoordinator 与强制 project-final 完成链**。
    - [ ] A19e6a：只保留一个可改变整项目 semantic 生命周期的 `migrate/resume` 主入口，由 CompletionCoordinator 固定执行 discover/BuildIR、plan/DAG、dispatch/repair、promotion、RustProjectIR/Cargo generation、`project-final` revalidation、project gates 和 complete；其他入口不能跳步或自行选择 gate。
      - [x] A19e6a1：新增根 `AGENTS.md` 与 canonical `project_migration_harness.py migrate` 外层入口。它使用 competition/required 默认值，先生成 hash-bound plan，再进入现有 `run-to-completion`，非 completed 统一返回非零；内部 `generate-candidate`/`execute-command` worker 模式优先，避免外层规则递归。低层写入口尚未全部降权，因此 A19e6a 仍保持未完成。
    ...
    - [ ] A19e6d：Coordinator 在每个阶段写入幂等 checkpoint 和 hash-bound completion receipt；中断恢复必须从 FSM/事件重新派生下一步，最终 receipt 绑定状态序列、project-final candidate set/generation、所有 verifier receipt、project gates 和 invariant-audit 结果。
      - [x] A19e6d1：候选晋升与最终 `project-final` 完成屏障现在共用 provider-execution reopener；模型已启动时必须恰有一个调用工件，并重新校验 request/preflight/model/session/invocation receipt/raw response/parsed response 的内容绑定。晋升后的缺失、歧义或漂移证据不能发布 completed。该项不替代 completion receipt 的两阶段发布与崩溃恢复。
      - [x] A19e6d2：OpenCode provider 进程使用独立进程组；timeout 或 ledger 启动回调失败会终止整个后代树并有界等待。stdout/stderr 先落临时文件，宿主只读取限制加一个字节，异常大响应不会先完整进入内存。Windows 聚焦测试 14 项通过（1 项 POSIX 条件跳过），WSL competition 环境相关组 17/17 通过；这不等于 cgroup 聚合资源证明。
      - [x] A19e6d3：完成 receipt 改为 crash-consistent 两阶段顺序：先原子写入 canonical receipt、fsync 文件与可用的父目录并重开，再在一个 SQLite 事务中把全部 unit/run completion transition 的 evidence SHA 绑定到该 receipt，最后提交 completed。落盘失败不会调用完成事务，事务中断会回滚全部完成 projection；receipt-first orphan 可在重启后幂等复用。已完成 run 的读取与 coordinator 恢复都会重开 receipt、BuildIR refs、project-final gate bundle、project test evidence 和 transition set，缺失、伪造或漂移只能 blocked。最终 Windows 项目迁移全套 1208 项通过且 25 项平台条件跳过，WSL 选定跨平台比赛组 75/75 通过；同时修复混合 adapter 对 Make 私有模块的越界依赖，并让旧 provider 行为测试改走新的受控进程执行边界。`finalizing` epoch 与 `CURRENT` 两阶段发布仍属 A19e6e。
    - [ ] A19e6e：完成单调 `completion_epoch`、`active -> finalizing -> completed` 与 `CURRENT` 的可恢复两阶段发布。进入 finalizing 后拒绝新 lease/attempt，且只有同一 cohort/generation 的全部 gate 与 invariant audit 通过后才能发布 verified receipt 与 `CURRENT`；失败或崩溃不得提前移动 last-good。
      - [x] A19e6e1：完成数据库侧 finalizing epoch 与完成凭据绑定。schema v9 为 run 增加独立 `completion_status`、单调 epoch 及 cohort/generation/gate-bundle/invariant/receipt SHA；v8 旧库只保留历史 `active -> completed` 重放，新完成命令强制 `active -> finalizing -> completed`。进入 finalizing 的单一事务重开 project-final cohort、managed generation、全部项目门和 quiescent invariant；此后应用检查与数据库触发器共同拒绝 assignment、lease、attempt、context refresh、candidate promotion、project repair、project gate 和 provider start。崩溃恢复复用同一 epoch，receipt v2 绑定冻结上下文，最后一个事务把全部 unit/run transition 绑定到 receipt SHA；漂移或持久化失败只能保持 finalizing/blocked。合并后 Windows 聚焦组 88/88 通过，所有生产模块不超过 300 行。现有 `CURRENT` 尚未绑定 epoch/cohort/generation/receipt，也没有崩溃安全的 prepare/commit 指针协议，因此 A19e6e 父项继续未勾选。
  ...
  **A19f 泛化防特判与有限 held-out 验收**
  ...
  - [ ] **A19f1：competition whole-project qualification matrix（机器验收合同）**。固定矩阵总数不超过 20 个 `case_id`，首批逐项覆盖 TinyCC、FlashDB、cJSON、libyaml、Lua 5.5、Expat、PostgreSQL 和 Linux kernel。一个 case 由项目、上游 pin、声明的全项目构建范围和正式 variant 集合共同定址；每个 pinned case 只能执行一次正式 qualification，失败后若据此修改规则，原 case 永久转为 development regression，后续必须使用新的 pin/`case_id` 且仍计入 20 个上限。禁止 1,000/10,000 次重复运行、近重复项目或多次挑选最好结果。
    ...
    | `wpq-expat` | Expat；`https://github.com/libexpat/libexpat.git`；开发 ref `R_2_8_2` 已知，正式 commit/tree 尚未写入 case manifest | `development-regression`；初始 probe 已推动工作目录、变体/链接闭包和 target-domain 消歧修复，永不计 held-out | `blocked`：原始数据库的 36 条编译记录、29 个唯一 C 源文件、7 组同源双变体和 7 个链接目标现已完整进入已验证 BuildIR/AI ContextPack；competition-profile 计划生成 489 个函数节点、407 个 SCC，跨目标伪歧义为 0，155 个 parser boundary 保留分类证据。schema v3 development regression 已真实执行 36/36 个 C2Rust 进程并生成 36/36 个隔离快照；export 重开门重算 380 个 unit-level/764 个 target-occurrence export 且冲突为 0。最新计划又把 407 个 SCC 全部绑定为可从 36 个 BuildIR unit 重算的 target scope，覆盖 6 个 terminal target；scope-set SHA-256 为 `d0aa78cf98106490f1c1eccb35b825cea8d3919238391126d33d6b18435d78e8`。RustProjectIR v3、多目标 Cargo、AI repair、测试清单一一对应和 Q1-Q11 仍未完成，因此绝不是全项目 pass。测试专用单变体回归与这些 development regression 均不得复写为 held-out 或正式 qualification。 |
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  1. A19d3d2a2g1 已关闭：migration graph、portfolio DAG、integration manifest 和 RustProjectIR reopener 已共用同一份可从 BuildIR 重算的 SCC/target scope；旧 manifest 只能走 v2 兼容路径，不能冒充 v3。
  2. A19d3d2a2g2-g3、g4a-g4b、g4c1、g4c2a1 与 g4c2a2 已关闭；现在完成 g4c2b 的实际 rustc/linker argv/trace 分层对账。g4c1 已绑定对象 occurrence、生成根模块顺序、真实 dep-info、最终产物 commitment 和 archive 代码对象成员位置；g4c2a1 保证 C/BuildIR 源端跨类别顺序不再丢失；g4c2a2 已给出 object/module、package dependency、native/system 的四层可重算期望。三者仍不能替代实际物理链接证据。只有期望与 linker/ELF/archive witness 完全一致，g4c2、g4c 与 g4 结构父项才可关闭。
  3. A19d3d2a2h1-h3 与 h4a-h4f 已关闭 CMake/CTest、有界 Meson direct-executable inventory、受限 Make 单文件 stdin、已发现 direct-executable case 双向全等、AI 候选/宿主静态 Make direct-leaf target、双沙箱 oracle、崩溃一致完成凭据及 AI repair 回路；早期 `make -n test/check` 仅保留历史重开。现在完成 h4/A19d3d2a 的递归/动态 Make、Kconfig、自定义 runner、其余 stdin、fixture/resource 与项目元数据发现完整性。全项目零静默跳过和零 zero-test 未证明前，不启动正式 qualification。
  ...
  **当前成功率与稳定性收口看板（2026-07-17）**：

  - 当前主开发切片是 A19d3d2a2g4c2b 与 A19d3d2a2h4。g4c2a1 已交付 C/BuildIR 源端 `input/search-root/system/native` 跨类别 occurrence 权威，g4c2a2 已交付 RustProjectIR v3 的 `module-dep-info-commitment`、`cargo-package-product`、`native-link-input`、`system-link-argument` 四层可重算期望；下一步是实际 rustc/linker argv/trace 对账，不能要求已被编译成同一 crate module 的原 C object 继续作为一一对应物理 linker 项。h4 已补 CTest、有界 Meson direct-executable inventory、受限 Make 单文件 stdin、已发现 direct-executable case 双向全等和 AI 候选/宿主静态 Make direct-leaf target，剩余递归/动态 Make、自定义 runner、其余 stdin、fixture/resource 与项目元数据发现完整性。旧 `make -n test/check` 仅兼容重开。当前结构证据与双沙箱结果只是通用门禁能力，不构成 `competition-exact`、真实项目成功率提高或八项目语义通过宣称。
