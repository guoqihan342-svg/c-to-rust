...
- [ ] 拆分 `crates/c2r-translator/src/lib.rs`：当前约 4k 行，先做行为保持拆分，把 CLI/manifest、旧字符串 translator、typed IR route、artifact 写入、unsafe/metadata 统计等责任拆到子模块；已完成公开 model schema 拆分、artifact/IO leaf helper（`write_json_file`、`write_text_file`、`translation_events_jsonl`）拆分和 feature-gated clang dry-run artifact writer（`write_clang_dry_run_artifact`）拆分，crate root API 保持兼容；仍待 CLI/manifest orchestration、legacy string translator、typed IR route、clang lowering report writer、unsafe/metadata 统计、parser/evidence builder/emitter 的后续拆分；拆分提交必须保持现有测试通过。
