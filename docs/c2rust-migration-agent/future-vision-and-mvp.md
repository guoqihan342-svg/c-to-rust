...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
              ...
              - [ ] A19d3d2a2g4：增加独立重开门，执行 `cargo metadata --locked --offline`、all-target 实际编译和 linker trace/ELF/archive 重开；metadata package/target 集合、实际 rustc target、链接输入顺序和 RustProjectIR v3 必须完全一致。该门通过仍只关闭结构重建，不得在 ProjectTestInventory、C/Rust oracle diff 与 project-final 未通过时提高 semantic numerator。
                ...
                - [ ] A19d3d2a2g4c：闭合从 BuildIR 原始 input occurrence 到 Rust 最终产物的逐 occurrence 可验证链。g4c1 已关闭对象/模块顺序承诺与最终产物定位这一结构子边界；g4c2 仍须证明所有输入类别在实际 rustc/linker argv/trace 中的统一物理顺序。因此 g4c、g4 与结构重建父项继续未勾选。
                  ...
                  - [ ] A19d3d2a2g4c2：捕获并重开统一的实际 rustc/linker argv 与 linker/archive trace，把 object、package dependency、允许的 native/system 输入及原始 archive member occurrence 逐项映射到同一序列，保留重复 occurrence 和跨类别完整相对物理顺序。dep-info、生成根顺序或最终 commitment 都不能替代该证据；native/system 参数无法唯一归属、trace 不完整或任一顺序不一致时必须 blocked。只有该项完成后，g4c 与 g4 结构父项才具备关闭条件；ProjectTestInventory、C/Rust oracle diff 与 project-final 未通过前仍不得提高 semantic numerator。
                    ...
                    - [ ] A19d3d2a2g4c2a2：把 BuildIR 的统一源端 occurrence 投影为 RustProjectIR v3 的分层链接期望。object occurrence 必须绑定 module/dep-info/commitment，package product 必须绑定 Cargo package target，允许的 native/system occurrence 必须保留 consumer、重复和顺序；search-root、repository positional native input、group/as-needed/whole-archive、response-file 方言或任一无法唯一表示的项继续 fail closed。该层只定义可重算期望，不能把 Rust 模块冒充原 C object 的物理 linker 项。
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  2. A19d3d2a2g2-g3、g4a-g4b、g4c1 与 g4c2a1 已关闭；现在完成 g4c2a2 的 RustProjectIR v3 分层链接期望和 g4c2b 的实际 rustc/linker argv/trace 对账。g4c1 已绑定对象 occurrence、生成根模块顺序、真实 dep-info、最终产物 commitment 和 archive 代码对象成员位置；g4c2a1 保证 C/BuildIR 源端跨类别顺序不再丢失。两者仍不能替代 Rust 端 object/module、package dependency、native/system 的分层实际证据。只有期望与 linker/ELF/archive witness 完全一致，g4 结构父项才可关闭。
  ...
  **当前成功率与稳定性收口看板（2026-07-17）**：
  ...
  - 当前主开发切片是 A19d3d2a2g4c2a2、A19d3d2a2g4c2b 与 A19d3d2a2h4。g4c2a1 已交付 C/BuildIR 源端 `input/search-root/system/native` 跨类别 occurrence 权威；下一步是 RustProjectIR v3 分层期望及实际 rustc/linker argv/trace 对账，不能要求已被编译成同一 crate module 的原 C object 继续作为一一对应物理 linker 项。h4 已补 CTest、有界 Meson direct-executable inventory、受限 Make 单文件 stdin、已发现 direct-executable case 双向全等和 AI 候选/宿主静态 Make direct-leaf target，剩余递归/动态 Make、自定义 runner、其余 stdin、fixture/resource 与项目元数据发现完整性。旧 `make -n test/check` 仅兼容重开。当前结构证据与双沙箱结果只是通用门禁能力，不构成 `competition-exact` 或八项目语义通过宣称。
