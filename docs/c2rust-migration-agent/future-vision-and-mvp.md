...
1. 先补会直接卡真实 C slice 的 Semantic IR / Lowering 最小切片和语义最小合同；Semantic IR / Lowering 边界是 P0 第一梯队，不得被模块拆分或治理任务后置。最小合同包括 implicit cast、integer conversion、ABI/`sizeof`、stdlib/extern provenance、macro/source provenance，以及 `switch`/`goto`/label/case/default 的最小 CFG 证据和 fail-closed 分类。
...
- [ ] 建立 OpenCode 单次比赛 runner 和输出校验器：提供 Linux-first wrapper（可由 OpenCode 一次 prompt 调用），串起 `config/competition-env/env.sh`、`toolchain-check.sh`、slice extraction、`auto_migrate.py --competition-clang-lane`、`validate_auto_translation_evidence.py --require-semantic-pass`、unsafe budget、OpenSpec 和最终摘要生成；runner 应支持准确性优先的顺序或并行 slice worker，要求并行 worker 输出隔离、状态可合并、失败不污染其它 slice；新增 schema/validator/test 校验 `target/competition-out/summary/competition-run-summary.json`，至少包含 `run_id`、`proof_class`（枚举：`competition-exact`、`ci-approximation`、`wsl-local-simulation`、`local-simulation`）、profile id/hash、clang source（`CLANG_PATH`/vendored/missing）、Cargo registry/mirror activation method、attempted/compiled/semantic_pass/refused/blocked、unsafe ratio、elapsed、artifact roots 和 final gate 状态；`config/competition-env/opencode-single-interaction.md` 中的示例摘要必须由同一 schema 约束，避免文档样例字段少于真实 runner 合同。进展：`validation/competition-run-summary.schema.json` 和 `validation/tools/validate_competition_run_summary.py` 已落地，校验 proof class、profile hash、clang source、Cargo mirror activation、slice counts、unsafe ratio、repo-relative artifact roots 和 final gate；`validation/tools/run_competition.py` 已提供顺序版 Linux-first runner，能串 `env.sh`、`toolchain-check.sh`、`auto_migrate.py --competition-clang-lane`、`validate_auto_translation_evidence.py --require-semantic-pass`、unsafe budget、OpenSpec、summary 生成和 summary validator，并在一个 slice 失败时继续处理后续 slice；`run_competition.py` 现在把每个子命令的 `step`、argv、returncode、stdout/stderr 和 repo-relative/out-root-relative `log_path` 追加到 `target/competition-out/logs/commands.jsonl`，失败命令也保留 stderr，且不会为未执行的 validator 伪造日志；runner 现在支持 `--extract-spec` JSON 输入，会在迁移前调用 `extract_source_slice.py`，把生成的 slice spec 写入 `target/competition-out/slice-specs/`，并允许 `--slice-spec` 与 `--extract-spec` 混合汇总；runner 也支持不经 JSON 的直接 CLI 参数组（`--source-repo-root`/`--source-file`/`--function`/`--target-id`/`--slice-id`，以及 `--include-path`/`--define` 等），会组装成同一 extraction spec 并写入统一输出目录；抽取失败会计入 slice failure 并写入命令日志，不会伪造后续 validator 日志；runner 现在会解析 `unsafe_budget.py` 的 JSON stdout，把真实 status、count 和 ratio 写入 `competition-run-summary.json`，并由 summary schema 和 runner 测试固定；`validation.tools.test_validate_competition_run_summary` 与 `validation.tools.test_run_competition` 已进入 core CI。剩余：并行 worker 状态合并仍未完成。
- [ ] 建立 competition Linux/WSL/CI smoke 分级入口：新增 bash 或 CI wrapper，在 Ubuntu/WSL/CI 上执行最小环境检查、核心 auto-evidence validator、`evidence_governance.py` 和轻量 full-regression；输出必须显式标记 `competition-exact`、`ci-approximation`、`wsl-local-simulation` 或 `local-simulation`，并记录不能满足真实比赛机精确条件的字段（kernel、Rust/Node/Python 版本、镜像源、clang 来源等）。
...
- [ ] 把高频 C 语义阻塞前移到 P0 最小合同：所有遇到的 clang `ImplicitCastExpr`、integer promotion、usual arithmetic conversions、narrowing/truncation 至少要进入显式 IR 或明确 fail-closed；`sizeof`、`_Alignof`、plain `char`/`short`/`long`/`long long`、`size_t`、target ABI、struct layout/packing/alignment 至少要有诊断/拒绝证据或第一批支持，禁止在没有 ABI/profile 绑定时把 `long`、`unsigned long`、`size_t` 等宽度静默猜成固定 64-bit；常见 C library/extern 直接调用（`memcpy`、`memset`、`memcmp`、`strlen`、`assert` 等）要有模型、shim、source-provenance 或 fail-closed reason；宏/预处理输入必须绑定 compile command、defines/includes、macro expansion/source mapping 证据，P0 只要求 provenance 或 fail-closed，完整 macro expansion tracking 仍属于后续能力扩展，避免把真实项目失败推迟到 P1 后才暴露。进展：typed IR direct-call emitter 已加入 exact-name reserved C macro/stdlib/extern surface denylist，`assert(int)` 不再能无模型发射成 `assert(value);`，错误原因要求显式 lowering 或 extern binding；`helper`/`observe` 这类用户 direct call 正向路径仍保留。`assert`、`static_assert`、`sizeof`、`offsetof`、`malloc/free`、`memcpy`/`memmove`/`memset`/`memcmp`、`strlen`、`printf`/`fprintf`/`sprintf`/`snprintf`、`puts`/`putchar`/`getchar`、`exit`/`abort` 已有参数化负例，防止未来参数弱化后误发射。no-profile `type_from_qual_type` 已把 `char`、`short`、`unsigned short`、`long`、`unsigned long`、`long long`、`unsigned long long`、`size_t` 统一列为 target-dependent integer width，缺少 ABI width provenance 时 fail-closed；固定宽度 `int8_t` 到 `uint64_t` 和 `signed char` 仍保留正向映射。`BuildProfile.target` / `TargetAbiProfile` 现在承载 slice spec 的 `triple_or_abi`、`int_width`、`char_width`、`plain_char_signed`、`short_width`、`long_width`、`long_long_width`、`pointer_width`；`ClangParseSpec` 会保留该 profile。profile-aware clang AST JSON lowering 现在会在显式字段存在时绑定 `size_t`、`char`、`short`、`unsigned short`、`long`、`unsigned long`、`long long` 和 `unsigned long long`，字段缺失或无 profile 时仍 fail-closed，不靠 LP64 常识放行；`auto_migrate.py` 生成 translator input 时不再丢弃 `build_profile.target`；`oracle_boundary_contract.target` 会保留这些可选 ABI 字段，并在字段存在时验证类型和正宽度。剩余：为真正要支持的 stdlib/extern 调用补 source-provenance、shim/model 和验证门禁；不能一刀切禁掉所有直接调用；继续补 `sizeof`/`_Alignof`/struct layout/packing/alignment 证据、LLP64 防混淆 fixture 和 route/profile evidence。
- [ ] 推进 Semantic IR / Lowering 最小切片：P0 不做完整重写，但必须把最容易污染语义的边界先拆开或显式记录，包括 `ImplicitCastExpr` 不能无证据透明剥离、array-to-pointer decay 要有 IR 节点或 fail-closed reason、ABI width provenance 要进入 type evidence、pointer-to-slice lowering 的前置条件要与 candidate Rust 分离记录。目标是让 IR 更像“描述 C”，让 lowering 更像“生成 Rust candidate”，而不是继续把两者混在 emitter 特例里。进展：clang frontend 已把普通表达式位置的 `ArrayToPointerDecay` 从透明剥离改为 `Unsupported`/fail-closed，并用表达式级单测固定 reason 包含 `ArrayToPointerDecay` 与 explicit IR 边界；新增 no-clang AST fixture 证明 `return *table;` 仍 fail-closed，同时允许仅在 `ArraySubscriptExpr` base 且索引 IR 已显式表达访问语义时剥离 decay，`table[i]` 已有 fixture replay 和 emitted Rust compile check，避免 blanket reject 误伤真实 clang 数组下标；target-dependent integer width 现在有 no-profile fail-closed 单测，防止 `size_t`/`unsigned long` 在没有比赛 ABI profile 时被猜成 64-bit；新增 no-clang AST fixture 证明同一个 `size_t` 函数在无 profile 下拒绝、在显式 `build_profile.target` 下能降为 typed IR 并发射 `usize` Rust。剩余：把 array decay 边界继续升级成显式 Semantic IR 节点或 lowering 规则，而不是长期留在 skeleton 特判；继续把 function-to-pointer decay、pointer value 传参/返回等边界显式化。当前能力只覆盖目标相关整数宽度绑定，不代表完整 C ABI、struct layout/packing/alignment、FFI 或 on-disk layout 支持。
- [ ] 建立 `switch`/`goto` 最小 CFG 证据与 fail-closed 分类：`switch`、`goto`、label、`case`、`default` 在 P0 至少要求 clang/source span 识别、basic block/edge 证据、unsupported construct 分类、repair hint 和 coverage matrix 负例；P0 不要求完整任意 CFG lowering，但必须交付 CFG schema、clang AST/typed-IR unsupported skeleton 或 fixture replay、以及至少一个最小 relooper/结构化恢复原型或显式 `relooper_precondition`/`relooper_refusal` evidence，不能只在 legacy compatibility path 里给 block/edge 诊断。完整 relooper、结构化控制流恢复和 Rust candidate lowering 仍属于 P1/Phase 2，不抢 P0 的真实 generated slice 语义通过目标。进展：legacy compatibility path 已对 label/goto/switch/case/default 输出细分 `unsupported_control_flow`、最小 CFG block/edge（如 `goto-again->label-again`、`switch-0->case-1`、`switch-0->default`）、`relooper_refusal:goto` / `relooper_refusal:switch` 证据并保持空 Rust draft；clang AST JSON fixture replay 已覆盖 `GotoStmt`/`SwitchStmt`/`LabelStmt`/`CaseStmt`/`DefaultStmt` 的 structured unsupported skeleton，错误原因绑定 `CFG/relooper support`；`validation/translator-coverage-matrix.json` 已记录 `complex-control-flow-fail-closed` partial capability、clang fixture replay evidence 和 relooper refusal evidence。剩余：route evidence 还未覆盖该类拒绝；最小 relooper/结构化恢复原型仍未落地。
