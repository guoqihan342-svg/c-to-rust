...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  5. A19d3c2b3c 与 A19d3d2：以已闭合的 project compile intake/AI repair/同门次复验链为基线，先完成 whole-cohort 唯一归属和 link/init/feature/ABI 的专属 verifier 分类，再扩展 RustProjectIR/Cargo 到真实多 target/feature/native-link 项目；可与 A19e7 专属 semantic runner 并行，但 Cargo generation 不得绕过 RustProjectIR。
