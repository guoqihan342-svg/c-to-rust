...
- [ ] **P0-A18：replay-compatible Rust API 合同前置**
  ...
  - [ ] **P0-A18b：结构化 ReplayCallPlan**。从 replay generator 抽出单一结构化计划，显式携带参数名/顺序/Rust 类型、C 参数映射、length retained、返回类型、ABI、unsafe 和 call args；renderer 与 prompt 只能消费同一计划。删除 `fdb_kv_to_blob/set/del` 等按函数名选择合同的旧分支，函数名只能作为合同数据。
    ...
    - [ ] **P0-A18b2：legacy adapter 全部计划化**。其余 scalar/record/external-sequence renderer 仍直接构造调用，尚未全部改为消费 ReplayCallPlan；因此 P0-A18b 总项保持未完成。
      ...
      - [ ] **P0-A18b2c：简单 legacy renderer 清零**。原盘点的 13 个 scalar return、mutable out report、record/pointer identity、opaque context 等直接调用 renderer 中已迁移 7 个，还剩 6 个；全部改为 plan producer 后删除 legacy fallback 与 readiness OR 列表。
        ...
        - [ ] **P0-A18b2c2：mutable-out/report 3/4**。single i32 output、dynamic i32 slice output 与 input-buffer out0+sum 已按 C 签名、参数方向、length companion 和 fixture observable 闭合生成 plan v2；固定 i32 array、受限 `Vec<i32>` initializer、最多 4096 项的 `i32_slice/i32_vec` codec、whole-binding observation、fixture metadata assertion/relation、完整符号/字段重命名和漂移拒绝均已覆盖。真实 store-add-one、copy-i32-ptr-arith、sum-i32-buffer 与 sum-i32-ptr-arith replay 不再进入 legacy renderer。剩余 call-metadata。
        - [ ] **P0-A18b2c3：record-pointer/opaque 5/5**。迁移 record pointer identity、record buffer/length identity、record projection identity、opaque key 与 opaque key/value。
