...
- [ ] 把高频 C 语义阻塞前移到 P0 最小合同：所有遇到的 clang `ImplicitCastExpr`、integer promotion、usual arithmetic conversions、narrowing/truncation 至少要进入显式 IR 或明确 fail-closed；`sizeof`、`_Alignof`、plain `char`/`short`/`long`/`long long`、`size_t`、target ABI、struct layout/packing/alignment 至少要有诊断/拒绝证据或第一批支持，禁止在没有 ABI/profile 绑定时把 `long`、`unsigned long`、`size_t` 等宽度静默猜成固定 64-bit；常见 C library/extern 直接调用（`memcpy`、`memset`、`memcmp`、`strlen`、`assert` 等）要有模型、shim、source-provenance 或 fail-closed reason；宏/预处理输入必须绑定 compile command、defines/includes、macro expansion/source mapping 证据，P0 只要求 provenance 或 fail-closed，完整 macro expansion tracking 仍属于后续能力扩展，避免把真实项目失败推迟到 P1 后才暴露。
  ...
  `BuildProfile.target` / `TargetAbiProfile` 现在承载 slice spec 的 `triple_or_abi`、`int_width`、`char_width`、`plain_char_signed`、`short_width`、`long_width`、`long_long_width`、`pointer_width`；`ClangParseSpec` 会保留该 profile。profile-aware clang AST JSON lowering 现在会在显式字段存在时绑定 `int`、`unsigned int`、`size_t`、`char`、`short`、`unsigned short`、`long`、`unsigned long`、`long long` 和 `unsigned long long`，字段缺失或无 profile 时仍 fail-closed，不靠 LP64 常识放行；clang `UnaryExprOrTypeTraitExpr` 的 `sizeof(type)` 已有第一批 profile-bound lowering：`sizeof(int)`、`sizeof(long)`、`sizeof(size_t)` 以及完整定长整数数组 type operand（如 `sizeof(int[3])`）会降为 `size_t`/`usize` 整数字面量，并验证结果能放入目标 `size_t` 宽度；已用 `int_width`、LP64/LLP64 `long_width` 防混淆单测、no-clang `sizeof(int)` / `sizeof(int[3])` fixture replay、无 target ABI profile 拒绝、incomplete array 拒绝和 size_t 结果溢出拒绝固定。expression operand `sizeof(x)`、incomplete/VLA array、record/struct layout、object operand 和 packing/alignment 仍在无显式语义/布局证据时 fail-closed；`_Alignof(type)` 已有显式 fail-closed 单测和 no-clang fixture replay，原因绑定到缺少 alignment/layout profile，禁止用宽度 profile 猜 alignment；`auto_migrate.py` 生成 translator input 时不再丢弃 `build_profile.target`；`oracle_boundary_contract.target` 会保留这些可选 ABI 字段，并在字段存在时验证类型和正宽度。
  剩余：为真正要支持的 stdlib/extern 调用补 source-provenance、shim/model 和验证门禁；不能一刀切禁掉所有直接调用；继续补 expression/object `sizeof`、struct layout、packing/alignment、正式 `_Alignof` 支持和 route/profile evidence。
