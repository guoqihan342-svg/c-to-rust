...
- [ ] 建立 translator 覆盖矩阵，而不是只数测试文件或测试条数：按 IR construct、clang fixture replay、手写 IR、负例、runtime emitted Rust、C/Rust diff、legacy fallback、route evidence 维度统计覆盖；每个新增语法能力必须在矩阵里有正例、负例和 fail-closed reason。
