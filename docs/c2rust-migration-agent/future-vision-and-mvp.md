...
- [ ] 把 route metadata 做成真实 candidate-selection layer，或把命名收窄成 provenance。当前 `GenericTypedIr` / `Unsupported` 主要是在 typed IR emission 之后贴标签；后续 route governance 仍需记录完整 candidate set、score/hard gate、selected generator、fallback chain、refusal reason 和 validation gates，并接入 C2Rust/LLM 候选调度。
