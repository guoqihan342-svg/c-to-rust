...
| 当前翻译主线 | P0-A18c / P0-A10 | A18c8h4g 已完成真实 libuv DeepSeek AI-first 辅助实跑并确认 repair 能从 replay compile failure 推进到 replay runtime failure；下一步按 A18c8h5a/h5b/h5c 收敛真实诊断传播、无 oracle 值的 case/field 反馈和 ABI 门禁 |
| 外部并行项 | P0-H9 | 在真实比赛主机完成 OpenCode + GLM-5.1 精确合同复验 |
| 最近开发阶段 | P0-A18c8h4g | WSL competition-like lane 使用 DeepSeek/max、fresh C oracle 和 3 轮 repair 完成真实调用；独立 validator 重开 29 个工件无结构错误。第 2/3 轮通过 rustc、oracle、unsafe 和 alias，但 replay/ABI/final 未通过，`semantic_pass=false`、成功计数仍为 38 |
...
| 2 | P0-A18c8 剩余 exact failure 收敛 | 进行中 | 先完成 h5a 真实 replay compiler/linker facts 传播，再完成 h5b 只暴露 case/field 路径的语义反馈，随后修复 h5c 安全候选 ABI 误拒绝；三项均需改名正例、漂移负例和同一 exact gate 重跑 |
  ...
  - [ ] **P0-A18c：有限跨项目验收**。固定 12 项必须全部可生成真实 replay call，首轮 rustc API mismatch 相比 A17 明显下降，并继续只按 exact gates 统计质量信号。
    ...
    - [ ] **P0-A18c8：剩余 exact failure 的通用语义闭包**。
      ...
      - [ ] **P0-A18c8h：通用可变标量地址到 `void *` 调用参数**。仅对可证明为完整整数标量 lvalue 的直接/记录字段地址、直接外部函数调用和明确的 mutable `void *` 参数建模临时可变借用；要求别名、存活期、对齐、宽度、调用签名和写后读取路径可验证。const、nullable、位域、packed/volatile/atomic、指针算术、跨调用逃逸和重叠可变借用继续拒绝，禁止按 `in_addr_t`、libuv 或固定 callee 名称放行。
        ...
        - [ ] **P0-A18c8h5a：真实 exact replay compiler/linker facts 闭包**。`validate_auto_migrate_candidate` 的 replay wrapper 必须在 phase=`compile` 时转发 hash-bound `compile_stderr`，并从 rustc JSON 的严格 linker child 诊断中只提取 C 标识符形状的 undefined symbol；不传 rendered source、命令、路径或任意 child 文本。compiler/linker facts 必须绑定 candidate、replay、fixture 和 target SHA，篡改任一绑定零转发。完成判据是中立 extern 名称的真实 wrapper 回归进入 repair facts，host path/secret/spoof 负例仍被清理，且 compile-only 外部调用不得因此获得语义通过。
