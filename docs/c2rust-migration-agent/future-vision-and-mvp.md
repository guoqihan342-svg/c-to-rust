...
| 当前翻译主线 | P0-A18c / P0-A10 | A18c8a/A18c8b/A18c8b1 已关闭；下一步实现 A18c8c 通用可执行 C oracle call plan，并分别处理函数表间接调用与记录布局 `sizeof` 缺口 |
| 外部并行项 | P0-H9 | 在真实比赛主机完成 OpenCode + GLM-5.1 精确合同复验 |
| 最近开发阶段 | P0-A18c8b1 | translator 已回放 hash-bound compile DB、真实 TU 与宏展开函数定位；当前真实项目在未证明的间接调用/记录布局处明确拒绝 |
...
| 2 | P0-A18c8 剩余 exact failure 收敛 | 进行中 | A18c8a/A18c8b/A18c8b1 已完成；下一项让通用 C oracle call plan 真正执行 fixture 调用与输出比较，并继续由 AI/确定性 fallback/验证层处理剩余语义缺口 |
