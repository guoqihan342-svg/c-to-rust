...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  1. 先完成 A19d3d2a2g 的 SCC/manifest target scope、RustProjectIR v3 和多 package Cargo workspace：同源 variant、archive/link 产品、entrypoint、export 和 module namespace 必须按 target 隔离，Cargo metadata、实际编译和 linker witness 与 IR 完全一致后才进入测试执行。
  2. 随后完成 A19d3d2a2h/A19d3d2a 的项目测试清单与双端 oracle：只从项目自有 CMake/CTest/Meson/Make/Kconfig 等元数据生成 Q3-Q4 inventory，把 runner、argv/stdin、fixture、工作目录和资源合同一一投影到 Rust target；原 C baseline、测试身份双向全等、零静默跳过和零 zero-test 未证明前，不启动正式 qualification。
