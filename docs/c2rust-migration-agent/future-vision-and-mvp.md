...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  3. A19d3d2a2h1-h3 与 h4a 已关闭 CMake/CTest direct-executable inventory、受限 Make `test` 配方 inventory、双沙箱 oracle、内容寻址完成凭据和 AI repair 回路；现在完成 h4/A19d3d2a 的 Meson、Automake/任意 target、递归 Make、Kconfig、自定义 runner、stdin/fixture/resource 与测试清单双向全等。零静默跳过和零 zero-test 未证明前，不启动正式 qualification。
  ...
  **当前成功率与稳定性收口看板（2026-07-16）**：
  ...
  - 当前主开发切片是 A19d3d2a2g4 与 A19d3d2a2h4：前者完成 v3 Cargo metadata/实际目标/linker witness 独立重开，后者在已完成的 CTest 与受限 Make `test` 配方基础上扩展 Meson/Automake/递归 Make、自定义 runner、stdin/fixture/resource 和完整性证明。当前清单与双沙箱结果只是通用门禁能力，不穿插八项目正式通过宣称。
