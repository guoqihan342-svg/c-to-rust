...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  1. 先完成 A19d3d2a2f 的 BuildIR-bound 逐 unit 隔离转译和 terminal-target plan：原 compile record、variant、生成快照、archive/link 顺序与 native/system 边界必须逐项可重开；此阶段只产生候选和目标计划，不运行 Cargo，也不提高 translator numerator。
  2. 再完成 A19d3d2a2g 的 SCC/manifest target scope、RustProjectIR v3 和多 package Cargo workspace：同源 variant、archive/link 产品、entrypoint、export 和 module namespace 必须按 target 隔离，Cargo metadata、实际编译和 linker witness 与 IR 完全一致后才进入测试执行。
