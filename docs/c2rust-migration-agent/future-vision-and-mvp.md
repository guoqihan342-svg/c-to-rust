...
- [ ] 明确比赛环境 clang 路线：`config/competition-env/environment.json` 当前没有把 clang 列为基线工具，`auto_migrate.py --emit-clang-lowering-report` 也是 opt-in。要么把 clang 声明为 optional/required capability，补 `CLANG_PATH`、安装/检查说明和比赛 wrapper，缺失时清晰失败；要么把默认比赛 lane 诚实标为非 clang 的 legacy/fail-closed evidence。不能静默依赖本机 Windows clang。
...
- [ ] 在声明 checksum/hash 翻译覆盖前修复无符号整数模运算语义：typed IR 不能对 C unsigned arithmetic 发射依赖 profile 的裸 Rust `+`、`-`、`*`。先加 debug/overflow-checks runtime 红测覆盖 `u32::MAX + 1`、`0u32 - 1` 和 unsigned multiply，再发射显式 wrapping 运算或等价的可审计 lowering 策略。signed overflow 和非法 shift count 是单独的 fail-closed/contract 问题。
...
- [ ] 把 route metadata 做成真实 candidate-selection layer，或把命名收窄成 provenance。当前 `GenericTypedIr` / `Unsupported` 主要是在 typed IR emission 之后贴标签；后续 route governance 应记录 candidate set、selected generator、fallback chain、refusal reason 和 validation gates，并禁止 clang-lowered typed IR 失败后无遥测地静默 fallback 到 legacy string translator。
