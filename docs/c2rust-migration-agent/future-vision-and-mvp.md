...
- [ ] 建立 OpenCode 单次比赛 runner 和输出校验器：提供 Linux-first wrapper（可由 OpenCode 一次 prompt 调用），串起 `config/competition-env/env.sh`、`toolchain-check.sh`、slice extraction、`auto_migrate.py --competition-clang-lane`、`validate_auto_translation_evidence.py --require-semantic-pass`、unsafe budget、OpenSpec 和最终摘要生成；runner 应支持准确性优先的顺序或并行 slice worker，要求并行 worker 输出隔离、状态可合并、失败不污染其它 slice；新增 schema/validator/test 校验 `target/competition-out/summary/competition-run-summary.json`，至少包含 `run_id`、proof class、profile id/hash、clang source（`CLANG_PATH`/vendored/missing）、attempted/compiled/semantic_pass/refused/blocked、unsafe ratio、elapsed、artifact roots 和 final gate 状态。
...
- [ ] 把 translator-generated L3 语义通过变成 P0 milestone：至少选 3 个真实 C 源 named slice（不能只靠手写 `flashDB_rust` 或 accepted evidence authoritative 边界），要求 generated Rust draft 本身通过 C oracle、Rust replay、diff、negative diff、unsafe ledger、final verification，并发布 manifest/profile/route/report hash；若 generated draft 仍只是 candidate，公开进度只能写 candidate generation，不得写自动翻译 accepted。
- [ ] 把高频 C 语义阻塞前移到 P0 最小合同：所有 clang `ImplicitCastExpr`、integer promotion、usual arithmetic conversions、narrowing/truncation 至少要进入显式 IR 或明确 fail-closed；`sizeof`、`_Alignof`、plain `char`/`short`/`long`/`long long`、target ABI、struct layout/packing/alignment 至少要有诊断/拒绝证据或第一批支持；常见 C library/extern 直接调用（`memcpy`、`memset`、`memcmp`、`strlen`、`assert` 等）要有模型、shim、source-provenance 或 fail-closed reason；宏/预处理输入必须绑定 compile command、defines/includes、macro expansion/source mapping 证据，避免把真实项目失败推迟到 P1 后才暴露。
