...
- [ ] **P0-A19：陌生 C 项目整项目 AI harness 泛化**
  ...
  **下一执行顺序（按完成条件推进，不按测试用例身份推进）**：
  ...
  2. A19d3d2a2g2-g3、g4a-g4b 与 g4c1 已关闭；现在完成 g4c2 的统一实际 rustc/linker argv 与跨类别物理顺序证据。g4c1 已绑定显式对象 occurrence、生成根模块顺序、真实 dep-info、最终产物 commitment 和 archive 代码对象成员位置，但不能替代 object/package-dependency/native/system 的统一实际顺序。只有该序列和 linker/ELF/archive witness 都与 RustProjectIR v3 完全一致，g4 结构父项才可关闭。
  ...
  **当前成功率与稳定性收口看板（2026-07-17）**：
  ...
  - 当前主开发切片是 A19d3d2a2g4c2 与 A19d3d2a2h4。g4c1 已完成显式对象 occurrence、生成根模块顺序、真实 rustc dep-info、最终产物固定 commitment 和 archive 代码对象成员定位；g4c2 剩余 native/system 统一 argv 及 object/package-dependency/native/system 跨类别实际 linker 物理交错证明。h4 已补 CTest、有界 Meson direct-executable inventory、受限 Make 单文件 stdin、已发现 direct-executable case 双向全等和 AI 候选/宿主静态 Make direct-leaf target，剩余递归/动态 Make、自定义 runner、其余 stdin、fixture/resource 与项目元数据发现完整性。旧 `make -n test/check` 仅兼容重开。当前结构证据与双沙箱结果只是通用门禁能力，不构成 `competition-exact` 或八项目语义通过宣称。
