...
**当前状态**：我们的 typed IR 有 `Cast { implicit }` 字段区分显式/隐式 cast，integer 类型有 signed/width 信息，指针有 `Pointer { pointee }`。clang 前端已把 unknown/missing `ImplicitCastExpr.castKind` 收紧为 `Unsupported`/fail-closed，已建模 `IntegralCast`/`IntegralPromotion` 可在普通 value context、`?:` condition context 和 `if`/`while`/`do-while`/`for` condition context 中保留为显式 IR cast，`LValueToRValue`/`NoOp` 仍是受限透明 skeleton 边界；但尚未做到所有隐式 cast 都进入统一 Semantic IR 节点，array/function decay、非整数 conversion、跨参数/返回/compound assignment 的完整 conversion discipline 等仍需要继续 formalize。
