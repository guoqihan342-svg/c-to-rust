...
- 按值 record 字段读取/简单赋值/字段 compound assignment/statement inc-dec/本地 copy/whole-record return：真实 clang AST 中的 `struct point p; return p.x;`、`p.x = value; return p.x;`、`p.x += value; return p.x;`、`p.x++; return p.x;`、`struct point q = p; q = r; return q.x;`、`struct point identity(struct point p) { return p; }` 现在可 lowering 成 typed IR record candidate；generic emitter 可生成最小/唯一具名完整直接标量字段清单驱动的 Rust `Point` struct、字段读写、RHS 为简单整数变量/字面量/整数 cast 的字段 compound assignment、statement 位置按值 dot-field inc/dec、本地 copy 和 `return p;`。这只是 candidate generation，不表示 C record layout/ABI 等价；`p->x`、value-position 字段 inc/dec、字段 compound assignment 复杂 RHS、指针/alias-sensitive 字段写、同名 tag、self-pointer 字段、union/bitfield/volatile/packed/非标量字段或 semantic acceptance 仍 fail closed。
...
- Standalone inc/dec statement：普通 `value++`、`++value`、`value--`、`--value` 在表达式值被丢弃的 statement 位置可 lowering 成 typed IR assignment，并发射为 `value = value +/- 1`；direct by-value record dot-field statement target（例如 `p.x++` / `--p.x`）也可 lowering 成 `p.x = p.x +/- 1`。这只覆盖简单整数变量 target 和 direct by-value record dot-field integer target；不表示 value-position inc/dec、condition/call argument/return 中 inc/dec、`ForStmt` step 中 record-field inc/dec、pointer/array/arrow/复杂 field target 或完整 C 自增表达式语义已支持。
