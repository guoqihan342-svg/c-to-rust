...
- 按值 record 字段读取/简单赋值：真实 clang AST 中的 `struct point p; return p.x;` 和 `p.x = value; return p.x;` 现在可 lowering 成 typed IR `IrExpr::Member`，并由 generic emitter 生成最小 Rust `Point` struct、`return p.x;` 和 `p.x = value;`；这只是 candidate generation，不表示 C record layout/ABI 等价、无字段使用的 record 参数、`p->x`、compound/update 字段写、指针/alias-sensitive 字段写、union/bitfield/volatile、record local/return 或 semantic acceptance 已支持。
