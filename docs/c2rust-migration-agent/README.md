...
- FlashDB accepted evidence?`real-fdb-blob-make` ????? L4 accepted-evidence authoritative ?????`real-fdb-calc-crc32`?`real-fdb-blob-make`?`real-fdb-kv-del`?`real-fdb-kv-set` ? `real-fdb-kv-to-blob` ? exact typed-IR generated Rust draft ??? `generated_draft_acceptance.status=passed`??? coverage matrix ??? `zlib-ng/adler32-step` ??? demo exact typed-IR drafts??? `translator_generated_semantic_pass_count=21`?raw C2Rust baseline ?? `candidate_context_only`????? translator-generated accepted draft?
- FlashDB KV ???`real-fdb-kv-set` ??? uninitialized DB `return_code=FDB_INIT_FAILED` fixture?initialized set/delete path?blob ????`fdb_kv_set_blob` ??? callee ???????`real-fdb-kv-del` ???? uninitialized DB fixture?initialized delete path ??? `del_kv` ???????`real-fdb-kv-to-blob` ??? 3-case fixture ? `kv->addr/value_len` ? `blob->saved.*` ? record-pointer ????? identity-return ?????? `fdb_tsl_to_blob`??? layout/ABI????????? alias/provenance?
