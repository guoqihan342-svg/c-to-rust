...
- direct call / stdlib 模型边界：`assert(int)`、`abs(int)`、target-ABI-bound `strlen(const char *)` 是显式最小模型；其它 C macro / stdlib / extern surface 仍需要单独模型或 signature/source binding，否则 fail-closed。`strlen` 当前只有 typed IR unit、no-clang fixture replay、可选 real-clang smoke 和 emitted-Rust compile 覆盖，尚没有 `strlen` 专属 named-slice L3 C/Rust diff 或 semantic pass。
