...
- FlashDB accepted evidence：`real-fdb-blob-make` 当前仍通过 L4 accepted-evidence authoritative 语义绑定；`real-fdb-calc-crc32`、`real-fdb-blob-make`、`real-fdb-kv-del` 和 `real-fdb-kv-set` 的 exact typed-IR generated Rust draft 已通过 `generated_draft_acceptance.status=passed`。当前 coverage matrix 另计入 `zlib-ng/adler32-step` 和一组 demo exact typed-IR drafts，派生 `translator_generated_semantic_pass_count=18`。raw C2Rust baseline 仍是 `candidate_context_only`，不能写成 translator-generated accepted draft。
- FlashDB KV 边界：`real-fdb-kv-set` 只关闭 uninitialized DB `return_code=FDB_INIT_FAILED` fixture；initialized set/delete path、blob 持久化、`fdb_kv_set_blob` 和完整 callee 语义仍未关闭。`real-fdb-kv-del` 也只关闭 uninitialized DB fixture，initialized delete path 和完整 `del_kv` 语义仍未关闭。
