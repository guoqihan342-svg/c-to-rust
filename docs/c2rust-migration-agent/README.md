...
- FlashDB accepted evidence：`real-fdb-blob-make` 当前仍通过 L4 accepted-evidence authoritative 语义绑定；`real-fdb-calc-crc32` 与 `real-fdb-blob-make` 的 exact typed-IR generated Rust draft 已通过 `generated_draft_acceptance.status=passed`。当前 coverage matrix 另计入 `zlib-ng/adler32-step`、`demo/signed-rshift-contract`、`demo/call-expression`、`demo/store-add-one`、`demo/add-one` 和 `demo/copy-i32-ptr-arith`，`translator_generated_semantic_pass_count=8`。raw C2Rust baseline 仍是 `candidate_context_only`，不能写成 translator-generated accepted draft。
