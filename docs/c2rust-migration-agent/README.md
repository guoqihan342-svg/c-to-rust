...
- 多声明展开：普通 compound body 中的 `DeclStmt` 现在可把多个简单 `VarDecl` 按源码顺序展开成连续 typed IR `Decl`，例如 `int a = 1, b = 2;`；`ForStmt` init 多声明、unsupported type/initializer、VLA、重复符号和 semantic acceptance 仍保持 fail-closed。
