...
- Standalone inc/dec statement：普通 `value++`、`++value`、`value--`、`--value` 在表达式值被丢弃的 statement 位置可 lowering 成 typed IR assignment，并发射为 `value = value +/- 1`；direct by-value record dot-field statement target（例如 `p.x++` / `--p.x`）和 direct mutable record pointer field statement target（例如 `p->x++` / `--p->x`）也可在各自 gate 下 lowering 成 `field = field +/- 1`。这只覆盖简单整数变量 target、direct by-value record dot-field integer target、direct single-pointer mutable record pointer scalar field target；不表示通用 value-position inc/dec、condition/return 中 inc/dec、call argument 中除单标量 direct-call 或单链 nested direct-call 子集之外的 inc/dec、`ForStmt` step 中 record-field inc/dec、array/复杂 field target 或完整 C 自增表达式语义已支持。
